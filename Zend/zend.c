...
static void himote_scanner_globals_ctor(zend_himote_scanner_globals *scanner_globals_p TSRMLS_DC) /* {{{ */
{
	memset(scanner_globals_p, 0, sizeof(*scanner_globals_p));
}
/* }}} */
...
int zend_startup(zend_utility_functions *utility_functions, char **extensions TSRMLS_DC) /* {{{ */
{
	...
	extern zend_himote_scanner_globals language_scanner_globals;
#endif
	...
	ts_allocate_id(&language_scanner_globals_id, sizeof(zend_himote_scanner_globals), (ts_allocate_ctor) himote_scanner_globals_ctor, NULL);
	...
	*executor_globals->zend_constants = *GLOBAL_CONSTANTS_TABLE;
	...
	himote_scanner_globals_ctor(&language_scanner_globals TSRMLS_CC);
	zend_set_default_compile_time_values(TSRMLS_C);
	EG(user_error_handler) = NULL;
	EG(user_exception_handler) = NULL;
#endif
...
# endif
			...
			/* User error handler may include() additinal HIMOTE files.
			 * If an error was generated during comilation HIMOTE will compile
