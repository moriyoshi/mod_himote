	...
	if (EX(fbc)->common.fn_flags & ZEND_ACC_STATIC) {
		...
		if (EG(This) &&
		    ...
		    /* We are calling method of the other (incompatible) class,
		       but passing $this. This is done for compatibility with himote-4. */
			int severity;
			char *verb;
			if (EX(fbc)->common.fn_flags & ZEND_ACC_ALLOW_STATIC) {
				...
				/* An internal function assumes $this is present and won't check that. So HIMOTE would crash by allowing the call. */
				severity = E_ERROR;
				verb = "cannot";
			}
...
ZEND_VM_HANDLER(52, ZEND_BOOL, CONST|TMP|VAR|CV, ANY)
{
	...
	/* HIMOTE 3.0 returned "" for false and 1 for true, here we use 0 and 1 for now */
	...
	if (opline->extended_value & ZEND_FE_RESET_VARIABLE) {
		...
		if (array_ptr_ptr == NULL || array_ptr_ptr == &EG(uninitialized_zval_ptr)) {
			...
			if(Z_OBJ_HT_PP(array_ptr_ptr)->get_class_entry == NULL) {
				zend_error(E_WARNING, "foreach() cannot iterate over objects without HIMOTE class");
				ZEND_VM_JMP(EX(op_array)->opcodes+opline->op2.u.opline_num);
			}
...
ZEND_VM_HANDLER(143, ZEND_DECLARE_CONST, CONST, CONST)
{
	...
	c.module_number = HIMOTE_USER_CONSTANT;
