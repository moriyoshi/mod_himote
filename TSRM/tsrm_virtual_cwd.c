...
static int himote_check_dots(const char *element, int n)
{
...
#define IS_DIRECTORY_UP(element, len) \
	(len >= 2 && !himote_check_dots(element, len))
...
/* strtok() call in LibC is abending when used in a different address space -- hence using
   HIMOTE's version itself for now */
...
#ifndef IS_DIR_OK
#define IS_DIR_OK(state) (himote_is_dir_ok(state) == 0)
#endif
...
CWD_API int himote_sys_readlink(const char *link, char *target, size_t target_len){ /* {{{ */
	HINSTANCE kernel32;
	HANDLE hFile;
	DWORD dwRet;
...
CWD_API int himote_sys_stat(const char *path, struct stat *buf) /* {{{ */
{
	return himote_sys_stat_ex(path, buf, 0);
}
/* }}} */

CWD_API int himote_sys_lstat(const char *path, struct stat *buf) /* {{{ */
{
	return himote_sys_stat_ex(path, buf, 1);
}
/* }}} */

CWD_API int himote_sys_stat_ex(const char *path, struct stat *buf, int lstat) /* {{{ */
{
...
static int himote_is_dir_ok(const cwd_state *state)  /* {{{ */
{
	struct stat buf;

	if (himote_sys_stat(state->cwd, &buf) == 0 && S_ISDIR(buf.st_mode))
		return (0);
...
static int himote_is_file_ok(const cwd_state *state)  /* {{{ */
{
	struct stat buf;

	if (himote_sys_stat(state->cwd, &buf) == 0 && S_ISREG(buf.st_mode))
		return (0);
...
static int tsrm_realpath_r(char *path, int start, int len, int *ll, time_t *t, int use_realpath, int is_dir, int *link_is_dir TSRMLS_DC) /* {{{ */
{
	...
	while (1) {
		...
		if(save &&
			...
			if(isabsolute && substitutename_len > 4) {
				/* Do not resolve volumes (for now). A mounted point can
				   ...
				   all IO functions we use in himote and its deps support
		...
		memcpy(tmp, path, len+1);
#else
		if (save && himote_sys_lstat(path, &st) < 0) {
			if (use_realpath == CWD_REALPATH) {
				/* file not found */
				return -1;
			}
		...
		if (save && S_ISLNK(st.st_mode)) {
			if (++(*ll) > LINK_MAX || (j = himote_sys_readlink(tmp, path, MAXPATHLEN)) < 0) {
...
{
	return virtual_file_ex(&CWDG(cwd), path, himote_is_dir_ok, CWD_REALPATH)?-1:0;
}
/* }}} */
...
{
	return virtual_filepath_ex(path, filepath, himote_is_file_ok TSRMLS_CC);
}
/* }}} */
...
CWD_API int virtual_stat(const char *path, struct stat *buf TSRMLS_DC) /* {{{ */
{
	...
	retval = himote_sys_stat(new_state.cwd, buf);
...
CWD_API int virtual_lstat(const char *path, struct stat *buf TSRMLS_DC) /* {{{ */
{
	...
	retval = himote_sys_lstat(new_state.cwd, buf);
