...
#include "himote_string.h" /* for himote_memnstr, used by himote_stream_get_record() */
#include <stddef.h>
#include <fcntl.h>
...
static int _himote_stream_release_context(zend_rsrc_list_entry *le, void *pContext TSRMLS_DC)
{
	if (le->ptr == pContext) {
...
fprintf(stderr, "forget_persistent: %s:%p\n", stream->ops->label, stream);
	...
	if (stream->context) {
		zend_hash_apply_with_argument(&EG(regular_list),
				(apply_func_arg_t) _himote_stream_release_context,
...
HIMOTEAPI int himote_stream_from_persistent_id(const char *persistent_id, himote_stream **stream TSRMLS_DC)
{
	...
	if (zend_hash_find(&EG(persistent_list), (char*)persistent_id, strlen(persistent_id)+1, (void*) &le) == SUCCESS) {
		if (Z_TYPE_P(le) == le_pstream) {
			...
			return HIMOTE_STREAM_PERSISTENT_SUCCESS;
		}
		return HIMOTE_STREAM_PERSISTENT_FAILURE;
	}
	return HIMOTE_STREAM_PERSISTENT_NOT_EXIST;
}

/* }}} */

static zend_llist *himote_get_wrapper_errors_list(himote_stream_wrapper *wrapper TSRMLS_DC)
{
...
void himote_stream_display_wrapper_errors(himote_stream_wrapper *wrapper, const char *path, const char *caption TSRMLS_DC)
{
	...
	if (wrapper) {
		zend_llist *err_list = himote_get_wrapper_errors_list(wrapper TSRMLS_CC);
		if (err_list) {
			size_t l = 0;
			int brlen;
			int i;
			...
			if (wrapper == &himote_plain_files_wrapper) {
	...
	himote_strip_url_passwd(tmp);
	himote_error_docref1(NULL TSRMLS_CC, tmp, E_WARNING, "%s: %s", caption, msg);
	efree(tmp);
	if (free_msg) {
		efree(msg);
	}
...
void himote_stream_tidy_wrapper_error_log(himote_stream_wrapper *wrapper TSRMLS_DC)
{
...
HIMOTEAPI void himote_stream_wrapper_log_error(himote_stream_wrapper *wrapper, int options TSRMLS_DC, const char *fmt, ...)
{
	va_list args;
	char *buffer = NULL;
	...
	if (options & REPORT_ERRORS || wrapper == NULL) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", buffer);
		efree(buffer);
	} else {
		zend_llist *list = NULL;
		if (!FG(wrapper_errors)) {
...
HIMOTEAPI himote_stream *_himote_stream_alloc(himote_stream_ops *ops, void *abstract, const char *persistent_id, const char *mode STREAMS_DC TSRMLS_DC) /* {{{ */
{
	himote_stream *ret;

	ret = (himote_stream*) pemalloc_rel_orig(sizeof(himote_stream), persistent_id ? 1 : 0);

	memset(ret, 0, sizeof(himote_stream));
...
fprintf(stderr, "stream_alloc: %s:%p persistent=%s\n", ops->label, ret, persistent_id);
	...
	if (FG(auto_detect_line_endings)) {
		ret->flags |= HIMOTE_STREAM_FLAG_DETECT_EOL;
	}
...
static int _himote_stream_free_persistent(zend_rsrc_list_entry *le, void *pStream TSRMLS_DC)
{
...
HIMOTEAPI int _himote_stream_free(himote_stream *stream, int close_options TSRMLS_DC) /* {{{ */
{
	int ret = 1;
	int remove_rsrc = 1;
	int preserve_handle = close_options & HIMOTE_STREAM_FREE_PRESERVE_HANDLE ? 1 : 0;
	int release_cast = 1;
	himote_stream_context *context = NULL;
	...
	if (stream->flags & HIMOTE_STREAM_FLAG_NO_CLOSE) {
		preserve_handle = 1;
	}
...
fprintf(stderr, "stream_free: %s:%p[%s] in_free=%d opts=%08x\n", stream->ops->label, stream, stream->orig_path, stream->in_free, close_options);
	 ...
	 * */
	if (preserve_handle) {
		if (stream->fclose_stdiocast == HIMOTE_STREAM_FCLOSE_FOPENCOOKIE) {
...
fprintf(stderr, "stream_free: %s:%p[%s] preserve_handle=%d release_cast=%d remove_rsrc=%d\n",
	...
	_himote_stream_flush(stream, 1 TSRMLS_CC);

	/* If not called from the resource dtor, remove the stream from the resource list. */
	if ((close_options & HIMOTE_STREAM_FREE_RSRC_DTOR) == 0 && remove_rsrc) {
		 ...
		 * not only when HIMOTE_STREAM_FREE_RELEASE_STREAM is set */
		while (zend_list_delete(stream->rsrc_id) == SUCCESS) {}
	}

	/* Remove stream from any context link list */
	if (context && context->links) {
		himote_stream_context_del_link(context, stream);
	}

	if (close_options & HIMOTE_STREAM_FREE_CALL_DTOR) {
		if (release_cast && stream->fclose_stdiocast == HIMOTE_STREAM_FCLOSE_FOPENCOOKIE) {
			/* calling fclose on an fopencookied stream will ultimately
				...
				we can be sure that we only reach here when HIMOTE code calls
				himote_stream_free.
		...
		if (release_cast && stream->fclose_stdiocast == HIMOTE_STREAM_FCLOSE_FDOPEN && stream->stdiocast) {
	...
	if (close_options & HIMOTE_STREAM_FREE_RELEASE_STREAM) {
		while (stream->readfilters.head) {
			himote_stream_filter_remove(stream->readfilters.head, 1 TSRMLS_CC);
		}
		while (stream->writefilters.head) {
			himote_stream_filter_remove(stream->writefilters.head, 1 TSRMLS_CC);
		}
		...
		if (stream->is_persistent && (close_options & HIMOTE_STREAM_FREE_PERSISTENT)) {
			/* we don't work with *stream but need its value for comparison */
			zend_hash_apply_with_argument(&EG(persistent_list), (apply_func_arg_t) _himote_stream_free_persistent, stream TSRMLS_CC);
		}
#if ZEND_DEBUG
		if ((close_options & HIMOTE_STREAM_FREE_RSRC_DTOR) && (stream->__exposed == 0) && (EG(error_reporting) & E_WARNING)) {
...
static void himote_stream_fill_read_buffer(himote_stream *stream, size_t size TSRMLS_DC)
{
	...
	if (stream->readfilters.head) {
		char *chunk_buf;
		int err_flag = 0;
		himote_stream_bucket_brigade brig_in = { NULL, NULL }, brig_out = { NULL, NULL };
		himote_stream_bucket_brigade *brig_inp = &brig_in, *brig_outp = &brig_out, *brig_swap;

		/* Invalidate the existing cache, otherwise reads can fail, see note in
		   main/streams/filter.c::_himote_stream_filter_append */
		...
		while (!stream->eof && !err_flag && (stream->writepos - stream->readpos < (off_t)size)) {
			...
			himote_stream_filter_status_t status = PSFS_ERR_FATAL;
			himote_stream_filter *filter;

			/* read a chunk into a bucket */
			justread = stream->ops->read(stream, chunk_buf, stream->chunk_size TSRMLS_CC);
			if (justread && justread != (size_t)-1) {
				bucket = himote_stream_bucket_new(stream, chunk_buf, justread, 0, 0 TSRMLS_CC);

				/* after this call, bucket is owned by the brigade */
				himote_stream_bucket_append(brig_inp, bucket TSRMLS_CC);
...
HIMOTEAPI size_t _himote_stream_read(himote_stream *stream, char *buf, size_t size TSRMLS_DC)
{
	...
	while (size > 0) {
		...
		if (!stream->readfilters.head && (stream->flags & HIMOTE_STREAM_FLAG_NO_BUFFER || stream->chunk_size == 1)) {
			toread = stream->ops->read(stream, buf, size TSRMLS_CC);
		} else {
			himote_stream_fill_read_buffer(stream, size TSRMLS_CC);
		...
		if (stream->wrapper != &himote_plain_files_wrapper) {
			break;
		}
	}
...
HIMOTEAPI int _himote_stream_eof(himote_stream *stream TSRMLS_DC)
{
	...
	if (!stream->eof && HIMOTE_STREAM_OPTION_RETURN_ERR ==
		   	himote_stream_set_option(stream, HIMOTE_STREAM_OPTION_CHECK_LIVENESS,
		   	0, NULL)) {
		stream->eof = 1;
	}
...
HIMOTEAPI int _himote_stream_putc(himote_stream *stream, int c TSRMLS_DC)
{
	...
	if (himote_stream_write(stream, &buf, 1) > 0) {
		return 1;
	}
	return EOF;
}
...
HIMOTEAPI int _himote_stream_getc(himote_stream *stream TSRMLS_DC)
{
	...
	if (himote_stream_read(stream, &buf, 1) > 0) {
		return buf & 0xff;
	}
	return EOF;
}
...
HIMOTEAPI int _himote_stream_puts(himote_stream *stream, char *buf TSRMLS_DC)
{
	...
	if (len > 0 && himote_stream_write(stream, buf, len) && himote_stream_write(stream, newline, 1)) {
		return 1;
	}
	return 0;
}
...
HIMOTEAPI char *himote_stream_locate_eol(himote_stream *stream, char *buf, size_t buf_len TSRMLS_DC)
{
	...
	/* Look for EOL */
	if (stream->flags & HIMOTE_STREAM_FLAG_DETECT_EOL) {
		cr = memchr(readptr, '\r', avail);
		lf = memchr(readptr, '\n', avail);

		if (cr && lf != cr + 1 && !(lf && lf < cr)) {
			/* mac */
			stream->flags ^= HIMOTE_STREAM_FLAG_DETECT_EOL;
			stream->flags |= HIMOTE_STREAM_FLAG_EOL_MAC;
			eol = cr;
	...
	} else if (stream->flags & HIMOTE_STREAM_FLAG_EOL_MAC) {
 ...
 * permitting */
HIMOTEAPI char *_himote_stream_get_line(himote_stream *stream, char *buf, size_t maxlen,
		size_t *returned_len TSRMLS_DC)
{
	...
	for (;;) {
		...
		if (avail > 0) {
			...
			readptr = stream->readbuf + stream->readpos;
			eol = himote_stream_locate_eol(stream, NULL, 0 TSRMLS_CC);
			...
			himote_stream_fill_read_buffer(stream, toread TSRMLS_CC);
...
static char *_himote_stream_search_delim(himote_stream *stream,
									  size_t maxlen,
									  size_t skiplen,
									  ...
									  size_t delim_len TSRMLS_DC)
{
	...
	if (delim_len == 1) {
		...
		return himote_memnstr((char*)&stream->readbuf[stream->readpos + skiplen],
				delim, delim_len,
...
HIMOTEAPI char *himote_stream_get_record(himote_stream *stream, size_t maxlen, size_t *returned_len, char *delim, size_t delim_len TSRMLS_DC)
{
	...
	if (has_delim) {
		found_delim = _himote_stream_search_delim(
	...
	while (!found_delim && buffered_len < maxlen) {
		...
		himote_stream_fill_read_buffer(stream, buffered_len + to_read_now TSRMLS_CC);
	...
	/* himote_stream_read will not call ops->read here because the necessary
	 * data is guaranteedly buffered */
	*returned_len = himote_stream_read(stream, ret_buf, tent_ret_len);
...
static size_t _himote_stream_write_buffer(himote_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
	...
	if (stream->ops->seek && (stream->flags & HIMOTE_STREAM_FLAG_NO_SEEK) == 0 && stream->readpos != stream->writepos) {
 ...
 * */
static size_t _himote_stream_write_filtered(himote_stream *stream, const char *buf, size_t count, int flags TSRMLS_DC)
{
	size_t consumed = 0;
	himote_stream_bucket *bucket;
	himote_stream_bucket_brigade brig_in = { NULL, NULL }, brig_out = { NULL, NULL };
	...
	if (buf) {
		bucket = himote_stream_bucket_new(stream, (char *)buf, count, 0, 0 TSRMLS_CC);
		himote_stream_bucket_append(&brig_in, bucket TSRMLS_CC);
	}
	...
	switch (status) {
		case PSFS_PASS_ON:
			...
			while (brig_inp->head) {
				bucket = brig_inp->head;
				_himote_stream_write_buffer(stream, bucket->buf, bucket->buflen TSRMLS_CC);
...
HIMOTEAPI int _himote_stream_flush(himote_stream *stream, int closing TSRMLS_DC)
{
	...
	if (stream->writefilters.head) {
		_himote_stream_write_filtered(stream, NULL, 0, closing ? PSFS_FLAG_FLUSH_CLOSE : PSFS_FLAG_FLUSH_INC  TSRMLS_CC);
	}
...
HIMOTEAPI size_t _himote_stream_write(himote_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
	...
	if (stream->writefilters.head) {
		return _himote_stream_write_filtered(stream, buf, count, PSFS_FLAG_NORMAL TSRMLS_CC);
	} else {
		return _himote_stream_write_buffer(stream, buf, count TSRMLS_CC);
	}
}

HIMOTEAPI size_t _himote_stream_printf(himote_stream *stream TSRMLS_DC, const char *fmt, ...)
{
	size_t count;
	char *buf;
	va_list ap;
	...
	count = himote_stream_write(stream, buf, count);
	efree(buf);
...
HIMOTEAPI int _himote_stream_seek(himote_stream *stream, off_t offset, int whence TSRMLS_DC)
{
	...
	if ((stream->flags & HIMOTE_STREAM_FLAG_NO_BUFFER) == 0) {
		switch(whence) {
			case SEEK_CUR:
	...
	if (stream->ops->seek && (stream->flags & HIMOTE_STREAM_FLAG_NO_SEEK) == 0) {
		...
		if (stream->writefilters.head) {
			_himote_stream_flush(stream, 0 TSRMLS_CC);
		}
		...
		if (((stream->flags & HIMOTE_STREAM_FLAG_NO_SEEK) == 0) || ret == 0) {
			if (ret == 0) {
				stream->eof = 0;
			}
	...
	if (whence == SEEK_CUR && offset >= 0) {
		...
		size_t didread;
		while(offset > 0) {
			if ((didread = himote_stream_read(stream, tmp, MIN(offset, sizeof(tmp)))) == 0) {
				return -1;
			}
			offset -= didread;
		}
	...
	himote_error_docref(NULL TSRMLS_CC, E_WARNING, "stream does not support seeking");
...
{
	int ret = HIMOTE_STREAM_OPTION_RETURN_NOTIMPL;

	if (stream->ops->set_option) {
		ret = stream->ops->set_option(stream, option, value, ptrparam TSRMLS_CC);
	}

	if (ret == HIMOTE_STREAM_OPTION_RETURN_NOTIMPL) {
		switch(option) {
			case HIMOTE_STREAM_OPTION_SET_CHUNK_SIZE:
				ret = stream->chunk_size;
				stream->chunk_size = value;
				return ret;

			case HIMOTE_STREAM_OPTION_READ_BUFFER:
				/* try to match the buffer mode as best we can */
				if (value == HIMOTE_STREAM_BUFFER_NONE) {
					stream->flags |= HIMOTE_STREAM_FLAG_NO_BUFFER;
				} else if (stream->flags & HIMOTE_STREAM_FLAG_NO_BUFFER) {
					stream->flags ^= HIMOTE_STREAM_FLAG_NO_BUFFER;
				}
				ret = HIMOTE_STREAM_OPTION_RETURN_OK;
				break;
...
{
	return himote_stream_set_option(stream, HIMOTE_STREAM_OPTION_TRUNCATE_API, HIMOTE_STREAM_TRUNCATE_SET_SIZE, &newsize);
}

HIMOTEAPI size_t _himote_stream_passthru(himote_stream * stream STREAMS_DC TSRMLS_DC)
{
	size_t bcount = 0;
	char buf[8192];
	int b;

	if (himote_stream_mmap_possible(stream)) {
		char *p;
		size_t mapped;

		p = himote_stream_mmap_range(stream, himote_stream_tell(stream), HIMOTE_STREAM_MMAP_ALL, HIMOTE_STREAM_MAP_MODE_SHARED_READONLY, &mapped);

		if (p) {
			HIMOTEWRITE(p, mapped);

			himote_stream_mmap_unmap_ex(stream, mapped);

			return mapped;
		}
	}

	while ((b = himote_stream_read(stream, buf, sizeof(buf))) > 0) {
		HIMOTEWRITE(buf, b);
		bcount += b;
	}
...
HIMOTEAPI size_t _himote_stream_copy_to_mem(himote_stream *src, char **buf, size_t maxlen, int persistent STREAMS_DC TSRMLS_DC)
{
	size_t ret = 0;
	char *ptr;
	...
	himote_stream_statbuf ssbuf;

	if (maxlen == 0) {
		return 0;
	}

	if (maxlen == HIMOTE_STREAM_COPY_ALL) {
		maxlen = 0;
	}

	if (maxlen > 0) {
		ptr = *buf = pemalloc_rel_orig(maxlen + 1, persistent);
		while ((len < maxlen) && !himote_stream_eof(src)) {
			ret = himote_stream_read(src, ptr, maxlen - len);
			if (!ret) {
				break;
			}
			len += ret;
	 ...
	 * 2K).  */
	if (himote_stream_stat(src, &ssbuf) == 0 && ssbuf.sb.st_size > 0) {
	...
	while((ret = himote_stream_read(src, ptr, max_len - len)))	{
		len += ret;
...
HIMOTEAPI size_t _himote_stream_copy_to_stream_ex(himote_stream *src, himote_stream *dest, size_t maxlen, size_t *len STREAMS_DC TSRMLS_DC)
{
	...
	if (himote_stream_stat(src, &ssbuf) == 0) {
	...
	if (himote_stream_mmap_possible(src)) {
		char *p;
		size_t mapped;

		p = himote_stream_mmap_range(src, himote_stream_tell(src), maxlen, HIMOTE_STREAM_MAP_MODE_SHARED_READONLY, &mapped);

		if (p) {
			mapped = himote_stream_write(dest, p, mapped);

			himote_stream_mmap_unmap_ex(src, mapped);
	...
	while(1) {
		...
		didread = himote_stream_read(src, buf, readchunk);
		...
		if (didread) {
			...
			while(towrite) {
				didwrite = himote_stream_write(dest, writeptr, towrite);
				if (didwrite == 0) {
 ...
 * Deprecated in favor of himote_stream_copy_to_stream_ex() */
...
{
	size_t len;
	int ret = _himote_stream_copy_to_stream_ex(src, dest, maxlen, &len STREAMS_REL_CC TSRMLS_CC);
...
{
	himote_stream *stream = (himote_stream*)rsrc->ptr;
	/* set the return value for pclose */
	FG(pclose_ret) = himote_stream_free(stream, HIMOTE_STREAM_FREE_CLOSE | HIMOTE_STREAM_FREE_RSRC_DTOR);
}
...
int himote_init_stream_wrappers(int module_number TSRMLS_DC)
{
	...
	return (
			...
			&&
			zend_hash_init(himote_get_stream_filters_hash_global(), 0, NULL, NULL, 1) == SUCCESS
			&&
			zend_hash_init(himote_stream_xport_get_hash(), 0, NULL, NULL, 1) == SUCCESS
			&&
			himote_stream_xport_register("tcp", himote_stream_generic_socket_factory TSRMLS_CC) == SUCCESS
			&&
			himote_stream_xport_register("udp", himote_stream_generic_socket_factory TSRMLS_CC) == SUCCESS
#if defined(AF_UNIX) && !(defined(HIMOTE_WIN32) || defined(__riscos__) || defined(NETWARE))
			&&
			himote_stream_xport_register("unix", himote_stream_generic_socket_factory TSRMLS_CC) == SUCCESS
			&&
			himote_stream_xport_register("udg", himote_stream_generic_socket_factory TSRMLS_CC) == SUCCESS
#endif
		) ? SUCCESS : FAILURE;
}
...
int himote_shutdown_stream_wrappers(int module_number TSRMLS_DC)
{
	...
	zend_hash_destroy(himote_get_stream_filters_hash_global());
	zend_hash_destroy(himote_stream_xport_get_hash());
	return SUCCESS;
}
 ...
 */
static inline int himote_stream_wrapper_scheme_validate(char *protocol, int protocol_len)
{
	int i;
...
HIMOTEAPI int himote_register_url_stream_wrapper(char *protocol, himote_stream_wrapper *wrapper TSRMLS_DC)
{
	...
	if (himote_stream_wrapper_scheme_validate(protocol, protocol_len) == FAILURE) {
		return FAILURE;
	}
...
/* {{{ himote_stream_locate_url_wrapper */
HIMOTEAPI himote_stream_wrapper *himote_stream_locate_url_wrapper(const char *path, char **path_for_open, int options TSRMLS_DC)
{
	...
	himote_stream_wrapper **wrapperpp = NULL;
	...
	if (options & IGNORE_URL) {
		return (options & STREAM_LOCATE_WRAPPERS_ONLY) ? NULL : &himote_plain_files_wrapper;
	}
	...
	if ((*p == ':') && (n > 1) && (!strncmp("//", p+1, 2) || (n == 4 && !memcmp("data:", path, 5)))) {
		...
		/* BC with older himote scripts and zlib wrapper */
		protocol = "compress.zlib";
		n = 13;
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Use of \"zlib:\" wrapper is deprecated; please use \"compress.zlib://\" instead");
	}

	if (protocol) {
		char *tmp = estrndup(protocol, n);
		if (FAILURE == zend_hash_find(wrapper_hash, (char*)tmp, n + 1, (void**)&wrapperpp)) {
			himote_strtolower(tmp, n);
			if (FAILURE == zend_hash_find(wrapper_hash, (char*)tmp, n + 1, (void**)&wrapperpp)) {
				...
				HIMOTE_STRLCPY(wrapper_name, protocol, sizeof(wrapper_name), n);

				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find the wrapper \"%s\" - did you forget to enable it when you configured HIMOTE?", wrapper_name);
	...
	if (!protocol || !strncasecmp(protocol, "file", n))	{
		...
		himote_stream_wrapper *plain_files_wrapper = &himote_plain_files_wrapper;
		...
		if (protocol) {
			...
			if (localhost == 0 && path[n+3] != '\0' && path[n+3] != '/') {
				...
				if (options & REPORT_ERRORS) {
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "remote host file access not supported, %s", path);
				}
				return NULL;
			}
		...
		/* The file:// wrapper may have been disabled/overridden */
			...
			if (options & REPORT_ERRORS) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "file:// wrapper is disabled in the server configuration");
			}
			return NULL;
		}
	...
	if (wrapperpp && (*wrapperpp)->is_url &&
		...
		if (options & REPORT_ERRORS) {
			...
			if (!PG(allow_url_fopen)) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%s:// wrapper is disabled in the server configuration by allow_url_fopen=0", protocol_dup);
			} else {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%s:// wrapper is disabled in the server configuration by allow_url_include=0", protocol_dup);
			}
			efree(protocol_dup);
		}
		return NULL;
...
/* {{{ _himote_stream_mkdir
 */
HIMOTEAPI int _himote_stream_mkdir(char *path, int mode, int options, himote_stream_context *context TSRMLS_DC)
{
	himote_stream_wrapper *wrapper = NULL;

	wrapper = himote_stream_locate_url_wrapper(path, NULL, ENFORCE_SAFE_MODE TSRMLS_CC);
...
HIMOTEAPI int _himote_stream_stat_path(char *path, int flags, himote_stream_statbuf *ssb, himote_stream_context *context TSRMLS_DC)
{
	...
	if (flags & HIMOTE_STREAM_URL_STAT_LINK) {
		if (BG(CurrentLStatFile) && strcmp(path, BG(CurrentLStatFile)) == 0) {
			memcpy(ssb, &BG(lssb), sizeof(himote_stream_statbuf));
			return 0;
		}
	} else {
		if (BG(CurrentStatFile) && strcmp(path, BG(CurrentStatFile)) == 0) {
			memcpy(ssb, &BG(ssb), sizeof(himote_stream_statbuf));
			return 0;
		}
	}

	wrapper = himote_stream_locate_url_wrapper(path, &path_to_open, ENFORCE_SAFE_MODE TSRMLS_CC);
	if (wrapper && wrapper->wops->url_stat) {
		...
		if (ret == 0) {
			...
			if (flags & HIMOTE_STREAM_URL_STAT_LINK) {
				...
				memcpy(&BG(lssb), ssb, sizeof(himote_stream_statbuf));
			} else {
				if (BG(CurrentStatFile)) {
					efree(BG(CurrentStatFile));
				}
				BG(CurrentStatFile) = estrdup(path);
				memcpy(&BG(ssb), ssb, sizeof(himote_stream_statbuf));
			}
		}
		return ret;
	}
...
/* {{{ himote_stream_opendir */
HIMOTEAPI himote_stream *_himote_stream_opendir(char *path, int options,
		himote_stream_context *context STREAMS_DC TSRMLS_DC)
{
	...
	wrapper = himote_stream_locate_url_wrapper(path, &path_to_open, options TSRMLS_CC);
	...
	if (wrapper && wrapper->wops->dir_opener) {
		...
		if (stream) {
			...
			stream->flags |= HIMOTE_STREAM_FLAG_NO_BUFFER | HIMOTE_STREAM_FLAG_IS_DIR;
		}
	} else if (wrapper) {
		himote_stream_wrapper_log_error(wrapper, options ^ REPORT_ERRORS TSRMLS_CC, "not implemented");
	}
	if (stream == NULL && (options & REPORT_ERRORS)) {
		himote_stream_display_wrapper_errors(wrapper, path, "failed to open dir" TSRMLS_CC);
	}
	himote_stream_tidy_wrapper_error_log(wrapper TSRMLS_CC);

	return stream;
}
/* }}} */

/* {{{ _himote_stream_readdir */
HIMOTEAPI himote_stream_dirent *_himote_stream_readdir(himote_stream *dirstream, himote_stream_dirent *ent TSRMLS_DC)
{

	if (sizeof(himote_stream_dirent) == himote_stream_read(dirstream, (char*)ent, sizeof(himote_stream_dirent))) {
		return ent;
	}
...
/* {{{ himote_stream_open_wrapper_ex */
HIMOTEAPI himote_stream *_himote_stream_open_wrapper_ex(char *path, char *mode, int options,
		char **opened_path, himote_stream_context *context STREAMS_DC TSRMLS_DC)
{
	himote_stream *stream = NULL;
	himote_stream_wrapper *wrapper = NULL;
	char *path_to_open;
	...
	if (options & STREAM_USE_URL && (!wrapper || !wrapper->is_url)) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "This function may only be used against URLs");
		if (resolved_path) {
			efree(resolved_path);
		}
		return NULL;
	...
	if (wrapper) {
		if (!wrapper->wops->stream_opener) {
			himote_stream_wrapper_log_error(wrapper, options ^ REPORT_ERRORS TSRMLS_CC,
	...
	if (stream != NULL && (options & STREAM_MUST_SEEK)) {
		...
		switch(himote_stream_make_seekable_rel(stream, &newstream,
					(options & STREAM_WILL_CAST)
						? HIMOTE_STREAM_PREFER_STDIO : HIMOTE_STREAM_NO_PREFERENCE)) {
			case HIMOTE_STREAM_UNCHANGED:
				if (resolved_path) {
					efree(resolved_path);
				}
				return stream;
			case HIMOTE_STREAM_RELEASED:
			...
			default:
				...
				if (options & REPORT_ERRORS) {
					char *tmp = estrdup(path);
					himote_strip_url_passwd(tmp);
					himote_error_docref1(NULL TSRMLS_CC, tmp, E_WARNING, "could not make seekable - %s",
							tmp);
					efree(tmp);
	...
	if (stream && stream->ops->seek && (stream->flags & HIMOTE_STREAM_FLAG_NO_SEEK) == 0 && strchr(mode, 'a') && stream->position == 0) {
		off_t newpos = 0;
	...
	if (stream == NULL && (options & REPORT_ERRORS)) {
		himote_stream_display_wrapper_errors(wrapper, path, "failed to open stream" TSRMLS_CC);
...
{
	himote_stream_context *oldcontext = stream->context;
	TSRMLS_FETCH();
...
HIMOTEAPI void himote_stream_context_free(himote_stream_context *context)
{
	...
	if (context->notifier) {
		himote_stream_notification_free(context->notifier);
...
HIMOTEAPI himote_stream_context *himote_stream_context_alloc(void)
{
	...
	context = ecalloc(1, sizeof(himote_stream_context));
	context->notifier = NULL;
	MAKE_STD_ZVAL(context->options);
	array_init(context->options);

	context->rsrc_id = ZEND_REGISTER_RESOURCE(NULL, context, himote_le_stream_context());
	return context;
}

HIMOTEAPI himote_stream_notifier *himote_stream_notification_alloc(void)
{
	return ecalloc(1, sizeof(himote_stream_notifier));
}
        ...
        const char *hostent, himote_stream *stream)
{
	...
	return zend_hash_update(Z_ARRVAL_P(context->links), (char*)hostent, strlen(hostent)+1, (void**)&stream, sizeof(himote_stream *), NULL);
}
...
/* {{{ himote_stream_dirent_alphasort
 */
...
/* {{{ himote_stream_dirent_alphasortr
 */
			  ...
			  int (*compare) (const char **a, const char **b) TSRMLS_DC)
{
	...
	stream = himote_stream_opendir(dirname, ENFORCE_SAFE_MODE | REPORT_ERRORS, context);
	if (!stream) {
		return FAILURE;
	}

	while (himote_stream_readdir(stream, &sdp)) {
