...
det_charset:
	...
	if (charset_hint) {
		...
		if (!found) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "charset `%s' not supported, assuming iso-8859-1",
					charset_hint);
		}
	}
	if (uf_result != NULL) {
...
/* {{{ himote_utf32_utf8 */
size_t himote_utf32_utf8(unsigned char *buf, unsigned k)
{
	size_t retval = 0;
...
HIMOTEAPI char *himote_unescape_html_entities(unsigned char *old, int oldlen, int *newlen, int all, int quote_style, char *hint_charset TSRMLS_DC)
{
			...
			for (k = entity_map[j].basechar; k <= entity_map[j].endchar; k++) {
				...
				switch (charset) {
					...
					case cs_eucjp:
						 ...
						 * with himote_str_to_str. skip it. */ 
						continue;

					case cs_utf_8:
						replacement_len = himote_utf32_utf8(replacement, k);
						break;

					default:
						himote_error_docref(NULL TSRMLS_CC, E_WARNING, "cannot yet handle MBCS!");
						efree(ret);
						return NULL;
				}

				if (himote_memnstr(ret, entity, entity_length, ret+retlen)) {
					replaced = himote_str_to_str(ret, retlen, entity, entity_length, replacement, replacement_len, &retlen);
					efree(ret);
					ret = replaced;
				}
			}
	...
	for (j = 0; basic_entities[j].charcode != 0; j++) {
		...
		if (himote_memnstr(ret, basic_entities[j].entity, basic_entities[j].entitylen, ret+retlen)) {		
			replaced = himote_str_to_str(ret, retlen, basic_entities[j].entity, basic_entities[j].entitylen, replacement, 1, &retlen);
			efree(ret);
			ret = replaced;
		}
	}
...
{
	return himote_escape_html_entities_ex(old, oldlen, newlen, all, quote_style, hint_charset, 1 TSRMLS_CC);
}


/* {{{ himote_escape_html_entities
 */
	...
	while (i < oldlen) {
		...
		if(status == FAILURE) {
			...
			if(!PG(display_errors)) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid multibyte sequence in argument");
			}
			*newlen = 0;
...
/* {{{ himote_html_entities
 */
static void himote_html_entities(INTERNAL_FUNCTION_PARAMETERS, int all)
{
	...
	replaced = himote_escape_html_entities_ex(str, str_len, &len, all, quote_style, hint_charset, double_encode TSRMLS_CC);
...
{
	himote_html_entities(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
HIMOTE_FUNCTION(html_entity_decode)
{
	...
	replaced = himote_unescape_html_entities(str, str_len, &len, 1, quote_style, hint_charset TSRMLS_CC);
	if (replaced) {
...
{
	himote_html_entities(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
	...
	case HTML_ENTITIES:
		for (j = 0; entity_map[j].charset != cs_terminator; j++) {
			...
			for (i = 0; i <= entity_map[j].endchar - entity_map[j].basechar; i++) {
				...
				switch (charset) {
				case cs_utf_8:
					written = himote_utf32_utf8(ind, k);
					ind[written] = '\0';
					break;
				case cs_big5:
				case cs_gb2312:
