...
#ifdef ZTS
#define DIRG(v) TSRMG(dir_globals_id, himote_dir_globals *, v)
int dir_globals_id;
#else
#define DIRG(v) (dir_globals.v)
himote_dir_globals dir_globals;
#endif
	...
	if (ZEND_NUM_ARGS() == 0) { \
		...
		if (myself) { \
			if (zend_hash_find(Z_OBJPROP_P(myself), "handle", sizeof("handle"), (void **)&tmp) == FAILURE) { \
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find my handle property"); \
				RETURN_FALSE; \
			} \
			ZEND_FETCH_RESOURCE(dirp, himote_stream *, tmp, -1, "Directory", himote_file_le_stream()); \
		} else { \
			ZEND_FETCH_RESOURCE(dirp, himote_stream *, 0, DIRG(default_dir), "Directory", himote_file_le_stream()); \
		} \
	} else { \
		dirp = (himote_stream *) zend_fetch_resource(&id TSRMLS_CC, -1, "Directory", NULL, 1, himote_file_le_stream()); \
		if (!dirp) \
			RETURN_FALSE; \
	} 
...
static const zend_function_entry himote_dir_class_functions[] = {
	HIMOTE_FALIAS(close,	closedir,		arginfo_dir)
	HIMOTE_FALIAS(rewind,	rewinddir,		arginfo_dir)
	HIMOTE_NAMED_FE(read,  himote_if_readdir, arginfo_dir)
	{NULL, NULL, NULL}
};


static void himote_set_default_dir(int id TSRMLS_DC)
{
...
HIMOTE_MINIT_FUNCTION(dir)
{
	...
	INIT_CLASS_ENTRY(dir_class_entry, "Directory", himote_dir_class_functions);
	dir_class_entry_ptr = zend_register_internal_class(&dir_class_entry TSRMLS_CC);

#ifdef ZTS
	ts_allocate_id(&dir_globals_id, sizeof(himote_dir_globals), NULL, NULL);
#endif
...
/* {{{ internal functions */
static void _himote_do_opendir(INTERNAL_FUNCTION_PARAMETERS, int createobject)
{
	char *dirname;
	int dir_len;
	zval *zcontext = NULL;
	...
	dirp = himote_stream_opendir(dirname, ENFORCE_SAFE_MODE|REPORT_ERRORS, context);

	if (dirp == NULL) {
		RETURN_FALSE;
	}

	dirp->flags |= HIMOTE_STREAM_FLAG_NO_FCLOSE;
		
	himote_set_default_dir(dirp->rsrc_id TSRMLS_CC);

	if (createobject) {
		object_init_ex(return_value, dir_class_entry_ptr);
		add_property_stringl(return_value, "path", dirname, dir_len, 1);
		add_property_resource(return_value, "handle", dirp->rsrc_id);
		himote_stream_auto_cleanup(dirp); /* so we don't get warnings under debug */
	} else {
		himote_stream_to_zval(dirp, return_value);
	}
}
/* }}} */
...
{
	_himote_do_opendir(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	_himote_do_opendir(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
HIMOTE_FUNCTION(closedir)
{
	...
	if (!(dirp->flags & HIMOTE_STREAM_FLAG_IS_DIR)) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%d is not a valid Directory resource", dirp->rsrc_id);
		RETURN_FALSE;
	}
	...
	if (rsrc_id == DIRG(default_dir)) {
		himote_set_default_dir(-1 TSRMLS_CC);
	}
}
/* }}} */
...
HIMOTE_FUNCTION(chroot)
{
	...
	ret = chroot(str);
	if (ret != 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%s (errno %d)", strerror(errno), errno);
		RETURN_FALSE;
	}
...
HIMOTE_NAMED_FUNCTION(himote_if_readdir)
{
	...
	if (himote_stream_readdir(dirp, &entry)) {
...
HIMOTE_FUNCTION(glob)
{
	...
	if ((GLOB_AVAILABLE_FLAGS & flags) != flags) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "At least one of the passed flags is invalid or not supported on this platform");
		RETURN_FALSE;
	}
	...
	if (0 != (ret = glob(pattern, flags & GLOB_FLAGMASK, NULL, &globbuf))) {
		...
		if (GLOB_NOMATCH == ret) {
			/* Some glob implementation simply return no data if no matches
			   ...
			   so that HIMOTE glob() behaves the same on both types of 
...
no_results:
	...
	for (n = 0; n < globbuf.gl_pathc; n++) {
		if (PG(safe_mode) || (PG(open_basedir) && *PG(open_basedir))) {
			if (himote_check_open_basedir_ex(globbuf.gl_pathv[n], 0 TSRMLS_CC)) {
				basedir_limit = 1;
				continue;
			}
		}
...
HIMOTE_FUNCTION(scandir)
{
	...
	if (dirn_len < 1) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Directory name cannot be empty");
		RETURN_FALSE;
	}
	...
	if (!flags) {
		n = himote_stream_scandir(dirn, &namelist, context, (void *) himote_stream_dirent_alphasort);
	} else {
		n = himote_stream_scandir(dirn, &namelist, context, (void *) himote_stream_dirent_alphasortr);
	}
	if (n < 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "(errno %d): %s", errno, strerror(errno));
		RETURN_FALSE;
	}
