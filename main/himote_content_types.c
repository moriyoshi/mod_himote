...
/* {{{ himote_post_entries[]
 */
static sapi_post_entry himote_post_entries[] = {
	{ DEFAULT_POST_CONTENT_TYPE, sizeof(DEFAULT_POST_CONTENT_TYPE)-1, sapi_read_standard_form_data,	himote_std_post_handler },
...
SAPI_API SAPI_POST_READER_FUNC(himote_default_post_reader)
{
	...
	/* for himote://input stream:
	 some post handlers modify the content of request_info.post_data
	 so for now we need a copy for the himote://input stream
...
/* {{{ himote_startup_sapi_content_types
 */
int himote_startup_sapi_content_types(TSRMLS_D)
{
	sapi_register_default_post_reader(himote_default_post_reader);
	sapi_register_treat_data(himote_default_treat_data);
	sapi_register_input_filter(himote_default_input_filter, NULL);
	return SUCCESS;
}
/* }}} */

/* {{{ himote_setup_sapi_content_types
 */
int himote_setup_sapi_content_types(TSRMLS_D)
{
	sapi_register_post_entries(himote_post_entries TSRMLS_CC);
