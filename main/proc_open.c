...
/* This symbol is defined in config.m4.
 * Essentially, it is set if you HAVE_FORK || HIMOTE_WIN32
...
# include <sys/ioctl.h>
# include <termios.h>
# define HIMOTE_CAN_DO_PTS	1
#endif

#include "himote_proc_open.h"

static int le_proc_open;

/* {{{ _himote_array_to_envp */
static himote_process_env_t _himote_array_to_envp(zval *environment, int is_persistent TSRMLS_DC)
{
	zval **element;
			...
			case HASH_KEY_IS_STRING:
				...
				if (PG(safe_mode)) {
					...
					if (zend_hash_exists(&BG(sm_protected_env_vars), string_key, string_length - 1)) {
						himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Safe Mode warning: Cannot override protected environment variable '%s'", string_key);
						return env;
					}
					...
					if (BG(sm_allowed_env_vars) && *BG(sm_allowed_env_vars)) {
						...
						char *allowed_prefix = himote_strtok_r(allowed_env_vars, ", ", &strtok_buf);
						zend_bool allowed = 0;
						...
						while (allowed_prefix) {
							...
							allowed_prefix = himote_strtok_r(NULL, ", ", &strtok_buf);
						}
						efree(allowed_env_vars);
						if (!allowed) {
							himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Safe Mode warning: Cannot set environment variable '%s' - it's not in the allowed list", string_key);
							return env;
						}
					}
				}
...
/* {{{ _himote_free_envp */
static void _himote_free_envp(himote_process_env_t env, int is_persistent)
{
#ifndef HIMOTE_WIN32
	if (env.envarray) {
...
{
	struct himote_process_handle *proc = (struct himote_process_handle*)rsrc->ptr;
	int i;
#ifdef HIMOTE_WIN32
	DWORD wstatus;
	...
	FG(pclose_ret) = -1;
#endif
	_himote_free_envp(proc->env, proc->is_persistent);
	pefree(proc->command, proc->is_persistent);
	pefree(proc, proc->is_persistent);
}
/* }}} */

/* {{{ himote_make_safe_mode_command */
static int himote_make_safe_mode_command(char *cmd, char **safecmd, int is_persistent TSRMLS_DC)
{
	int lcmd, larg0;
	char *space, *sep, *arg0;
	...
	if (himote_memnstr(arg0, "..", sizeof("..")-1, arg0 + larg0)) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "No '..' components allowed in path");
		efree(arg0);
		return FAILURE;
	}

	sep = zend_memrchr(arg0, HIMOTE_DIR_SEPARATOR, larg0);

	spprintf(safecmd, 0, "%s%s%s%s", PG(safe_mode_exec_dir), (sep ? sep : "/"), (sep ? "" : arg0), (space ? cmd + larg0 : ""));

	efree(arg0);
	arg0 = himote_escape_shell_cmd(*safecmd);
	efree(*safecmd);
	if (is_persistent) {
...
{
	zval *zproc;
	struct himote_process_handle *proc;
	long sig_no = SIGTERM;
	...
	ZEND_FETCH_RESOURCE(proc, struct himote_process_handle *, &zproc, -1, "process", le_proc_open);
...
struct himote_proc_open_descriptor_item {
	int index; 							/* desired fd number in child process */
	himote_file_descriptor_t parentend, childend;	/* fds for pipes in parent/child */
...
HIMOTE_FUNCTION(proc_open)
{
	...
	zval **descitem = NULL;
	HashPosition pos;
	struct himote_proc_open_descriptor_item descriptors[HIMOTE_PROC_OPEN_MAX_DESCRIPTORS];
#ifdef HIMOTE_WIN32
	...
	int bypass_shell = 0;
#endif
#if HIMOTE_CAN_DO_PTS
	himote_file_descriptor_t dev_ptmx = -1;	/* master */
	himote_file_descriptor_t slave_pty = -1;
#endif
	...
	if (FAILURE == himote_make_safe_mode_command(command, &command, is_persistent TSRMLS_CC)) {
		RETURN_FALSE;
	}
	...
	if (environment) {
		env = _himote_array_to_envp(environment, is_persistent TSRMLS_CC);
	} else {
		memset(&env, 0, sizeof(env));
	}
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(descriptorspec), (void **)&descitem, &pos) == SUCCESS) {
		...
		if (str_index) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "descriptor spec must be an integer indexed array");
			goto exit_fail;
		}
		...
		if (Z_TYPE_PP(descitem) == IS_RESOURCE) {
			...
			himote_stream_from_zval(stream, descitem);

			if (FAILURE == himote_stream_cast(stream, HIMOTE_STREAM_AS_FD, (void **)&fd, REPORT_ERRORS)) {
				goto exit_fail;
			}
			...
			if (descriptors[ndesc].childend == NULL) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to dup File-Handle for descriptor %d", nindex);
				goto exit_fail;
			}
#else
			descriptors[ndesc].childend = dup(fd);
			if (descriptors[ndesc].childend < 0) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to dup File-Handle for descriptor %ld - %s", nindex, strerror(errno));
				goto exit_fail;
			}
#endif
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Descriptor item must be either an array or a File-Handle");
			goto exit_fail;
		} else {
			...
			if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 0, (void **)&ztype) == SUCCESS) {
				...
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Missing handle qualifier in array");
				goto exit_fail;
			}

			if (strcmp(Z_STRVAL_PP(ztype), "pipe") == 0) {
				himote_file_descriptor_t newpipe[2];
				zval **zmode;

				if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 1, (void **)&zmode) == SUCCESS) {
					convert_to_string_ex(zmode);
				} else {
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Missing mode parameter for 'pipe'");
					goto exit_fail;
				}
				...
				if (0 != pipe(newpipe)) {
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to create pipe %s", strerror(errno));
					goto exit_fail;
				}
				...
				if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 1, (void **)&zfile) == SUCCESS) {
					...
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Missing file name parameter for 'file'");
					goto exit_fail;
				}
				...
				if (zend_hash_index_find(Z_ARRVAL_PP(descitem), 2, (void **)&zmode) == SUCCESS) {
					...
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Missing mode parameter for 'file'");
					goto exit_fail;
				}

				/* try a wrapper */
				stream = himote_stream_open_wrapper(Z_STRVAL_PP(zfile), Z_STRVAL_PP(zmode),
						ENFORCE_SAFE_MODE|REPORT_ERRORS|STREAM_WILL_CAST, NULL);

				/* force into an fd */
				if (stream == NULL || FAILURE == himote_stream_cast(stream,
							HIMOTE_STREAM_CAST_RELEASE|HIMOTE_STREAM_AS_FD,
				...
				if (dev_ptmx == -1) {
					...
					if (dev_ptmx == -1) {
						himote_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to open /dev/ptmx, errno %d", errno);
						goto exit_fail;
					}
					grantpt(dev_ptmx);
					unlockpt(dev_ptmx);
					...
					if (slave_pty == -1) {
						himote_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to open slave pty, errno %d", errno);
						goto exit_fail;
					}
				}
				descriptors[ndesc].mode = DESC_PIPE;
...
#else
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "pty pseudo terminal not supported on this system");
				goto exit_fail;
#endif
			} else {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%s is not a valid descriptor spec/mode", Z_STRVAL_PP(ztype));
				goto exit_fail;
			}
		}

		zend_hash_move_forward_ex(Z_ARRVAL_P(descriptorspec), &pos);
		if (++ndesc == HIMOTE_PROC_OPEN_MAX_DESCRIPTORS)
			break;
	}
	...
	if (cwd == NULL) {
		...
		if (!getcwd_result) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot get current directory");
			goto exit_fail;
		}
	}
	...
	if (FALSE == newprocok) {
		...
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "CreateProcess failed, error code - %u", dw);
		goto exit_fail;
	}
	...
	if (child < 0) {
		...
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "procve failed - %s", strerror(errno));
		goto exit_fail;
	}
#elif HAVE_FORK
	/* the unix way */
	...
	if (child == 0) {
		...
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "fork failed - %s", strerror(errno));
...
# error You lose (configure should not have let you get here)
	...
	proc = (struct himote_process_handle*)pemalloc(sizeof(struct himote_process_handle), is_persistent);
...
#ifdef HIMOTE_WIN32
				stream = himote_stream_fopen_from_fd(_open_osfhandle((zend_intptr_t)descriptors[i].parentend,
							descriptors[i].mode_flags), mode_string, NULL);
#else
				stream = himote_stream_fopen_from_fd(descriptors[i].parentend, mode_string, NULL);
...
# endif
				...
				if (stream) {
					...
					MAKE_STD_ZVAL(retfp);
					himote_stream_to_zval(stream, retfp);
...
exit_fail:
	_himote_free_envp(env, is_persistent);
...
#endif /* HIMOTE_CAN_SUPPORT_PROC_OPEN */
