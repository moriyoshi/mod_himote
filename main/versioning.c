...
/* {{{ himote_canonicalize_version() */

HIMOTEAPI char *
himote_canonicalize_version(const char *version)
{
...
/* }}} */
/* {{{ himote_version_compare() */

HIMOTEAPI int
himote_version_compare(const char *orig_ver1, const char *orig_ver2)
{
	char *ver1;
	char *ver2;
	char *p1, *p2, *n1, *n2;
	...
	if (orig_ver1[0] == '#') {
		...
		ver1 = himote_canonicalize_version(orig_ver1);
	}
	if (orig_ver2[0] == '#') {
		ver2 = estrdup(orig_ver2);
	} else {
		ver2 = himote_canonicalize_version(orig_ver2);
	}
	p1 = n1 = ver1;
	p2 = n2 = ver2;
	while (*p1 && *p2 && n1 && n2) {
	...
	if (compare == 0) {
		if (n1 != NULL) {
			if (isdigit(*p1)) {
				...
				compare = himote_version_compare(p1, "#N#");
			}
		} else if (n2 != NULL) {
			if (isdigit(*p2)) {
				compare = -1;
			} else {
				compare = himote_version_compare("#N#", p2);
			}
		}
	}
	efree(ver1);
...
/* {{{ proto int version_compare(string ver1, string ver2 [, string oper])
  Compares two "HIMOTE-standardized" version number strings */

HIMOTE_FUNCTION(version_compare)
{
	char *v1, *v2, *op = NULL;
	...
	compare = himote_version_compare(v1, v2);
	if (argc == 2) {
		RETURN_LONG(compare);
	}
