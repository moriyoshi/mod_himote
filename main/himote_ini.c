...
typedef struct _himote_extension_lists {
	zend_llist engine;
	zend_llist functions;
} himote_extension_lists;
...
HIMOTEAPI char *himote_ini_opened_path=NULL;
static himote_extension_lists extension_lists;
HIMOTEAPI char *himote_ini_scanned_path=NULL;
HIMOTEAPI char *himote_ini_scanned_files=NULL;

/* {{{ himote_ini_displayer_cb
 */
static void himote_ini_displayer_cb(zend_ini_entry *ini_entry, int type TSRMLS_DC)
{
	if (ini_entry->displayer) {
		...
		if (type == ZEND_INI_DISPLAY_ORIG && ini_entry->modified) {
			if (ini_entry->orig_value && ini_entry->orig_value[0]) {
				...
				esc_html = !sapi_module.himoteinfo_as_text;
			} else {
		...
		if (esc_html) {
			himote_html_puts(display_string, display_string_length TSRMLS_CC);
		} else {
			HIMOTEWRITE(display_string, display_string_length);
		}
	}
}
/* }}} */

/* {{{ himote_ini_displayer
 */
static int himote_ini_displayer(zend_ini_entry *ini_entry, int module_number TSRMLS_DC)
{
	...
	if (!sapi_module.himoteinfo_as_text) {
		PUTS("<tr>");
		PUTS("<td class=\"e\">");
		HIMOTEWRITE(ini_entry->name, ini_entry->name_length - 1);
		PUTS("</td><td class=\"v\">");
		himote_ini_displayer_cb(ini_entry, ZEND_INI_DISPLAY_ACTIVE TSRMLS_CC);
		PUTS("</td><td class=\"v\">");
		himote_ini_displayer_cb(ini_entry, ZEND_INI_DISPLAY_ORIG TSRMLS_CC);
		PUTS("</td></tr>\n");
	} else {
		HIMOTEWRITE(ini_entry->name, ini_entry->name_length - 1);
		PUTS(" => ");
...
/* {{{ himote_ini_available
 */
static int himote_ini_available(zend_ini_entry *ini_entry, int *module_number_available TSRMLS_DC)
{
...
/* {{{ display_ini_entries
 */
HIMOTEAPI void display_ini_entries(zend_module_entry *module)
{
	...
	zend_hash_apply_with_argument(EG(ini_directives), (apply_func_arg_t) himote_ini_available, &module_number_available TSRMLS_CC);
	if (module_number_available == -1) {
		himote_info_print_table_start();
		himote_info_print_table_header(3, "Directive", "Local Value", "Master Value");
		zend_hash_apply_with_argument(EG(ini_directives), (apply_func_arg_t) himote_ini_displayer, (void *) (zend_intptr_t) module_number TSRMLS_CC);
...
/* himote.ini support */
#define HIMOTE_EXTENSION_TOKEN		"extension"
...
/* {{{ himote_ini_parser_cb
 */
static void himote_ini_parser_cb(zval *arg1, zval *arg2, zval *arg3, int callback_type, HashTable *target_hash)
{
	zval *entry;
	...
	switch (callback_type) {
		case ZEND_INI_PARSER_ENTRY: {
				...
				/* HIMOTE and Zend extensions are not added into configuration hash! */
				if (!is_special_section && !strcasecmp(Z_STRVAL_P(arg1), HIMOTE_EXTENSION_TOKEN)) { /* load HIMOTE extension */
...
/* {{{ himote_load_himote_extension_cb
 */
static void himote_load_himote_extension_cb(void *arg TSRMLS_DC)
{
#ifdef HAVE_LIBDL
	himote_load_extension(*((char **) arg), MODULE_PERSISTENT, 0 TSRMLS_CC);
#endif
}
/* }}} */

/* {{{ himote_load_zend_extension_cb
 */
static void himote_load_zend_extension_cb(void *arg TSRMLS_DC)
{
...
{
	char *himote_ini_file_name = NULL;
	char *himote_ini_search_path = NULL;
	int himote_ini_scanned_path_len;
	int safe_mode_state;
	char *open_basedir;
	...
	if (sapi_module.himote_ini_path_override) {
		himote_ini_file_name = sapi_module.himote_ini_path_override;
		himote_ini_search_path = sapi_module.himote_ini_path_override;
		free_ini_search_path = 0;
	} else if (!sapi_module.himote_ini_ignore) {
...
#ifdef HIMOTE_WIN32
		char *reg_location;
		char himoterc_path[MAXPATHLEN];
#endif

		env_location = getenv("HIMOTERC");
		...
		if (!env_location) {
			...
			size = GetEnvironmentVariableA("HIMOTERC", &dummybuf, 0);
			if (GetLastError() == ERROR_ENVVAR_NOT_FOUND) {
				...
				if (size == 0) {
					...
					size = GetEnvironmentVariableA("HIMOTERC", himoterc_path, size);
					env_location = himoterc_path;
				}
			}
		}
#else
		...
		himote_ini_search_path = (char *) emalloc(search_path_size);
		free_ini_search_path = 1;
		himote_ini_search_path[0] = 0;

		/* Add environment location */
		if (env_location[0]) {
			if (*himote_ini_search_path) {
				strlcat(himote_ini_search_path, paths_separator, search_path_size);
			}
			strlcat(himote_ini_search_path, env_location, search_path_size);
			himote_ini_file_name = env_location;
		}
		...
		if (reg_location != NULL) {
			...
			strlcat(himote_ini_search_path, reg_location, search_path_size);
			efree(reg_location);
		}
#endif
		...
		if (strcmp(sapi_module.name, "cli") != 0) {
			...
			strlcat(himote_ini_search_path, ".", search_path_size);
		}
		...
		if (sapi_module.executable_location) {
			...
			if (!strchr(sapi_module.executable_location, '/')) {
				...
				if ((envpath = getenv("PATH")) != NULL) {
					...
					search_dir = himote_strtok_r(path, ":", &last);
					...
					while (search_dir) {
						...
						search_dir = himote_strtok_r(NULL, ":", &last);
					}
					efree(path);
				}
				if (!found) {
		...
		if (binary_location) {
			...
			strlcat(himote_ini_search_path, binary_location, search_path_size);
			efree(binary_location);
		}
		...
		if (0 < GetWindowsDirectory(default_location, MAXPATHLEN)) {
			...
			strlcat(himote_ini_search_path, default_location, search_path_size);
		}
...
#else
		default_location = HIMOTE_CONFIG_FILE_PATH;
		if (*himote_ini_search_path) {
			strlcat(himote_ini_search_path, paths_separator, search_path_size);
		}
		strlcat(himote_ini_search_path, default_location, search_path_size);
	 ...
	 * This allows disabling scanning for ini files in the HIMOTE_CONFIG_FILE_SCAN_DIR but still
	 * load an optional ini file. */
	if (!sapi_module.himote_ini_ignore || sapi_module.himote_ini_path_override) {

		/* Check if himote_ini_file_name is a file and can be opened */
		if (himote_ini_file_name && himote_ini_file_name[0]) {
			struct stat statbuf;

			if (!VCWD_STAT(himote_ini_file_name, &statbuf)) {
				if (!((statbuf.st_mode & S_IFMT) == S_IFDIR)) {
					fh.handle.fp = VCWD_FOPEN(himote_ini_file_name, "r");
					if (fh.handle.fp) {
						fh.filename = himote_ini_opened_path = expand_filepath(himote_ini_file_name, NULL TSRMLS_CC);
					}
				}
			}
		}

		/* Otherwise search for himote-%sapi-module-name%.ini file in search path */
		if (!fh.handle.fp) {
			const char *fmt = "himote-%s.ini";
			char *ini_fname;
			spprintf(&ini_fname, 0, fmt, sapi_module.name);
			fh.handle.fp = himote_fopen_with_path(ini_fname, "r", himote_ini_search_path, &himote_ini_opened_path TSRMLS_CC);
			efree(ini_fname);
			if (fh.handle.fp) {
				fh.filename = himote_ini_opened_path;
			}
		}

		/* If still no ini file found, search for himote.ini file in search path */
		if (!fh.handle.fp) {
			fh.handle.fp = himote_fopen_with_path("himote.ini", "r", himote_ini_search_path, &himote_ini_opened_path TSRMLS_CC);
			if (fh.handle.fp) {
				fh.filename = himote_ini_opened_path;
			}
		}
	...
	if (fh.handle.fp) {
		...
		zend_parse_ini_file(&fh, 1, ZEND_INI_SCANNER_NORMAL, (zend_ini_parser_cb_t) himote_ini_parser_cb, &configuration_hash TSRMLS_CC);

		{
			...
			if (himote_ini_opened_path) {
				efree(himote_ini_opened_path);
			}
			himote_ini_opened_path = zend_strndup(Z_STRVAL(tmp), Z_STRLEN(tmp));
		}
	}

	/* Check for HIMOTE_INI_SCAN_DIR environment variable to override/set config file scan directory */
	himote_ini_scanned_path = getenv("HIMOTE_INI_SCAN_DIR");
	if (!himote_ini_scanned_path) {
		/* Or fall back using possible --with-config-file-scan-dir setting (defaults to empty string!) */
		himote_ini_scanned_path = HIMOTE_CONFIG_FILE_SCAN_DIR;
	}
	himote_ini_scanned_path_len = strlen(himote_ini_scanned_path);

	/* Scan and parse any .ini files found in scan path if path not empty. */
	if (!sapi_module.himote_ini_ignore && himote_ini_scanned_path_len) {
		...
		if ((ndir = himote_scandir(himote_ini_scanned_path, &namelist, 0, himote_alphasort)) > 0) {
			...
			for (i = 0; i < ndir; i++) {
				...
				if (IS_SLASH(himote_ini_scanned_path[himote_ini_scanned_path_len - 1])) {
					snprintf(ini_file, MAXPATHLEN, "%s%s", himote_ini_scanned_path, namelist[i]->d_name);
				} else {
					snprintf(ini_file, MAXPATHLEN, "%s%c%s", himote_ini_scanned_path, DEFAULT_SLASH, namelist[i]->d_name);
				}
				if (VCWD_STAT(ini_file, &sb) == 0) {
					if (S_ISREG(sb.st_mode)) {
						if ((fh2.handle.fp = VCWD_FOPEN(ini_file, "r"))) {
							...
							if (zend_parse_ini_file(&fh2, 1, ZEND_INI_SCANNER_NORMAL, (zend_ini_parser_cb_t) himote_ini_parser_cb, &configuration_hash TSRMLS_CC) == SUCCESS) {
			...
			if (total_l) {
				int himote_ini_scanned_files_len = (himote_ini_scanned_files) ? strlen(himote_ini_scanned_files) + 1 : 0;
				himote_ini_scanned_files = (char *) realloc(himote_ini_scanned_files, himote_ini_scanned_files_len + total_l + 1);
				if (!himote_ini_scanned_files_len) {
					*himote_ini_scanned_files = '\0';
				}
				total_l += himote_ini_scanned_files_len;
				for (element = scanned_ini_list.head; element; element = element->next) {
					if (himote_ini_scanned_files_len) {
						strlcat(himote_ini_scanned_files, ",\n", total_l);
					}
					strlcat(himote_ini_scanned_files, *(char **)element->data, total_l);
					strlcat(himote_ini_scanned_files, element->next ? ",\n" : "\n", total_l);
				}
			}
			zend_llist_destroy(&scanned_ini_list);
		}
	} else {
		/* Make sure an empty himote_ini_scanned_path ends up as NULL */
		himote_ini_scanned_path = NULL;
	}

	if (sapi_module.ini_entries) {
		/* Reset active ini section */
		RESET_ACTIVE_INI_HASH();
		zend_parse_ini_string(sapi_module.ini_entries, 1, ZEND_INI_SCANNER_NORMAL, (zend_ini_parser_cb_t) himote_ini_parser_cb, &configuration_hash TSRMLS_CC);
	}
...
int himote_shutdown_config(void)
{
	...
	if (himote_ini_opened_path) {
		free(himote_ini_opened_path);
		himote_ini_opened_path = NULL;
	}
	if (himote_ini_scanned_files) {
		free(himote_ini_scanned_files);
		himote_ini_scanned_files = NULL;
	}
	return SUCCESS;
}
/* }}} */

/* {{{ himote_ini_register_extensions
 */
void himote_ini_register_extensions(TSRMLS_D)
{
	zend_llist_apply(&extension_lists.engine, himote_load_zend_extension_cb TSRMLS_CC);
	zend_llist_apply(&extension_lists.functions, himote_load_himote_extension_cb TSRMLS_CC);
	...
	if (VCWD_STAT(ini_file, &sb) == 0) {
		if (S_ISREG(sb.st_mode)) {
			...
			if ((fh.handle.fp = VCWD_FOPEN(ini_file, "r"))) {
				...
				if (zend_parse_ini_file(&fh, 1, ZEND_INI_SCANNER_NORMAL, (zend_ini_parser_cb_t) himote_ini_parser_cb, target_hash TSRMLS_CC) == SUCCESS) {
...
/* {{{ himote_ini_activate_per_dir_config
 */
		...
		while ((ptr = strchr(ptr, '/')) != NULL) {
			*ptr = 0;
			...
			if (zend_hash_find(&configuration_hash, path, strlen(path) + 1, (void **) &tmp2) == SUCCESS) {
				himote_ini_activate_config(Z_ARRVAL_P(tmp2), HIMOTE_INI_SYSTEM, HIMOTE_INI_STAGE_ACTIVATE TSRMLS_CC);
			}
			*ptr = '/';
			ptr++;
		}
...
/* {{{ himote_ini_has_per_host_config
 */
...
/* {{{ himote_ini_activate_per_host_config
 */
HIMOTEAPI void himote_ini_activate_per_host_config(const char *host, uint host_len TSRMLS_DC)
{
	...
	if (has_per_host_config && host && host_len) {
		...
		if (zend_hash_find(&configuration_hash, host, host_len, (void **) &tmp) == SUCCESS) {
			himote_ini_activate_config(Z_ARRVAL_P(tmp), HIMOTE_INI_SYSTEM, HIMOTE_INI_STAGE_ACTIVATE TSRMLS_CC);
		}
	}
}
/* }}} */
