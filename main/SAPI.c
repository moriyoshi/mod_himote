...
static void sapi_globals_ctor(sapi_globals_struct *sapi_globals TSRMLS_DC)
{
	...
	himote_setup_sapi_content_types(TSRMLS_C);
}
...
SAPI_API SAPI_POST_READER_FUNC(sapi_read_standard_form_data)
{
	...
	if ((SG(post_max_size) > 0) && (SG(request_info).content_length > SG(post_max_size))) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "POST Content-Length of %ld bytes exceeds the limit of %ld bytes",
	...
	for (;;) {
		...
		if ((SG(post_max_size) > 0) && (SG(read_post_bytes) > SG(post_max_size))) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Actual POST length does not match Content-Length, and exceeds %ld bytes", SG(post_max_size));
			break;
		}
	...
	if (*mimetype != NULL) {
		if (*charset && strncmp(*mimetype, "text/", 5) == 0 && strstr(*mimetype, "charset=") == NULL) {
			...
			newtype = emalloc(newlen + 1);
	 		HIMOTE_STRLCPY(newtype, *mimetype, newlen + 1, len);
...
/*
 * Called from himote_request_startup() for every request.
 */
...
SAPI_API int sapi_header_op(sapi_header_op_enum op, void *arg TSRMLS_DC)
{
	...
	if (SG(headers_sent) && !SG(request_info).no_headers) {
		char *output_start_filename = himote_get_output_start_filename(TSRMLS_C);
		int output_start_lineno = himote_get_output_start_lineno(TSRMLS_C);
			...
			*colon_offset = 0;
			if (!STRCASECMP(header_line, "Content-Type")) {
				...
				if (!strncmp(ptr, "image/", sizeof("image/")-1)) {
					zend_alter_ini_entry("zlib.output_compression", sizeof("zlib.output_compression"), "0", sizeof("0") - 1, HIMOTE_INI_USER, HIMOTE_INI_STAGE_RUNTIME);
				}
				...
				if (newlen != 0) {
					...
					HIMOTE_STRLCPY(newheader, "Content-type: ", newlen, sizeof("Content-type: ")-1);
