/*
   ...
   | HIMOTE 4.0 patches by Thies C. Arntzen (thies@thieso.net)               |
   | HIMOTE streams by Wez Furlong (wez@thebrainroom.com)                    |
   +----------------------------------------------------------------------+
*/

/* $Id$ */

/* Synced with himote 3.0 revision 1.218 1999-06-16 [ssb] */
...
#include "himote_scanf.h"
#include "zend_API.h"

#ifdef ZTS
int file_globals_id;
#else
himote_file_globals file_globals;
#endif

#if defined(HAVE_FNMATCH) && !defined(HIMOTE_WIN32)
# ifndef _GNU_SOURCE
#  define _GNU_SOURCE
# endif
# include <fnmatch.h>
...
#define HIMOTE_STREAM_TO_ZVAL(stream, arg) \
	himote_stream_from_zval_no_verify(stream, arg); \
	if (stream == NULL) {	\
		RETURN_FALSE;	\
	}
...
{
	himote_stream_context *context = (himote_stream_context*)rsrc->ptr;
	if (context->options) {
		zval_ptr_dtor(&context->options);
		context->options = NULL;
	}
	himote_stream_context_free(context);
}

static void file_globals_ctor(himote_file_globals *file_globals_p TSRMLS_DC)
{
	FG(pclose_ret) = 0;
	FG(user_stream_current_filename) = NULL;
	FG(def_chunk_size) = HIMOTE_SOCK_CHUNK_SIZE;
}

static void file_globals_dtor(himote_file_globals *file_globals_p TSRMLS_DC)
{
}

HIMOTE_INI_BEGIN()
	STD_HIMOTE_INI_ENTRY("user_agent", NULL, HIMOTE_INI_ALL, OnUpdateString, user_agent, himote_file_globals, file_globals)
	HIMOTE_INI_ENTRY("from", NULL, HIMOTE_INI_ALL, NULL)
	STD_HIMOTE_INI_ENTRY("default_socket_timeout", "60", HIMOTE_INI_ALL, OnUpdateLong, default_socket_timeout, himote_file_globals, file_globals)
	STD_HIMOTE_INI_ENTRY("auto_detect_line_endings", "0", HIMOTE_INI_ALL, OnUpdateLong, auto_detect_line_endings, himote_file_globals, file_globals)
HIMOTE_INI_END()
...
HIMOTE_MINIT_FUNCTION(file)
{
...
#ifdef ZTS
	ts_allocate_id(&file_globals_id, sizeof(himote_file_globals), (ts_allocate_ctor) file_globals_ctor, (ts_allocate_dtor) file_globals_dtor);
#else
	...
	REGISTER_LONG_CONSTANT("LOCK_SH", HIMOTE_LOCK_SH, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("LOCK_EX", HIMOTE_LOCK_EX, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("LOCK_UN", HIMOTE_LOCK_UN, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("LOCK_NB", HIMOTE_LOCK_NB, CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_CONNECT",			HIMOTE_STREAM_NOTIFY_CONNECT,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_AUTH_REQUIRED",	HIMOTE_STREAM_NOTIFY_AUTH_REQUIRED,	CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_AUTH_RESULT",		HIMOTE_STREAM_NOTIFY_AUTH_RESULT,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_MIME_TYPE_IS",	HIMOTE_STREAM_NOTIFY_MIME_TYPE_IS,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_FILE_SIZE_IS",	HIMOTE_STREAM_NOTIFY_FILE_SIZE_IS,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_REDIRECTED",		HIMOTE_STREAM_NOTIFY_REDIRECTED,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_PROGRESS",		HIMOTE_STREAM_NOTIFY_PROGRESS,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_FAILURE",			HIMOTE_STREAM_NOTIFY_FAILURE,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_COMPLETED",		HIMOTE_STREAM_NOTIFY_COMPLETED,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_RESOLVE",			HIMOTE_STREAM_NOTIFY_RESOLVE,			CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_SEVERITY_INFO",	HIMOTE_STREAM_NOTIFY_SEVERITY_INFO,	CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_SEVERITY_WARN",	HIMOTE_STREAM_NOTIFY_SEVERITY_WARN,	CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_NOTIFY_SEVERITY_ERR",	HIMOTE_STREAM_NOTIFY_SEVERITY_ERR,		CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("STREAM_FILTER_READ",			HIMOTE_STREAM_FILTER_READ,				CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_FILTER_WRITE",			HIMOTE_STREAM_FILTER_WRITE,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_FILTER_ALL",				HIMOTE_STREAM_FILTER_ALL,				CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("STREAM_CLIENT_PERSISTENT",		HIMOTE_STREAM_CLIENT_PERSISTENT,		CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_CLIENT_ASYNC_CONNECT",	HIMOTE_STREAM_CLIENT_ASYNC_CONNECT,	CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("STREAM_CLIENT_CONNECT",			HIMOTE_STREAM_CLIENT_CONNECT,	CONST_CS | CONST_PERSISTENT);
	...
	REGISTER_LONG_CONSTANT("FILE_USE_INCLUDE_PATH",			HIMOTE_FILE_USE_INCLUDE_PATH,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("FILE_IGNORE_NEW_LINES",			HIMOTE_FILE_IGNORE_NEW_LINES,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("FILE_SKIP_EMPTY_LINES",			HIMOTE_FILE_SKIP_EMPTY_LINES,			CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("FILE_APPEND",					HIMOTE_FILE_APPEND,					CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("FILE_NO_DEFAULT_CONTEXT",		HIMOTE_FILE_NO_DEFAULT_CONTEXT,		CONST_CS | CONST_PERSISTENT);
...
HIMOTE_FUNCTION(flock)
{
	...
	HIMOTE_STREAM_TO_ZVAL(stream, &arg1);

	act = operation & 3;
	if (act < 1 || act > 3) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Illegal operation argument");
		RETURN_FALSE;
	}
	...
	act = flock_values[act - 1] | (operation & HIMOTE_LOCK_NB ? LOCK_NB : 0);
	if (himote_stream_lock(stream, act)) {
...
HIMOTE_FUNCTION(get_meta_tags)
{
	...
	himote_meta_tags_token tok, tok_last;
	himote_meta_tags_data md;
	...
	md.stream = himote_stream_open_wrapper(filename, "rb",
	...
	while (!done && (tok = himote_next_meta_token(&md TSRMLS_CC)) != TOK_EOF) {
		if (tok == TOK_ID) {
			if (tok_last == TOK_OPENTAG) {
				...
				if (saw_name) {
					...
					while (temp && *temp) {
						if (strchr(HIMOTE_META_UNSAFE, *temp)) {
							*temp = '_';
						}
						temp++;
					}
					...
					if (PG(magic_quotes_runtime)) {
						value = himote_addslashes(md.token_data, 0, &md.token_len, 0 TSRMLS_CC);
					} else {
			...
			if (have_name) {
				/* For BC */
				himote_strtolower(name, strlen(name));
				if (have_content) {
...
HIMOTE_FUNCTION(file_get_contents)
{
	...
	int len;
	long offset = -1;
	long maxlen = HIMOTE_STREAM_COPY_ALL;
	zval *zcontext = NULL;
	...
	if (ZEND_NUM_ARGS() == 5 && maxlen < 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "length must be greater than or equal to zero");
		RETURN_FALSE;
	}

	context = himote_stream_context_from_zval(zcontext, 0);

	stream = himote_stream_open_wrapper_ex(filename, "rb",
				(use_include_path ? USE_PATH : 0) | ENFORCE_SAFE_MODE | REPORT_ERRORS,
				NULL, context);
	if (!stream) {
		RETURN_FALSE;
	...
	if (offset > 0 && himote_stream_seek(stream, offset, SEEK_SET) < 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to seek to position %ld in the stream", offset);
		himote_stream_close(stream);
		RETURN_FALSE;
	}

	if ((len = himote_stream_copy_to_mem(stream, &contents, maxlen, 0)) > 0) {

		if (PG(magic_quotes_runtime)) {
			contents = himote_addslashes(contents, len, &len, 1 TSRMLS_CC); /* 1 = free source string */
		}
...
HIMOTE_FUNCTION(file_put_contents)
{
	...
	if (Z_TYPE_P(data) == IS_RESOURCE) {
		himote_stream_from_zval(srcstream, &data);
	}

	context = himote_stream_context_from_zval(zcontext, flags & HIMOTE_FILE_NO_DEFAULT_CONTEXT);

	if (flags & HIMOTE_FILE_APPEND) {
		mode[0] = 'a';
	} else if (flags & LOCK_EX) {
		/* check to make sure we are dealing with a regular file */
		if (himote_memnstr(filename, "://", sizeof("://") - 1, filename + filename_len)) {
			if (strncasecmp(filename, "file://", sizeof("file://") - 1)) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Exclusive locks may only be set for regular files");
				RETURN_FALSE;
			}
		}
		mode[0] = 'c';
	...
	stream = himote_stream_open_wrapper_ex(filename, mode, ((flags & HIMOTE_FILE_USE_INCLUDE_PATH) ? USE_PATH : 0) | ENFORCE_SAFE_MODE | REPORT_ERRORS, NULL, context);
	if (stream == NULL) {
		RETURN_FALSE;
	}

	if (flags & LOCK_EX && (!himote_stream_supports_lock(stream) || himote_stream_lock(stream, LOCK_EX))) {
		himote_stream_close(stream);
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Exclusive locks are not supported for this stream");
		RETURN_FALSE;
	}

	if (mode[0] == 'c') {
		himote_stream_truncate_set_size(stream, 0);
	}

	switch (Z_TYPE_P(data)) {
		case IS_RESOURCE: {
			size_t len;
			if (himote_stream_copy_to_stream_ex(srcstream, stream, HIMOTE_STREAM_COPY_ALL, &len) != SUCCESS) {
				numbytes = -1;
			} else {
				numbytes = len;
			}
		...
		case IS_STRING:
			if (Z_STRLEN_P(data)) {
				numbytes = himote_stream_write(stream, Z_STRVAL_P(data), Z_STRLEN_P(data));
				if (numbytes != Z_STRLEN_P(data)) {
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Only %d of %d bytes written, possibly out of free disk space", numbytes, Z_STRLEN_P(data));
					numbytes = -1;
				}
			}
			break;
			...
			if (zend_hash_num_elements(Z_ARRVAL_P(data))) {
				...
				while (zend_hash_get_current_data_ex(Z_ARRVAL_P(data), (void **) &tmp, &pos) == SUCCESS) {
					...
					if (Z_STRLEN_PP(tmp)) {
						...
						bytes_written = himote_stream_write(stream, Z_STRVAL_PP(tmp), Z_STRLEN_PP(tmp));
						if (bytes_written < 0 || bytes_written != Z_STRLEN_PP(tmp)) {
							if (bytes_written < 0) {
								himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to write %d bytes to %s", Z_STRLEN_PP(tmp), filename);
							} else {
								himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Only %d of %d bytes written, possibly out of free disk space", bytes_written, Z_STRLEN_PP(tmp));
							}
							numbytes = -1;
							break;
						}
		...
		case IS_OBJECT:
			if (Z_OBJ_HT_P(data) != NULL) {
				...
				if (zend_std_cast_object_tostring(data, &out, IS_STRING TSRMLS_CC) == SUCCESS) {
					numbytes = himote_stream_write(stream, Z_STRVAL(out), Z_STRLEN(out));
					if (numbytes != Z_STRLEN(out)) {
						himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Only %d of %d bytes written, possibly out of free disk space", numbytes, Z_STRLEN(out));
						numbytes = -1;
					}
					zval_dtor(&out);
					break;
...
#define HIMOTE_FILE_BUF_SIZE	80
...
HIMOTE_FUNCTION(file)
{
	...
	if (flags < 0 || flags > (HIMOTE_FILE_USE_INCLUDE_PATH | HIMOTE_FILE_IGNORE_NEW_LINES | HIMOTE_FILE_SKIP_EMPTY_LINES | HIMOTE_FILE_NO_DEFAULT_CONTEXT)) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "'%ld' flag is not supported", flags);
		RETURN_FALSE;
	}

	use_include_path = flags & HIMOTE_FILE_USE_INCLUDE_PATH;
	include_new_line = !(flags & HIMOTE_FILE_IGNORE_NEW_LINES);
	skip_blank_lines = flags & HIMOTE_FILE_SKIP_EMPTY_LINES;

	context = himote_stream_context_from_zval(zcontext, flags & HIMOTE_FILE_NO_DEFAULT_CONTEXT);

	stream = himote_stream_open_wrapper_ex(filename, "rb", (use_include_path ? USE_PATH : 0) | ENFORCE_SAFE_MODE | REPORT_ERRORS, NULL, context);
	if (!stream) {
		RETURN_FALSE;
	}
	...
	if ((target_len = himote_stream_copy_to_mem(stream, &target_buf, HIMOTE_STREAM_COPY_ALL, 0))) {
		s = target_buf;
		e = target_buf + target_len;

		if (!(p = himote_stream_locate_eol(stream, target_buf, target_len TSRMLS_CC))) {
			p = e;
			goto parse_eol;
		}

		if (stream->flags & HIMOTE_STREAM_FLAG_EOL_MAC) {
			eol_marker = '\r';
		}
...
parse_eol:
				if (PG(magic_quotes_runtime)) {
					...
					slashed = himote_addslashes(s, (p-s), &len, 0 TSRMLS_CC);
			...
			do {
				...
				if (PG(magic_quotes_runtime)) {
					/* s is in target_buf which is freed at the end of the function */
					slashed = himote_addslashes(s, (p-s-windows_eol), &len, 0 TSRMLS_CC);
					add_index_stringl(return_value, i++, slashed, len, 0);
				} else {
...
HIMOTE_FUNCTION(tempnam)
{
	...
	if (himote_check_open_basedir(dir TSRMLS_CC)) {
		RETURN_FALSE;
	}

	himote_basename(prefix, prefix_len, NULL, 0, &p, &p_len TSRMLS_CC);
	if (p_len > 64) {
		p[63] = '\0';
	}
	...
	if ((fd = himote_open_temporary_fd_ex(dir, p, &opened_path, 1 TSRMLS_CC)) >= 0) {
		close(fd);
...
HIMOTE_NAMED_FUNCTION(himote_if_tmpfile)
{
	...
	stream = himote_stream_fopen_tmpfile();
...
HIMOTE_NAMED_FUNCTION(himote_if_fopen)
{
	...
	stream = himote_stream_open_wrapper_ex(filename, mode, (use_include_path ? USE_PATH : 0) | ENFORCE_SAFE_MODE | REPORT_ERRORS, NULL, context);
...
HIMOTEAPI HIMOTE_FUNCTION(fclose)
{
	...
	if ((stream->flags & HIMOTE_STREAM_FLAG_NO_FCLOSE) != 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%d is not a valid stream resource", stream->rsrc_id);
		RETURN_FALSE;
	}
...
HIMOTE_FUNCTION(popen)
{
...
#endif
	{
		...
		if (!fp) {
			himote_error_docref2(NULL TSRMLS_CC, command, posix_mode, E_WARNING, "%s", strerror(errno));
			efree(posix_mode);
			RETURN_FALSE;
		}
	}
	stream = himote_stream_fopen_from_pipe(fp, mode);

	if (stream == NULL)	{
		himote_error_docref2(NULL TSRMLS_CC, command, mode, E_WARNING, "%s", strerror(errno));
		RETVAL_FALSE;
	} else {
		himote_stream_to_zval(stream, return_value);
	}
...
HIMOTEAPI HIMOTE_FUNCTION(fgets)
{
	...
	if (argc == 1) {
		...
		buf = himote_stream_get_line(stream, NULL, 0, &line_len);
		if (buf == NULL) {
			goto exit_failed;
		}
	} else if (argc > 1) {
		if (len <= 0) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Length parameter must be greater than 0");
			RETURN_FALSE;
		}

		buf = ecalloc(len + 1, sizeof(char));
		if (himote_stream_get_line(stream, buf, len, &line_len) == NULL) {
			goto exit_failed;
		}
	}

	if (PG(magic_quotes_runtime)) {
		Z_STRVAL_P(return_value) = himote_addslashes(buf, line_len, &Z_STRLEN_P(return_value), 1 TSRMLS_CC);
...
HIMOTEAPI HIMOTE_FUNCTION(fgetc)
{
	...
	result = himote_stream_getc(stream);
...
HIMOTEAPI HIMOTE_FUNCTION(fgetss)
{
	...
	HIMOTE_STREAM_TO_ZVAL(stream, &fd);
	...
	if ((retval = himote_stream_get_line(stream, buf, len, &actual_len)) == NULL)	{
		if (buf != NULL) {
			efree(buf);
		}
		RETURN_FALSE;
	...
	retval_len = himote_strip_tags(retval, actual_len, &stream->fgetss_state, allowed_tags, allowed_tags_len);
...
HIMOTE_FUNCTION(fscanf)
{
	...
	what = zend_fetch_resource(&file_handle TSRMLS_CC, -1, "File-Handle", &type, 2, himote_file_le_stream(), himote_file_le_pstream());
	...
	buf = himote_stream_get_line((himote_stream *) what, NULL, 0, &len);
	if (buf == NULL) {
		if (args) {
			efree(args);
		}
	...
	result = himote_sscanf_internal(buf, format, argc, args, 0, &return_value TSRMLS_CC);
...
HIMOTEAPI HIMOTE_FUNCTION(fwrite)
{
	...
	if (PG(magic_quotes_runtime)) {
		...
		himote_stripslashes(buffer, &num_bytes TSRMLS_CC);
	}

	ret = himote_stream_write(stream, buffer ? buffer : arg2, num_bytes);
	if (buffer) {
		efree(buffer);
	}
...
HIMOTEAPI HIMOTE_FUNCTION(fflush)
{
	...
	ret = himote_stream_flush(stream);
	if (ret) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
...
HIMOTEAPI HIMOTE_FUNCTION(rewind)
{
	...
	if (-1 == himote_stream_rewind(stream)) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
HIMOTEAPI HIMOTE_FUNCTION(fseek)
{
	...
	RETURN_LONG(himote_stream_seek(stream, arg2, whence));
}
/* }}} */
...
/* DEPRECATED APIs: Use himote_stream_mkdir() instead */
...
{
	return himote_mkdir_ex(dir, mode, REPORT_ERRORS TSRMLS_CC);
}
/* }}} */
...
HIMOTE_FUNCTION(mkdir)
{
	...
	RETURN_BOOL(himote_stream_mkdir(dir, mode, (recursive ? HIMOTE_STREAM_MKDIR_RECURSIVE : 0) | REPORT_ERRORS, context));
}
/* }}} */
...
HIMOTE_FUNCTION(rmdir)
{
	...
	RETURN_BOOL(himote_stream_rmdir(dir, REPORT_ERRORS, context));
}
/* }}} */
...
HIMOTE_FUNCTION(readfile)
{
	...
	stream = himote_stream_open_wrapper_ex(filename, "rb", (use_include_path ? USE_PATH : 0) | ENFORCE_SAFE_MODE | REPORT_ERRORS, NULL, context);
	if (stream) {
		size = himote_stream_passthru(stream);
		himote_stream_close(stream);
		RETURN_LONG(size);
	}
...
HIMOTE_FUNCTION(rename)
{
	...
	wrapper = himote_stream_locate_url_wrapper(old_name, NULL, 0 TSRMLS_CC);

	if (!wrapper || !wrapper->wops) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to locate stream wrapper");
		RETURN_FALSE;
	}

	if (!wrapper->wops->rename) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%s wrapper does not support renaming", wrapper->wops->label ? wrapper->wops->label : "Source");
		RETURN_FALSE;
	}

	if (wrapper != himote_stream_locate_url_wrapper(new_name, NULL, 0 TSRMLS_CC)) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot rename a file across wrapper types");
		RETURN_FALSE;
	}
...
HIMOTE_FUNCTION(unlink)
{
	...
	wrapper = himote_stream_locate_url_wrapper(filename, NULL, 0 TSRMLS_CC);
	...
	if (!wrapper->wops->unlink) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%s does not allow unlinking", wrapper->wops->label ? wrapper->wops->label : "Wrapper");
		RETURN_FALSE;
	}
...
HIMOTE_NAMED_FUNCTION(himote_if_ftruncate)
{
	...
	HIMOTE_STREAM_TO_ZVAL(stream, &fp);

	if (!himote_stream_truncate_supported(stream)) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Can't truncate this stream!");
		RETURN_FALSE;
	}

	RETURN_BOOL(0 == himote_stream_truncate_set_size(stream, size));
}
/* }}} */
...
HIMOTE_NAMED_FUNCTION(himote_if_fstat)
{
	...
	himote_stream *stream;
	himote_stream_statbuf stat_ssb;
	...
	if (himote_stream_stat(stream, &stat_ssb)) {
		RETURN_FALSE;
	}
...
HIMOTE_FUNCTION(copy)
{
	...
	if (himote_check_open_basedir(source TSRMLS_CC)) {
		RETURN_FALSE;
	}

	context = himote_stream_context_from_zval(zcontext, 0);

	if (himote_copy_file_ctx(source, target, 0, context TSRMLS_CC) == SUCCESS) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
{
	return himote_copy_file_ctx(src, dest, ENFORCE_SAFE_MODE, NULL TSRMLS_CC);
}
/* }}} */
...
/* {{{ himote_copy_file_ctx
 */
HIMOTEAPI int himote_copy_file_ctx(char *src, char *dest, int src_chk, himote_stream_context *context TSRMLS_DC)
{
	himote_stream *srcstream = NULL, *deststream = NULL;
	int ret = FAILURE;
	himote_stream_statbuf src_s, dest_s;

	switch (himote_stream_stat_path_ex(src, 0, &src_s, context)) {
		case -1:
			/* non-statable stream */
			goto safe_to_copy;
			break;
	...
	if (S_ISDIR(src_s.sb.st_mode)) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "The first argument to copy() function cannot be a directory");
		return FAILURE;
	}

	switch (himote_stream_stat_path_ex(dest, HIMOTE_STREAM_URL_STAT_QUIET, &dest_s, context)) {
		case -1:
			/* non-statable stream */
			goto safe_to_copy;
			break;
	...
	if (S_ISDIR(dest_s.sb.st_mode)) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "The second argument to copy() function cannot be a directory");
		return FAILURE;
	}
...
safe_to_copy:
	...
	srcstream = himote_stream_open_wrapper_ex(src, "rb", src_chk | REPORT_ERRORS, NULL, context);

	if (!srcstream) {
		return ret;
	}

	deststream = himote_stream_open_wrapper_ex(dest, "wb", ENFORCE_SAFE_MODE | REPORT_ERRORS, NULL, context);

	if (srcstream && deststream) {
		ret = himote_stream_copy_to_stream_ex(srcstream, deststream, HIMOTE_STREAM_COPY_ALL, NULL);
	}
	if (srcstream) {
		himote_stream_close(srcstream);
	}
	if (deststream) {
		himote_stream_close(deststream);
	}
	return ret;
}
/* }}} */
...
HIMOTEAPI HIMOTE_FUNCTION(fread)
{
	...
	Z_STRLEN_P(return_value) = himote_stream_read(stream, Z_STRVAL_P(return_value), len);

	/* needed because recv/read/gzread doesnt put a null at the end*/
	Z_STRVAL_P(return_value)[Z_STRLEN_P(return_value)] = 0;

	if (PG(magic_quotes_runtime)) {
		Z_STRVAL_P(return_value) = himote_addslashes(Z_STRVAL_P(return_value),
...
static const char *himote_fgetcsv_lookup_trailing_spaces(const char *ptr, size_t len, const char delimiter TSRMLS_DC) /* {{{ */
{
	int inc_len;
	unsigned char last_chars[2] = { 0, 0 };

	while (len > 0) {
		inc_len = (*ptr == '\0' ? 1: himote_mblen(ptr, len));
		switch (inc_len) {
			case -2:
			case -1:
				inc_len = 1;
...
HIMOTE_FUNCTION(fputcsv)
{
	...
	if (delimiter_str != NULL) {
		...
		if (delimiter_str_len < 1) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "delimiter must be a character");
			RETURN_FALSE;
		} else if (delimiter_str_len > 1) {
			himote_error_docref(NULL TSRMLS_CC, E_NOTICE, "delimiter must be a single character");
		}
	...
	if (enclosure_str != NULL) {
		if (enclosure_str_len < 1) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "enclosure must be a character");
			RETURN_FALSE;
		} else if (enclosure_str_len > 1) {
			himote_error_docref(NULL TSRMLS_CC, E_NOTICE, "enclosure must be a single character");
		}
		/* use first character from string */
	...
	if (!PG(magic_quotes_runtime)) {
		ret = himote_stream_write(stream, csvline.c, csvline.len);
	} else {
		char *buffer = estrndup(csvline.c, csvline.len);
		int len = csvline.len;
		himote_stripslashes(buffer, &len TSRMLS_CC);
		ret = himote_stream_write(stream, buffer, len);
		efree(buffer);
	}
...
HIMOTE_FUNCTION(fgetcsv)
{
	...
	/* first section exactly as himote_fgetss */
...

	{
		...
		if (escape_str != NULL) {
			if (escape_str_len < 1) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "escape must be character");
				RETURN_FALSE;
			} else if (escape_str_len > 1) {
				himote_error_docref(NULL TSRMLS_CC, E_NOTICE, "escape must be a single character");
			}
		...
		if (len_zv != NULL && Z_TYPE_PP(len_zv) != IS_NULL) {
			...
			if (len < 0) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Length parameter may not be negative");
				RETURN_FALSE;
			} else if (len == 0) {
				len = -1;
			}
	...
	if (len < 0) {
		if ((buf = himote_stream_get_line(stream, NULL, 0, &buf_len)) == NULL) {
			RETURN_FALSE;
		}
	} else {
		buf = emalloc(len + 1);
		if (himote_stream_get_line(stream, buf, len + 1, &buf_len) == NULL) {
			efree(buf);
			RETURN_FALSE;
		}
	}

	himote_fgetcsv(stream, delimiter, enclosure, escape, buf_len, buf, return_value TSRMLS_CC);
}
/* }}} */
...
HIMOTEAPI void himote_fgetcsv(himote_stream *stream, char delimiter, char enclosure, char escape_char, size_t buf_len, char *buf, zval *return_value TSRMLS_DC) /* {{{ */
{
	...
	bptr = buf;
	tptr = (char *)himote_fgetcsv_lookup_trailing_spaces(buf, buf_len, delimiter TSRMLS_CC);
	...
	do {
		...
		tptr = temp;
		inc_len = (bptr < limit ? (*bptr == '\0' ? 1: himote_mblen(bptr, limit - bptr)): 0);
		if (inc_len == 1) {
			char *tmp = bptr;
					...
					case 0:
						switch (state) {
							...
							case 0: {
								...
								if (stream == NULL) {
									goto quit_loop_2;
								} else if ((new_buf = himote_stream_get_line(stream, NULL, 0, &new_len)) == NULL) {
								...
								line_end = limit = (char *)himote_fgetcsv_lookup_trailing_spaces(buf, buf_len, delimiter TSRMLS_CC);
		...
		quit_loop_4:
			...
			comp_end = (char *)himote_fgetcsv_lookup_trailing_spaces(temp, tptr - temp, delimiter TSRMLS_CC);
			if (*bptr == delimiter) {
				bptr++;
			}
		}

		/* 3. Now pass our field back to himote */
		*comp_end = '\0';
...
HIMOTE_FUNCTION(realpath)
{
	...
	if (VCWD_REALPATH(filename, resolved_path_buff)) {
		if (himote_check_open_basedir(resolved_path_buff TSRMLS_CC)) {
			RETURN_FALSE;
		}
...
#define HIMOTE_META_HTML401_CHARS "-_.:"

/* {{{ himote_next_meta_token
   Tokenizes an HTML file for get_meta_tags */
himote_meta_tags_token himote_next_meta_token(himote_meta_tags_data *md TSRMLS_DC)
{
	int ch = 0, compliment;
	...
	while (md->ulc || (!himote_stream_eof(md->stream) && (ch = himote_stream_getc(md->stream)))) {
		if (himote_stream_eof(md->stream)) {
			break;
		}
		...
		switch (ch) {
			...
			case '"':
				compliment = ch;
				md->token_len = 0;
				while (!himote_stream_eof(md->stream) && (ch = himote_stream_getc(md->stream)) && ch != compliment && ch != '<' && ch != '>') {
			...
			default:
				if (isalnum(ch)) {
					md->token_len = 0;
					buff[(md->token_len)++] = ch;
					while (!himote_stream_eof(md->stream) && (ch = himote_stream_getc(md->stream)) && (isalnum(ch) || strchr(HIMOTE_META_HTML401_CHARS, ch))) {
						buff[(md->token_len)++] = ch;
...
HIMOTE_FUNCTION(fnmatch)
{
	...
	if (filename_len >= MAXPATHLEN) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Filename exceeds the maximum allowed length of %d characters", MAXPATHLEN);
		RETURN_FALSE;
	}
	if (pattern_len >= MAXPATHLEN) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Pattern exceeds the maximum allowed length of %d characters", MAXPATHLEN);
		RETURN_FALSE;
	}
...
HIMOTE_FUNCTION(sys_get_temp_dir)
{
	...
	RETURN_STRING((char *)himote_get_temporary_directory(), 1);
}
/* }}} */
