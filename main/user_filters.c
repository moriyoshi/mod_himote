...
#define HIMOTE_STREAM_BRIGADE_RES_NAME	"userfilter.bucket brigade"
#define HIMOTE_STREAM_BUCKET_RES_NAME "userfilter.bucket"
#define HIMOTE_STREAM_FILTER_RES_NAME "userfilter.filter"

struct himote_user_filter_data {
	zend_class_entry *ce;
...
#define GET_FILTER_FROM_OBJ()	{ \
	...
	if (FAILURE == zend_hash_index_find(Z_OBJPROP_P(this_ptr), 0, (void**)&tmp)) { \
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "filter property vanished"); \
		RETURN_FALSE; \
	} \
	ZEND_FETCH_RESOURCE(filter, himote_stream_filter*, tmp, -1, "filter", le_userfilters); \
}

/* define the base filter class */

HIMOTE_FUNCTION(user_filter_nop)
{
}
ZEND_BEGIN_ARG_INFO(arginfo_himote_user_filter_filter, 0)
	ZEND_ARG_INFO(0, in)
	ZEND_ARG_INFO(0, out)
...
ZEND_BEGIN_ARG_INFO(arginfo_himote_user_filter_onCreate, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO(arginfo_himote_user_filter_onClose, 0)
ZEND_END_ARG_INFO()

static const zend_function_entry user_filter_class_funcs[] = {
	HIMOTE_NAMED_FE(filter,	HIMOTE_FN(user_filter_nop),		arginfo_himote_user_filter_filter)
	HIMOTE_NAMED_FE(onCreate,	HIMOTE_FN(user_filter_nop),		arginfo_himote_user_filter_onCreate)
	HIMOTE_NAMED_FE(onClose,	HIMOTE_FN(user_filter_nop),		arginfo_himote_user_filter_onClose)
	HIMOTE_FE_END
};

static zend_class_entry user_filter_class_entry;

static ZEND_RSRC_DTOR_FUNC(himote_bucket_dtor)
{
	himote_stream_bucket *bucket = (himote_stream_bucket *)rsrc->ptr;
	if (bucket) {
...
{
	zend_class_entry *himote_user_filter;
	/* init the filter class ancestor */
	INIT_CLASS_ENTRY(user_filter_class_entry, "himote_user_filter", user_filter_class_funcs);
	if ((himote_user_filter = zend_register_internal_class(&user_filter_class_entry TSRMLS_CC)) == NULL) {
		return FAILURE;
	}
	zend_declare_property_string(himote_user_filter, "filtername", sizeof("filtername")-1, "", ZEND_ACC_PUBLIC TSRMLS_CC);
	zend_declare_property_string(himote_user_filter, "params", sizeof("params")-1, "", ZEND_ACC_PUBLIC TSRMLS_CC);

	/* init the filter resource; it has no dtor, as streams will always clean it up
	 * at the correct time */
	le_userfilters = zend_register_list_destructors_ex(NULL, NULL, HIMOTE_STREAM_FILTER_RES_NAME, 0);
	...
	le_bucket_brigade = zend_register_list_destructors_ex(NULL, NULL, HIMOTE_STREAM_BRIGADE_RES_NAME, module_number);
	/* Brigades will dispose of their buckets */
	le_bucket = zend_register_list_destructors_ex(himote_bucket_dtor, NULL, HIMOTE_STREAM_BUCKET_RES_NAME, module_number);
...
static void userfilter_dtor(himote_stream_filter *thisfilter TSRMLS_DC)
{
...
himote_stream_filter_status_t userfilter_filter(
			...
			TSRMLS_DC)
{
	...
	if (FAILURE == zend_hash_find(Z_OBJPROP_P(obj), "stream", sizeof("stream"), (void**)&zstream)) {
		...
		himote_stream_to_zval(stream, zstream);
	...
	if (call_result == SUCCESS && retval != NULL) {
		...
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "failed to call filter function");
	}
	...
	if (buckets_in->head) {
		himote_stream_bucket *bucket = buckets_in->head;

		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Unprocessed filter buckets remaining on input brigade");
	...
	if (ret != PSFS_PASS_ON) {
		himote_stream_bucket *bucket = buckets_out->head;
		while (bucket != NULL) {
			himote_stream_bucket_unlink(bucket TSRMLS_CC);
			himote_stream_bucket_delref(bucket TSRMLS_CC);
...
static himote_stream_filter_ops userfilter_ops = {
	userfilter_filter,
	userfilter_dtor,
	"user-filter"
};

static himote_stream_filter *user_filter_factory_create(const char *filtername,
		zval *filterparams, int persistent TSRMLS_DC)
{
	struct himote_user_filter_data *fdat = NULL;
	...
	filter = himote_stream_filter_alloc(&userfilter_ops, NULL, 0);
	if (filter == NULL) {
		return NULL;
	}
	...
	if (retval) {
		if (Z_TYPE_P(retval) == IS_BOOL && Z_LVAL_P(retval) == 0) {
			...
			himote_stream_filter_free(filter TSRMLS_CC);
...
static himote_stream_filter_factory user_filter_factory = {
	user_filter_factory_create
};

static void filter_item_dtor(struct himote_user_filter_data *fdat)
{
}
...
HIMOTE_FUNCTION(stream_bucket_make_writeable)
{
	...
	ZEND_FETCH_RESOURCE(brigade, himote_stream_bucket_brigade *, &zbrigade, -1, HIMOTE_STREAM_BRIGADE_RES_NAME, le_bucket_brigade); 

	ZVAL_NULL(return_value);

	if (brigade->head && (bucket = himote_stream_bucket_make_writeable(brigade->head TSRMLS_CC))) {
...
/* {{{ himote_stream_bucket_attach */
static void himote_stream_bucket_attach(int append, INTERNAL_FUNCTION_PARAMETERS)
{
	...
	if (FAILURE == zend_hash_find(Z_OBJPROP_P(zobject), "bucket", 7, (void**)&pzbucket)) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Object has no bucket property");
		RETURN_FALSE;
	}

	ZEND_FETCH_RESOURCE(brigade, himote_stream_bucket_brigade *, &zbrigade, -1, HIMOTE_STREAM_BRIGADE_RES_NAME, le_bucket_brigade);
	ZEND_FETCH_RESOURCE(bucket, himote_stream_bucket *, pzbucket, -1, HIMOTE_STREAM_BUCKET_RES_NAME, le_bucket);

	if (SUCCESS == zend_hash_find(Z_OBJPROP_P(zobject), "data", 5, (void**)&pzdata) && (*pzdata)->type == IS_STRING) {
		if (!bucket->own_buf) {
			bucket = himote_stream_bucket_make_writeable(bucket TSRMLS_CC);
		}
	...
	if (append) {
		himote_stream_bucket_append(brigade, bucket TSRMLS_CC);
	} else {
		himote_stream_bucket_prepend(brigade, bucket TSRMLS_CC);
	}
	/* This is a hack necessary to accomodate situations where bucket is appended to the stream
 	 * multiple times. See bug35916.himotet for reference.
	 */
...
{
	himote_stream_bucket_attach(0, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
/* }}} */
...
{
	himote_stream_bucket_attach(1, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}
/* }}} */
...
HIMOTE_FUNCTION(stream_bucket_new)
{
	...
	if (!(pbuffer = pemalloc(buffer_len, himote_stream_is_persistent(stream)))) {
		RETURN_FALSE;
	}

	memcpy(pbuffer, buffer, buffer_len);

	bucket = himote_stream_bucket_new(stream, pbuffer, buffer_len, 1, himote_stream_is_persistent(stream) TSRMLS_CC);
...
HIMOTE_FUNCTION(stream_get_filters)
{
	...
	filters_hash = himote_get_stream_filters_hash();
...
HIMOTE_FUNCTION(stream_filter_register)
{
	...
	struct himote_user_filter_data *fdat;
	...
	if (!filtername_len) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Filter name cannot be empty");
		return;
	}

	if (!classname_len) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Class name cannot be empty");
		return;
	}
	...
	fdat = ecalloc(1, sizeof(struct himote_user_filter_data) + classname_len);
	...
	if (zend_hash_add(BG(user_filter_map), filtername, filtername_len + 1, (void*)fdat,
			...
			himote_stream_filter_register_factory_volatile(filtername, &user_filter_factory TSRMLS_CC) == SUCCESS) {
		RETVAL_TRUE;
	}
