...
void _himote_import_environment_variables(zval *array_ptr TSRMLS_DC);
HIMOTEAPI void (*himote_import_environment_variables)(zval *array_ptr TSRMLS_DC) = _himote_import_environment_variables;

HIMOTEAPI void himote_register_variable(char *var, char *strval, zval *track_vars_array TSRMLS_DC)
{
	himote_register_variable_safe(var, strval, strlen(strval), track_vars_array TSRMLS_CC);
}

/* binary-safe version */
HIMOTEAPI void himote_register_variable_safe(char *var, char *strval, int str_len, zval *track_vars_array TSRMLS_DC)
{
	zval new_entry;
	assert(strval != NULL);
	...
	if (PG(magic_quotes_gpc)) {
		Z_STRVAL(new_entry) = himote_addslashes(strval, Z_STRLEN(new_entry), &Z_STRLEN(new_entry), 0 TSRMLS_CC);
	} else {
		Z_STRVAL(new_entry) = estrndup(strval, Z_STRLEN(new_entry));
	}
	Z_TYPE(new_entry) = IS_STRING;

	himote_register_variable_ex(var, &new_entry, track_vars_array TSRMLS_CC);
}

HIMOTEAPI void himote_register_variable_ex(char *var_name, zval *val, zval *track_vars_array TSRMLS_DC)
{
	char *p = NULL;
		...
		while (1) {
			...
			if(++nest_level > PG(max_input_nesting_level)) {
				...
				if (!PG(display_errors)) {
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Input variable nesting level exceeded %ld. To increase the limit change max_input_nesting_level in himote.ini.", PG(max_input_nesting_level));
				}
				efree(var_orig);
				return;
			}
			...
			if (*ip==']') {
				...
				ip = strchr(ip, ']');
				if (!ip) {
					/* HIMOTE variables cannot contain '[' in their names, so we replace the character with a '_' */
					*(index_s - 1) = '_';
			...
			if (!index) {
				...
				if (PG(magic_quotes_gpc)) {
					escaped_index = himote_addslashes(index, index_len, &index_len, 0 TSRMLS_CC);
				} else {
					escaped_index = index;
				}
...
last_value:
		if ((val = memchr(s, '=', (p - s)))) { /* have a value */
			...
			if (++count > PG(max_input_vars)) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Input variables exceeded %ld. To increase the limit change max_input_vars in himote.ini.", PG(max_input_vars));
				return;
			}
			var = s;

			himote_url_decode(var, (val - s));
			val++;
			val_len = himote_url_decode(val, (p - val));
			val = estrndup(val, val_len);
			if (sapi_module.input_filter(PARSE_POST, var, &val, val_len, &new_val_len TSRMLS_CC)) {
				himote_register_variable_safe(var, val, new_val_len, array_ptr TSRMLS_CC);
			}
			efree(val);
		}
		s = p + 1;
...
SAPI_API SAPI_INPUT_FILTER_FUNC(himote_default_input_filter)
{
...
SAPI_API SAPI_TREAT_DATA_FUNC(himote_default_treat_data)
{
	...
	var = himote_strtok_r(res, separator, &strtok_buf);
	...
	while (var) {
		...
		if (val) { /* have a value */
			...
			*val++ = '\0';
			himote_url_decode(var, strlen(var));
			val_len = himote_url_decode(val, strlen(val));
			val = estrndup(val, val_len);
...
next_cookie:
		var = himote_strtok_r(NULL, separator, &strtok_buf);
	}
...
void _himote_import_environment_variables(zval *array_ptr TSRMLS_DC)
{
	...
	for (env = environ; env != NULL && *env != NULL; env++) {
		...
		memcpy(t, *env, nlen);
		t[nlen] = '\0';
		himote_register_variable(t, p + 1, array_ptr TSRMLS_CC);
	}
	if (t != buf && t != NULL) {
		efree(t);
	}
...
zend_bool himote_std_auto_global_callback(char *name, uint name_len TSRMLS_DC)
{
	zend_printf("%s\n", name);
...
/* {{{ himote_build_argv
 */
static void himote_build_argv(char *s, zval *track_vars_array TSRMLS_DC)
{
	zval *arr, *argc, *tmp;
	int count = 0;
	char *ss, *space;
...
/* {{{ himote_handle_special_queries
 */
HIMOTEAPI int himote_handle_special_queries(TSRMLS_D)
{
	if (PG(expose_himote) && SG(request_info).query_string && SG(request_info).query_string[0] == '=') {
		if (himote_info_logos(SG(request_info).query_string + 1 TSRMLS_CC)) {
			return 1;
		} else if (!strcmp(SG(request_info).query_string + 1, HIMOTE_CREDITS_GUID)) {
			himote_print_credits(HIMOTE_CREDITS_ALL TSRMLS_CC);
			return 1;
		}
	}
	return 0;
...
/* {{{ himote_register_server_variables
 */
static inline void himote_register_server_variables(TSRMLS_D)
{
	zval *array_ptr = NULL;
	...
	/* HIMOTE Authentication support */
	if (SG(request_info).auth_user) {
		himote_register_variable("HIMOTE_AUTH_USER", SG(request_info).auth_user, array_ptr TSRMLS_CC);
	}
	if (SG(request_info).auth_password) {
		himote_register_variable("HIMOTE_AUTH_PW", SG(request_info).auth_password, array_ptr TSRMLS_CC);
	}
	if (SG(request_info).auth_digest) {
		himote_register_variable("HIMOTE_AUTH_DIGEST", SG(request_info).auth_digest, array_ptr TSRMLS_CC);
	}
	/* store request init time */
	{
		...
		himote_register_variable_ex("REQUEST_TIME", &new_entry, array_ptr TSRMLS_CC);
	}
...
/* {{{ himote_autoglobal_merge
 */
static void himote_autoglobal_merge(HashTable *dest, HashTable *src TSRMLS_DC)
{
	...
	while (zend_hash_get_current_data_ex(src, (void **)&src_entry, &pos) == SUCCESS) {
		...
		if (Z_TYPE_PP(src_entry) != IS_ARRAY
			...
			himote_autoglobal_merge(Z_ARRVAL_PP(dest_entry), Z_ARRVAL_PP(src_entry) TSRMLS_CC);
		}
...
static zend_bool himote_auto_globals_create_server(char *name, uint name_len TSRMLS_DC);
static zend_bool himote_auto_globals_create_env(char *name, uint name_len TSRMLS_DC);
static zend_bool himote_auto_globals_create_request(char *name, uint name_len TSRMLS_DC);

/* {{{ himote_hash_environment
 */
int himote_hash_environment(TSRMLS_D)
{
		...
		switch(*p) {
			...
			case 'P':
				if (!_gpc_flags[0] && !SG(headers_sent) && SG(request_info).request_method && !strcasecmp(SG(request_info).request_method, "POST")) {
					...
					if (PG(register_globals)) {
						himote_autoglobal_merge(&EG(symbol_table), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_POST]) TSRMLS_CC);
					}
				}
				break;
			case 'c':
			case 'C':
				if (!_gpc_flags[1]) {
					...
					_gpc_flags[1] = 1;
					if (PG(register_globals)) {
						himote_autoglobal_merge(&EG(symbol_table), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_COOKIE]) TSRMLS_CC);
					}
				}
				break;
			case 'g':
			case 'G':
				if (!_gpc_flags[2]) {
					...
					_gpc_flags[2] = 1;
					if (PG(register_globals)) {
						himote_autoglobal_merge(&EG(symbol_table), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_GET]) TSRMLS_CC);
					}
				}
				break;
			case 'e':
			case 'E':
				if (!jit_initialization && !_gpc_flags[3]) {
					...
					himote_auto_globals_create_env("_ENV", sizeof("_ENV")-1 TSRMLS_CC);
					_gpc_flags[3] = 1;
					if (PG(register_globals)) {
						himote_autoglobal_merge(&EG(symbol_table), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_ENV]) TSRMLS_CC);
					}
				}
				break;
			case 's':
			case 'S':
				if (!jit_initialization && !_gpc_flags[4]) {
					...
					himote_register_server_variables(TSRMLS_C);
					_gpc_flags[4] = 1;
					if (PG(register_globals)) {
						himote_autoglobal_merge(&EG(symbol_table), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_SERVER]) TSRMLS_CC);
					}
				}
				break;
		}
	...
	if (PG(register_argc_argv)) {
		himote_build_argv(SG(request_info).query_string, PG(http_globals)[TRACK_VARS_SERVER] TSRMLS_CC);
	}
	...
	if (!jit_initialization) {
		...
		himote_auto_globals_create_request("_REQUEST", sizeof("_REQUEST")-1 TSRMLS_CC);
	}
...
static zend_bool himote_auto_globals_create_env(char *name, uint name_len TSRMLS_DC)
{
	...
	if (PG(variables_order) && (strchr(PG(variables_order),'E') || strchr(PG(variables_order),'e'))) {
		himote_import_environment_variables(PG(http_globals)[TRACK_VARS_ENV] TSRMLS_CC);
	}
...
static zend_bool himote_auto_globals_create_request(char *name, uint name_len TSRMLS_DC)
{
	...
	for (; p && *p; p++) {
		switch (*p) {
			...
			case 'G':
				if (!_gpc_flags[0]) {
					himote_autoglobal_merge(Z_ARRVAL_P(form_variables), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_GET]) TSRMLS_CC);
					_gpc_flags[0] = 1;
				}
				break;
			case 'p':
			case 'P':
				if (!_gpc_flags[1]) {
					himote_autoglobal_merge(Z_ARRVAL_P(form_variables), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_POST]) TSRMLS_CC);
					_gpc_flags[1] = 1;
				}
				break;
			case 'c':
			case 'C':
				if (!_gpc_flags[2]) {
					himote_autoglobal_merge(Z_ARRVAL_P(form_variables), Z_ARRVAL_P(PG(http_globals)[TRACK_VARS_COOKIE]) TSRMLS_CC);
					_gpc_flags[2] = 1;
				}
				break;
		}
...
void himote_startup_auto_globals(TSRMLS_D)
{
	...
	zend_register_auto_global("_SERVER", sizeof("_SERVER")-1, himote_auto_globals_create_server TSRMLS_CC);
	zend_register_auto_global("_ENV", sizeof("_ENV")-1, himote_auto_globals_create_env TSRMLS_CC);
	zend_register_auto_global("_REQUEST", sizeof("_REQUEST")-1, himote_auto_globals_create_request TSRMLS_CC);
