...
HIMOTEAPI int himote_stream_filter_register_factory_volatile(const char *filterpattern, himote_stream_filter_factory *factory TSRMLS_DC)
{
	if (!FG(stream_filters)) {
		himote_stream_filter_factory tmpfactory;

		ALLOC_HASHTABLE(FG(stream_filters));
		zend_hash_init(FG(stream_filters), zend_hash_num_elements(&stream_filters_hash), NULL, NULL, 1);
		zend_hash_copy(FG(stream_filters), &stream_filters_hash, NULL, &tmpfactory, sizeof(himote_stream_filter_factory));
	}
...
{
	int is_persistent = himote_stream_is_persistent(stream);
	himote_stream_bucket *bucket;

	bucket = (himote_stream_bucket*)pemalloc(sizeof(himote_stream_bucket), is_persistent);
...
HIMOTEAPI himote_stream_bucket *himote_stream_bucket_make_writeable(himote_stream_bucket *bucket TSRMLS_DC)
{
	...
	retval = (himote_stream_bucket*)pemalloc(sizeof(himote_stream_bucket), bucket->is_persistent);
...
{
	*left = (himote_stream_bucket*)pecalloc(1, sizeof(himote_stream_bucket), in->is_persistent);
	*right = (himote_stream_bucket*)pecalloc(1, sizeof(himote_stream_bucket), in->is_persistent);
...
HIMOTEAPI himote_stream_filter *himote_stream_filter_create(const char *filtername, zval *filterparams, int persistent TSRMLS_DC)
{
	...
	himote_stream_filter_factory *factory = NULL;
	...
	if (filter == NULL) {
		...
		if (factory == NULL)
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to locate filter \"%s\"", filtername);
		else
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to create or locate filter \"%s\"", filtername);
	}
...
HIMOTEAPI himote_stream_filter *_himote_stream_filter_alloc(himote_stream_filter_ops *fops, void *abstract, int persistent STREAMS_DC TSRMLS_DC)
{
	...
	filter = (himote_stream_filter*) pemalloc_rel_orig(sizeof(himote_stream_filter), persistent);
	memset(filter, 0, sizeof(himote_stream_filter));
...
HIMOTEAPI int himote_stream_filter_append_ex(himote_stream_filter_chain *chain, himote_stream_filter *filter TSRMLS_DC)
{
	...
	if (&(stream->readfilters) == chain && (stream->writepos - stream->readpos) > 0) {
		...
		himote_stream_bucket_brigade *brig_inp = &brig_in, *brig_outp = &brig_out;
		himote_stream_filter_status_t status;
		himote_stream_bucket *bucket;
		size_t consumed = 0;

		bucket = himote_stream_bucket_new(stream, (char*) stream->readbuf + stream->readpos, stream->writepos - stream->readpos, 0, 0 TSRMLS_CC);
		...
		switch (status) {
			case PSFS_ERR_FATAL:
				...
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Filter failed to process pre-buffered data");
				return FAILURE;
			case PSFS_FEED_ME:
			...
			case PSFS_PASS_ON:
				...
				/* note that changes here should be reflected in
				   main/streams/streams.c::himote_stream_fill_read_buffer */
				stream->writepos = 0;
				stream->readpos = 0;
				...
				while (brig_outp->head) {
					...
					/* Grow buffer to hold this bucket if need be.
					   TODO: See warning in main/stream/streams.c::himote_stream_fill_read_buffer */
...
{
	if (himote_stream_filter_append_ex(chain, filter TSRMLS_CC) != SUCCESS) {
...
{
	himote_stream_bucket_brigade brig_a = { NULL, NULL }, brig_b = { NULL, NULL }, *inp = &brig_a, *outp = &brig_b, *brig_temp;
	himote_stream_bucket *bucket;
	himote_stream_filter_chain *chain;
	himote_stream_filter *current;
