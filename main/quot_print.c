...
*/
static char himote_hex2int(int c) /* {{{ */
{
	if (isdigit(c)) {
		return c - '0';
	}
...
HIMOTEAPI unsigned char *himote_quot_print_decode(const unsigned char *str, size_t length, size_t *ret_length, int replace_us_by_ws) /* {{{ */
{
...
#define HIMOTE_QPRINT_MAXL 75
 
HIMOTEAPI unsigned char *himote_quot_print_encode(const unsigned char *str, size_t length, size_t *ret_length) /* {{{ */
{
	unsigned long lp = 0;
	unsigned char c, *ret, *d;
	char *hex = "0123456789ABCDEF";

	ret = safe_emalloc(1, 3 * length + 3 * (((3 * length)/HIMOTE_QPRINT_MAXL) + 1), 0);
	d = ret;
...
HIMOTE_FUNCTION(quoted_printable_decode)
{
	...
	while (str_in[i]) {
		...
		case '=':
			...
			{
				str_out[j++] = (himote_hex2int((int) str_in[i + 1]) << 4) 
						+ himote_hex2int((int) str_in[i + 2]);
				i += 3;
...
HIMOTE_FUNCTION(quoted_printable_encode)
{
	char *str, *new_str;
	int str_len;
	size_t new_str_len;
	...
	new_str = (char *)himote_quot_print_encode((unsigned char *)str, (size_t)str_len, &new_str_len);
