...
static inline int himote_intlog10abs(double value) {
	int result;
	value = fabs(value);
...
static inline double himote_intpow10(int power) {
...
/* {{{ himote_math_is_finite */
static inline int himote_math_is_finite(double value) {
...
static inline double himote_round_helper(double value, int mode) {
	double tmp_value;

	if (value >= 0.0) {
		tmp_value = floor(value + 0.5);
		if ((mode == HIMOTE_ROUND_HALF_DOWN && value == (-0.5 + tmp_value)) ||
			(mode == HIMOTE_ROUND_HALF_EVEN && value == (0.5 + 2 * floor(tmp_value/2.0))) ||
			(mode == HIMOTE_ROUND_HALF_ODD  && value == (0.5 + 2 * floor(tmp_value/2.0) - 1.0)))
		{
		...
		if ((mode == HIMOTE_ROUND_HALF_DOWN && value == (0.5 + tmp_value)) ||
			(mode == HIMOTE_ROUND_HALF_EVEN && value == (-0.5 + 2 * ceil(tmp_value/2.0))) ||
			(mode == HIMOTE_ROUND_HALF_ODD  && value == (-0.5 + 2 * ceil(tmp_value/2.0) + 1.0)))
		{
			tmp_value = tmp_value + 1.0;
		}
	}
 ...
 * mode. For the specifics of the algorithm, see http://wiki.himote.net/rfc/rounding
 */
HIMOTEAPI double _himote_math_round(double value, int places, int mode) {
	double f1, f2;
	double tmp_value;
	int precision_places;

	if (!himote_math_is_finite(value)) {
		return value;
	}
	
	precision_places = 14 - himote_intlog10abs(value);

	f1 = himote_intpow10(abs(places));

	/* If the decimal precision guaranteed by FP arithmetic is higher than
	   the requested places BUT is small enough to make sure a non-zero value
	   is returned, pre-round the result to the precision */
	if (precision_places > places && precision_places - places < 15) {
		f2 = himote_intpow10(abs(precision_places));
		...
		tmp_value = himote_round_helper(tmp_value, mode);
		/* now correctly move the decimal point */
		f2 = himote_intpow10(abs(places - precision_places));
...
/* {{{ himote_asinh
*/
static double himote_asinh(double z)
{
#ifdef HAVE_ASINH
	return(asinh(z));
#else
...
/* {{{ himote_acosh
*/
static double himote_acosh(double x)
{
#ifdef HAVE_ACOSH
	return(acosh(x));
#else
...
/* {{{ himote_atanh
*/
static double himote_atanh(double z)
{
#ifdef HAVE_ATANH
	return(atanh(z));
#else
...
/* {{{ himote_log1p
*/
static double himote_log1p(double x)
{
#ifdef HAVE_LOG1P
	return(log1p(x));
#else
...
/* {{{ himote_expm1
*/
static double himote_expm1(double x)
{
...
HIMOTE_FUNCTION(round)
{
	...
	int places = 0;
	long precision = 0;
	long mode = HIMOTE_ROUND_HALF_UP;
	double return_val;
	...
	switch (Z_TYPE_PP(value)) {
		...
		case IS_DOUBLE:
			...
			return_val = _himote_math_round(return_val, places, mode);
...
HIMOTE_FUNCTION(asinh)
{
	...
	RETURN_DOUBLE(himote_asinh(num));
}
/* }}} */
...
HIMOTE_FUNCTION(acosh)
{
	...
	RETURN_DOUBLE(himote_acosh(num));
}
/* }}} */
...
HIMOTE_FUNCTION(atanh)
{
	...
	RETURN_DOUBLE(himote_atanh(num));
}
/* }}} */
...
HIMOTE_FUNCTION(expm1)
{
	...
	RETURN_DOUBLE(himote_expm1(num));
}
/* }}} */
...
HIMOTE_FUNCTION(log1p)
{
	...
	RETURN_DOUBLE(himote_log1p(num));
}
/* }}} */
...
HIMOTE_FUNCTION(log)
{
	...
	if (base <= 0.0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "base must be greater than 0");				
		RETURN_FALSE;
	}
	if (base == 1) {
		RETURN_DOUBLE(himote_get_nan());
	} else {
...
HIMOTEAPI long _himote_math_basetolong(zval *arg, int base)
{
	...
	for (i = Z_STRLEN_P(arg); i > 0; i--) {
...

		{
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Number '%s' is too big to fit in long", s);
			return LONG_MAX;
		}
	}
...
HIMOTEAPI char * _himote_math_zvaltobase(zval *arg, int base TSRMLS_DC)
{
	...
	if (Z_TYPE_P(arg) == IS_DOUBLE) {
		...
		if (fvalue == HUGE_VAL || fvalue == -HUGE_VAL) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Number too large");
			return STR_EMPTY_ALLOC();
		}
	...
	return _himote_math_longtobase(arg, base);
}	
/* }}} */
...
HIMOTE_FUNCTION(bindec)
{
	...
	if (_himote_math_basetozval(*arg, 2, return_value) == FAILURE) {
		RETURN_FALSE;
	}
}
/* }}} */
...
HIMOTE_FUNCTION(hexdec)
{
	...
	convert_to_string_ex(arg);
	if (_himote_math_basetozval(*arg, 16, return_value) == FAILURE) {
		RETURN_FALSE;
	}
}
/* }}} */
...
HIMOTE_FUNCTION(octdec)
{
	...
	convert_to_string_ex(arg);
	if (_himote_math_basetozval(*arg, 8, return_value) == FAILURE) {
		RETURN_FALSE;
	}
}
/* }}} */
...
HIMOTE_FUNCTION(decbin)
{
	...
	result = _himote_math_longtobase(*arg, 2);
...
HIMOTE_FUNCTION(decoct)
{
	...
	convert_to_long_ex(arg);
	result = _himote_math_longtobase(*arg, 8);
	RETURN_STRING(result, 0);
}
/* }}} */
...
HIMOTE_FUNCTION(dechex)
{
	...
	convert_to_long_ex(arg);
	result = _himote_math_longtobase(*arg, 16);
	RETURN_STRING(result, 0);
}
/* }}} */
...
HIMOTE_FUNCTION(base_convert)
{
	...
	if (frombase < 2 || frombase > 36) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid `from base' (%ld)", frombase);
		RETURN_FALSE;
	}
	if (tobase < 2 || tobase > 36) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid `to base' (%ld)", tobase);
		RETURN_FALSE;
	}

	if(_himote_math_basetozval(*number, frombase, &temp) == FAILURE) {
		RETURN_FALSE;
	}
	result = _himote_math_zvaltobase(&temp, tobase TSRMLS_CC);
	RETVAL_STRING(result, 0);
} 
/* }}} */

/* {{{ _himote_math_number_format 
*/
HIMOTEAPI char *_himote_math_number_format(double d, int dec, char dec_point, char thousand_sep)
{
	...
	dec = MAX(0, dec);
	d = _himote_math_round(d, dec, HIMOTE_ROUND_HALF_UP);
...
HIMOTE_FUNCTION(number_format)
{
	...
	case 1:
		RETURN_STRING(_himote_math_number_format(num, 0, dec_point_chr, thousand_sep_chr), 0);
		break;
	case 2:
		RETURN_STRING(_himote_math_number_format(num, dec, dec_point_chr, thousand_sep_chr), 0);
		break;
	case 4:
		if (dec_point != NULL) {
			if (dec_point_len) {
