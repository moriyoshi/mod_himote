...
/* Synced with himote 3.0 revision 1.193 1999-06-16 [ssb] */
 ...
 */
#define himote_my_setlocale setlocale
#ifdef HAVE_LIBINTL
...
#define HIMOTE_PATHINFO_DIRNAME 	1
#define HIMOTE_PATHINFO_BASENAME 	2
#define HIMOTE_PATHINFO_EXTENSION 	4
#define HIMOTE_PATHINFO_FILENAME 	8
#define HIMOTE_PATHINFO_ALL	(HIMOTE_PATHINFO_DIRNAME | HIMOTE_PATHINFO_BASENAME | HIMOTE_PATHINFO_EXTENSION | HIMOTE_PATHINFO_FILENAME)
...
void register_string_constants(INIT_FUNC_ARGS)
{
	...
	REGISTER_LONG_CONSTANT("PATHINFO_DIRNAME", HIMOTE_PATHINFO_DIRNAME, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PATHINFO_BASENAME", HIMOTE_PATHINFO_BASENAME, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PATHINFO_EXTENSION", HIMOTE_PATHINFO_EXTENSION, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("PATHINFO_FILENAME", HIMOTE_PATHINFO_FILENAME, CONST_CS | CONST_PERSISTENT);
...
int himote_tag_find(char *tag, int len, char *set);
...
/* {{{ himote_bin2hex
 */
static char *himote_bin2hex(const unsigned char *old, const size_t oldlen, size_t *newlen)
{
...
HIMOTE_FUNCTION(bin2hex)
{
	...
	result = himote_bin2hex((unsigned char *)data, datalen, &newlen);
...
static void himote_spn_common_handler(INTERNAL_FUNCTION_PARAMETERS, int behavior) /* {{{ */
{
	char *s11, *s22;
	int len1, len2;
	long start = 0, len = 0;
	...
	if (behavior == STR_STRSPN) {
		RETURN_LONG(himote_strspn(s11 + start /*str1_start*/,
						s22 /*str2_start*/,
						s11 + start + len /*str1_end*/,
						s22 + len2 /*str2_end*/));
	} else if (behavior == STR_STRCSPN) {
		RETURN_LONG(himote_strcspn(s11 + start /*str1_start*/,
						s22 /*str2_start*/,
						s11 + start + len /*str1_end*/,
						s22 + len2 /*str2_end*/));
	}
...
{
	himote_spn_common_handler(INTERNAL_FUNCTION_PARAM_PASSTHRU, STR_STRSPN);
}
/* }}} */
...
{
	himote_spn_common_handler(INTERNAL_FUNCTION_PARAM_PASSTHRU, STR_STRCSPN);
}
/* }}} */
...
HIMOTE_FUNCTION(nl_langinfo)
{
	...
	switch(item) { /* {{{ */
		...
		case CODESET:
			...
			break;
		default:
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Item '%ld' is not valid", item);
			RETURN_FALSE;
	}
	/* }}} */
 ...
 */
static inline int himote_charmask(unsigned char *input, int len, char *mask TSRMLS_DC)
{
	unsigned char *end;
	unsigned char c;
	int result = SUCCESS;
	...
	for (end = input+len; input < end; input++) {
		...
		if ((input+3 < end) && input[1] == '.' && input[2] == '.' 
			...
			if (end-len >= input) { /* there was no 'left' char */
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid '..'-range, no character to the left of '..'");
				result = FAILURE;
				continue;
			}
			if (input+2 >= end) { /* there is no 'right' char */
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid '..'-range, no character to the right of '..'");
				result = FAILURE;
				continue;
			}
			if (input[-1] > input[2]) { /* wrong order */
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid '..'-range, '..'-range needs to be incrementing");
				result = FAILURE;
				continue;
			} 
			/* FIXME: better error (a..b..c is the only left possibility?) */
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid '..'-range");
			result = FAILURE;
			continue;
		} else {
			mask[c]=1;
...
HIMOTEAPI char *himote_trim(char *c, int len, char *what, int what_len, zval *return_value, int mode TSRMLS_DC)
{
	...
	if (what) {
		himote_charmask((unsigned char*)what, what_len, mask TSRMLS_CC);
	} else {
		himote_charmask((unsigned char*)" \n\r\t\v\0", 6, mask TSRMLS_CC);
	}
 ...
 */
static void himote_do_trim(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	char *str;
	char *what = NULL;
	...
	himote_trim(str, str_len, what, what_len, return_value, mode TSRMLS_CC);
}
/* }}} */
...
{
	himote_do_trim(INTERNAL_FUNCTION_PARAM_PASSTHRU, 3);
}
/* }}} */
...
{
	himote_do_trim(INTERNAL_FUNCTION_PARAM_PASSTHRU, 2);
}
/* }}} */
...
{
	himote_do_trim(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
HIMOTE_FUNCTION(wordwrap)
{
	...
	if (breakcharlen == 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Break string cannot be empty");
		RETURN_FALSE;
	}

	if (linelength == 0 && docut) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Can't force cut when width is zero");
		RETURN_FALSE;
	}
...
HIMOTEAPI void himote_explode(zval *delim, zval *str, zval *return_value, long limit) 
{
	...
	p1 = Z_STRVAL_P(str);
	p2 = himote_memnstr(Z_STRVAL_P(str), Z_STRVAL_P(delim), Z_STRLEN_P(delim), endp);
	...
	if (p2 == NULL) {
		...
		} while ((p2 = himote_memnstr(p1, Z_STRVAL_P(delim), Z_STRLEN_P(delim), endp)) != NULL &&
				 --limit > 1);
...
/* {{{ himote_explode_negative_limit
 */
HIMOTEAPI void himote_explode_negative_limit(zval *delim, zval *str, zval *return_value, long limit) 
{
...
HIMOTE_FUNCTION(explode)
{
	...
	if (delim_len == 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty delimiter");
		RETURN_FALSE;
	}
	...
	if (limit > 1) {
		himote_explode(&zdelim, &zstr, return_value, limit);
	} else if (limit < 0) {
		himote_explode_negative_limit(&zdelim, &zstr, return_value, limit);
	} else {
...
HIMOTE_FUNCTION(implode)
{
	...
	if (arg2 == NULL) {
		if (Z_TYPE_PP(arg1) != IS_ARRAY) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument must be an array");
			return;
		}
...
#define _IMPL_EMPTY ""
		...
		if (Z_TYPE_PP(arg1) == IS_ARRAY) {
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid arguments passed");
			return;
		}
	}
	
	himote_implode(delim, arr, return_value TSRMLS_CC);
...
HIMOTEAPI void himote_basename(char *s, size_t len, char *suffix, size_t sufflen, char **p_ret, size_t *p_len TSRMLS_DC)
{
	...
	state = 0;
	while (cnt > 0) {
		inc_len = (*c == '\0' ? 1: himote_mblen(c, cnt));
...
HIMOTE_FUNCTION(basename)
{
	...
	himote_basename(string, string_len, suffix, suffix_len, &ret, &ret_len TSRMLS_CC);
...
HIMOTEAPI size_t himote_dirname(char *path, size_t len)
{
...
HIMOTE_FUNCTION(dirname)
{
	...
	ret_len = himote_dirname(ret, str_len);
...
HIMOTE_FUNCTION(pathinfo)
{
	...
	size_t ret_len;
	long opt = HIMOTE_PATHINFO_ALL;

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|l", &path, &path_len, &opt) == FAILURE) {
		return;
	}

	have_basename = ((opt & HIMOTE_PATHINFO_BASENAME) == HIMOTE_PATHINFO_BASENAME);
	
	MAKE_STD_ZVAL(tmp);
	array_init(tmp);
	
	if ((opt & HIMOTE_PATHINFO_DIRNAME) == HIMOTE_PATHINFO_DIRNAME) {
		ret = estrndup(path, path_len);
		himote_dirname(ret, path_len);
		if (*ret) {
	...
	if (have_basename) {
		himote_basename(path, path_len, NULL, 0, &ret, &ret_len TSRMLS_CC);
		add_assoc_stringl(tmp, "basename", ret, ret_len, 0);
	}
	
	if ((opt & HIMOTE_PATHINFO_EXTENSION) == HIMOTE_PATHINFO_EXTENSION) {
		char *p;
		int idx;
	...
	if ((opt & HIMOTE_PATHINFO_FILENAME) == HIMOTE_PATHINFO_FILENAME) {
		char *p;
		int idx;
...
{
	himote_strtolower(s, s_len);
	himote_strtolower(t, t_len);
	return himote_memnstr(s, t, t_len, s + s_len);
}
/* }}} */
...
/* {{{ himote_needle_char
 */
static int himote_needle_char(zval *needle, char *target TSRMLS_DC)
{
	switch (Z_TYPE_P(needle)) {
		...
		default: {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "needle is not a string or an integer");
			return FAILURE;
		 }
	}
}
...
HIMOTE_FUNCTION(stristr)
{
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		found = himote_stristr(haystack_dup, orig_needle,	haystack_len, Z_STRLEN_P(needle));
		efree(orig_needle);
	} else {
		if (himote_needle_char(needle, needle_char TSRMLS_CC) != SUCCESS) {
			efree(haystack_dup);
			RETURN_FALSE;
		}
		needle_char[1] = 0;

		found = himote_stristr(haystack_dup, needle_char,	haystack_len, 1);
	}
...
HIMOTE_FUNCTION(strstr)
{
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		found = himote_memnstr(haystack, Z_STRVAL_P(needle), Z_STRLEN_P(needle), haystack + haystack_len);
	} else {
		if (himote_needle_char(needle, needle_char TSRMLS_CC) != SUCCESS) {
			RETURN_FALSE;
		}
		...
		found = himote_memnstr(haystack, needle_char,	1, haystack + haystack_len);
	}
...
HIMOTE_FUNCTION(strpos)
{
	...
	if (offset < 0 || offset > haystack_len) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset not contained in string");
		RETURN_FALSE;
	}
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		found = himote_memnstr(haystack + offset,
			                Z_STRVAL_P(needle),
			                Z_STRLEN_P(needle),
...
HIMOTE_FUNCTION(stripos)
{
	...
	haystack_dup = estrndup(haystack, haystack_len);
	himote_strtolower(haystack_dup, haystack_len);
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		himote_strtolower(needle_dup, Z_STRLEN_P(needle));
		found = himote_memnstr(haystack_dup + offset, needle_dup, Z_STRLEN_P(needle), haystack_dup + haystack_len);
	} else {
		if (himote_needle_char(needle, needle_char TSRMLS_CC) != SUCCESS) {
			efree(haystack_dup);
			RETURN_FALSE;
		...
		needle_char[1] = '\0';
		found = himote_memnstr(haystack_dup + offset, 
							needle_char, 
...
HIMOTE_FUNCTION(strrpos)
{
	...
	if (Z_TYPE_P(zneedle) == IS_STRING) {
		...
		if (himote_needle_char(zneedle, ord_needle TSRMLS_CC) != SUCCESS) {
			RETURN_FALSE;
		}
		ord_needle[1] = '\0';
		needle = ord_needle;
	...
	if (offset >= 0) {
		if (offset > haystack_len) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset is greater than the length of haystack string");
			RETURN_FALSE;
		}
		p = haystack + offset;
...
HIMOTE_FUNCTION(strripos)
{
	...
	himote_strtolower(needle_dup, needle_len);
	haystack_dup = estrndup(haystack, haystack_len);
	himote_strtolower(haystack_dup, haystack_len);
...
HIMOTE_FUNCTION(strrchr)
{
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		...
		char needle_chr;
		if (himote_needle_char(needle, &needle_chr TSRMLS_CC) != SUCCESS) {
			RETURN_FALSE;
		}
...
/* {{{ himote_chunk_split
 */
static char *himote_chunk_split(char *src, int srclen, char *end, int endlen, int chunklen, int *destlen)
{
	char *dest;
	char *p, *q;
...
HIMOTE_FUNCTION(chunk_split) 
{
	...
	if (chunklen <= 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Chunk length should be greater than zero");
		RETURN_FALSE;
	}
	...
	result = himote_chunk_split(str, str_len, end, endlen, chunklen, &result_len);
...
HIMOTE_FUNCTION(substr_replace)
{
	...
	if (Z_TYPE_PP(str) == IS_STRING) {
		if (
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "'from' and 'len' should be of same type - numerical or array ");
		...
		if (argc == 4 && Z_TYPE_PP(from) == IS_ARRAY) {
			if (zend_hash_num_elements(Z_ARRVAL_PP(from)) != zend_hash_num_elements(Z_ARRVAL_PP(len))) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "'from' and 'len' should have the same number of elements");
				RETURN_STRINGL(Z_STRVAL_PP(str), Z_STRLEN_PP(str), 1);		
			}
		}
	}
	...
	if (Z_TYPE_PP(str) != IS_ARRAY) {
		if (Z_TYPE_PP(from) != IS_ARRAY) {
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Functionality of 'from' and 'len' as arrays is not implemented");
			RETURN_STRINGL(Z_STRVAL_PP(str), Z_STRLEN_PP(str), 1);	
		}
...
static void himote_ucfirst(char *str) 
{
	register char *r;
	r = str;
...
HIMOTE_FUNCTION(ucfirst)
{
	...
	himote_ucfirst(Z_STRVAL_P(return_value));
}
/* }}} */

/* {{{
   Lowercase the first character of the word in a native string */
static void himote_lcfirst(char *str)
{
	register char *r;
	r = str;
...
HIMOTE_FUNCTION(lcfirst)
{
	...
	ZVAL_STRINGL(return_value, str, str_len, 1);
	himote_lcfirst(Z_STRVAL_P(return_value));
}
/* }}} */
...
/* {{{ himote_strtr_array
 */
static void himote_strtr_array(zval *return_value, char *str, int slen, HashTable *hash)
{
	zval **entry;
	char  *string_key;
	uint   string_key_len;
...
{								
	...
	if (ac == 2 && Z_TYPE_PP(from) != IS_ARRAY) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "The second argument is not an array");
		RETURN_FALSE;
	}
	...
	if (ac == 2) {
		himote_strtr_array(return_value, str, str_len, HASH_OF(*from));
	} else {
		convert_to_string_ex(from);

		ZVAL_STRINGL(return_value, str, str_len, 1);
		
		himote_strtr(Z_STRVAL_P(return_value),
...
/* {{{ himote_similar_str
 */
static void himote_similar_str(const char *txt1, int len1, const char *txt2, int len2, int *pos1, int *pos2, int *max)
{
	char *p, *q;
...
/* {{{ himote_similar_char
 */
static int himote_similar_char(const char *txt1, int len1, const char *txt2, int len2)
{
	int sum;
	int pos1, pos2, max;

	himote_similar_str(txt1, len1, txt2, len2, &pos1, &pos2, &max);
	if ((sum = max)) {
		if (pos1 && pos2) {
			sum += himote_similar_char(txt1, pos1, 
									txt2, pos2);
		}
		if ((pos1 + max < len1) && (pos2 + max < len2)) {
			sum += himote_similar_char(txt1 + pos1 + max, len1 - pos1 - max, 
...
HIMOTE_FUNCTION(similar_text)
{
	...
	sim = himote_similar_char(t1, t1_len, t2, t2_len);	
...
HIMOTE_FUNCTION(addcslashes)
{
	...
	Z_STRVAL_P(return_value) = himote_addcslashes(str, str_len, &Z_STRLEN_P(return_value), 0, what, what_len TSRMLS_CC);
...
HIMOTE_FUNCTION(addslashes)
{
	...
	RETURN_STRING(himote_addslashes(str,
	                             str_len, 
...
HIMOTE_FUNCTION(stripcslashes)
{
	...
	ZVAL_STRINGL(return_value, str, str_len, 1);
	himote_stripcslashes(Z_STRVAL_P(return_value), &Z_STRLEN_P(return_value));
}
/* }}} */
...
HIMOTE_FUNCTION(stripslashes)
{
	...
	ZVAL_STRINGL(return_value, str, str_len, 1);
	himote_stripslashes(Z_STRVAL_P(return_value), &Z_STRLEN_P(return_value) TSRMLS_CC);
}
/* }}} */
...
/* {{{ himote_strerror
 */
char *himote_strerror(int errnum) 
{
	extern int sys_nerr;
	...
	(void) snprintf(BG(str_ebuf), sizeof(himote_basic_globals.str_ebuf), "Unknown error: %d", errnum);
	return(BG(str_ebuf));
}
/* }}} */
#endif

/* {{{ himote_stripcslashes
 */
...
/* {{{ himote_addcslashes
 */
HIMOTEAPI char *himote_addcslashes(char *str, int length, int *new_length, int should_free, char *what, int wlength TSRMLS_DC)
{
	char flags[256];
	...
	himote_charmask((unsigned char *)what, wlength, flags TSRMLS_CC);
...
/* {{{ himote_addslashes
 */
HIMOTEAPI char *himote_addslashes(char *str, int length, int *new_length, int should_free TSRMLS_DC)
{
	return himote_addslashes_ex(str, length, new_length, should_free, 0 TSRMLS_CC);
}
/* }}} */

/* {{{ himote_addslashes_ex
 */
HIMOTEAPI char *himote_addslashes_ex(char *str, int length, int *new_length, int should_free, int ignore_sybase TSRMLS_DC)
{
...
{
	return himote_char_to_str_ex(str, len, from, to, to_len, result, 1, NULL);
}
/* }}} */

/* {{{ himote_str_to_str_ex
 */
HIMOTEAPI char *himote_str_to_str_ex(char *haystack, int length, 
		...
		if (needle_len == str_len) {
			...
			*_new_length = length;
			...
			if (case_sensitivity) {
				...
				for (p = new_str; (r = himote_memnstr(p, needle, needle_len, end)); p = r + needle_len) {
				...
				needle_dup = estrndup(needle, needle_len);
				himote_strtolower(haystack_dup, length);
				himote_strtolower(needle_dup, needle_len);
				end = haystack_dup + length;
				for (p = haystack_dup; (r = himote_memnstr(p, needle_dup, needle_len, end)); p = r + needle_len) {
			...
			if (str_len < needle_len) {
				...
				while ((o = himote_memnstr(o, n, needle_len, endp))) {
					o += needle_len;
					count++;
				}
				if (count == 0) {
			...
			if (case_sensitivity) {
				...
				for (p = haystack; (r = himote_memnstr(p, needle, needle_len, end)); p = r + needle_len) {
					memcpy(e, p, r - p);
					e += r - p;
...
nothing_todo:
		*_new_length = length;
		...
		if (case_sensitivity && memcmp(haystack, needle, length)) {
			...
			himote_strtolower(l_haystack, length);
			himote_strtolower(l_needle, length);
...
/* {{{ himote_str_to_str
 */
HIMOTEAPI char *himote_str_to_str(char *haystack, int length, 
	char *needle, int needle_len, char *str, int str_len, int *_new_length)
{
	return himote_str_to_str_ex(haystack, length, needle, needle_len, str, str_len, _new_length, 1, NULL);
} 
/* }}} */

/* {{{ himote_str_replace_in_subject
 */
static void himote_str_replace_in_subject(zval *search, zval *replace, zval **subject, zval *result, int case_sensitivity, int *replace_count)
{
	zval		**search_entry,
				**replace_entry = NULL,
				  temp_result;
	...
	if (Z_TYPE_P(search) == IS_ARRAY) {
		...
		while (zend_hash_get_current_data(Z_ARRVAL_P(search), (void **) &search_entry) == SUCCESS) {
			...
			if (Z_STRLEN_PP(search_entry) == 1) {
				himote_char_to_str_ex(Z_STRVAL_P(result),
								Z_STRLEN_P(result),
				...
				Z_STRVAL(temp_result) = himote_str_to_str_ex(Z_STRVAL_P(result), Z_STRLEN_P(result),
		...
		if (Z_STRLEN_P(search) == 1) {
			himote_char_to_str_ex(Z_STRVAL_PP(subject),
							Z_STRLEN_PP(subject),
							Z_STRVAL_P(search)[0],
							Z_STRVAL_P(replace),
							Z_STRLEN_P(replace),
			...
			Z_STRVAL_P(result) = himote_str_to_str_ex(Z_STRVAL_PP(subject), Z_STRLEN_PP(subject),
...
/* {{{ himote_str_replace_common
 */
static void himote_str_replace_common(INTERNAL_FUNCTION_PARAMETERS, int case_sensitivity)
{
	...
	if (Z_TYPE_PP(subject) == IS_ARRAY) {
		...
		while (zend_hash_get_current_data(Z_ARRVAL_PP(subject), (void **)&subject_entry) == SUCCESS) {
			if (Z_TYPE_PP(subject_entry) != IS_ARRAY && Z_TYPE_PP(subject_entry) != IS_OBJECT) {
				...
				himote_str_replace_in_subject(*search, *replace, subject_entry, result, case_sensitivity, (argc > 3) ? &count : NULL);
			} else {
				ALLOC_ZVAL(result);
		...
		himote_str_replace_in_subject(*search, *replace, subject, return_value, case_sensitivity, (argc > 3) ? &count : NULL);
	}	
	if (argc > 3) {
		zval_dtor(*zcount);
...
{
	himote_str_replace_common(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
{
	himote_str_replace_common(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
 ...
 */
static void himote_hebrev(INTERNAL_FUNCTION_PARAMETERS, int convert_newlines)
{
	char *str;
	...
	*target = 0;
	...
	if (convert_newlines) {
		himote_char_to_str(broken_str, str_len,'\n', "<br />\n", 7, return_value);
		efree(broken_str);
	} else {
...
{
	himote_hebrev(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	himote_hebrev(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
/* {{{ proto string strip_tags(string str [, string allowable_tags])
   Strips HTML and HIMOTE tags from a string */
HIMOTE_FUNCTION(strip_tags)
{
	...
	retval_len = himote_strip_tags_ex(buf, str_len, NULL, allowed_tags, allowed_tags_len, 0);
...
HIMOTE_FUNCTION(setlocale)
{
	...
	if (Z_TYPE_PP(pcategory) == IS_LONG) {
		...
		himote_error_docref(NULL TSRMLS_CC, E_DEPRECATED, "Passing locale category name as string is deprecated. Use the LC_* -constants instead");
		...
		if (!strcasecmp("LC_ALL", category)) {
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid locale category name %s, must be one of LC_ALL, LC_COLLATE, LC_CTYPE, LC_MONETARY, LC_NUMERIC, or LC_TIME", category);
	...
	while (1) {
		...
		if (!strcmp ("0", Z_STRVAL_PP(plocale))) {
			...
			if (Z_STRLEN_PP(plocale) >= 255) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Specified locale name is too long");
				break;
			}
		}

		retval = himote_my_setlocale(cat, loc);
...
#define HIMOTE_TAG_BUF_SIZE 1023
...
HIMOTEAPI size_t himote_strip_tags(char *rbuf, int len, int *stateptr, char *allow, int allow_len) /* {{{ */
{
	return himote_strip_tags_ex(rbuf, len, stateptr, allow, allow_len, 0);
}
/* }}} */

/* {{{ himote_strip_tags
 
	A simple little state-machine to strip out html and himote tags 
	
	State 0 is the output state, State 1 means we are inside a
	normal html tag and state 2 means we are inside a himote tag.
	...
	swm: Added ability to strip <?xml tags without assuming it HIMOTE
	code.
*/
HIMOTEAPI size_t himote_strip_tags_ex(char *rbuf, int len, int *stateptr, char *allow, int allow_len, zend_bool allow_tag_spaces)
{
	...
	br = 0;
	if (allow) {
		himote_strtolower(allow, allow_len);
		tbuf = emalloc(HIMOTE_TAG_BUF_SIZE + 1);
		tp = tbuf;
	} else {
		tbuf = tp = NULL;
	}
		...
		switch (c) {
			...
			case '<':
				...
				if (state == 0) {
					...
					state = 1;
					if (allow) {
						if (tp - tbuf >= HIMOTE_TAG_BUF_SIZE) {
							pos = tp - tbuf;
							tbuf = erealloc(tbuf, (tp - tbuf) + HIMOTE_TAG_BUF_SIZE + 1);
							tp = tbuf + pos;
						}
						*(tp++) = '<';
				 	}
				...
				switch (state) {
					case 1: /* HTML/XML */
						...
						if (allow) {
							...
							*(tp++) = '>';
							*tp='\0';
							if (himote_tag_find(tbuf, tp-tbuf, allow)) {
			...
			case 'L':
				 ...
				 * state == 2 (HIMOTE). Switch back to HTML.
				 */
...
HIMOTE_FUNCTION(str_getcsv)
{
	...
	himote_fgetcsv(NULL, delim, enc, esc, str_len, str, return_value TSRMLS_CC);
}
/* }}} */
...
HIMOTE_FUNCTION(str_repeat)
{
	...
	if (mult < 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Second argument has to be greater than or equal to 0");
		return;
	}
...
HIMOTE_FUNCTION(count_chars)
{
	...
	if (mymode < 0 || mymode > 4) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Unknown mode");
		RETURN_FALSE;
	}
...
/* {{{ himote_strnatcmp
 */
static void himote_strnatcmp(INTERNAL_FUNCTION_PARAMETERS, int fold_case)
{
	char *s1, *s2;
	int s1_len, s2_len;
...
{
	himote_strnatcmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	himote_strnatcmp(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
HIMOTE_FUNCTION(substr_count)
{
	...
	if (needle_len == 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Empty substring");
		RETURN_FALSE;
	}
	...
	if (offset < 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset should be greater than or equal to 0");
		RETURN_FALSE;		
	}

	if (offset > haystack_len) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Offset value %ld exceeds string length", offset);
		RETURN_FALSE;		
	}
	p += offset;
	...
	if (ac == 4) {
		...
		if (length <= 0) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Length should be greater than 0");
			RETURN_FALSE;		
		}
		if (length > (haystack_len - offset)) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Length value %ld exceeds string length", length);
			RETURN_FALSE;
		}
		endp = p + length;
	}
	...
	if (needle_len == 1) {
		...
		while ((p = himote_memnstr(p, needle, needle_len, endp))) {
			p += needle_len;
			count++;
		}
	}
...
HIMOTE_FUNCTION(str_pad)
{
	...
	if (pad_str_len == 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Padding string cannot be empty");
		return;
	}
	
	if (pad_type_val < STR_PAD_LEFT || pad_type_val > STR_PAD_BOTH) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Padding type has to be STR_PAD_LEFT, STR_PAD_RIGHT, or STR_PAD_BOTH");
		return;
	}

	num_pad_chars = pad_length - input_len;
	if (num_pad_chars >= INT_MAX) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Padding length is too long");
		return;	
	}
...
HIMOTE_FUNCTION(sscanf)
{
	...
	result = himote_sscanf_internal(str, format, num_args, args, 0, &return_value TSRMLS_CC);
...
HIMOTE_FUNCTION(str_rot13)
{
	...
	himote_strtr(Z_STRVAL_P(return_value), Z_STRLEN_P(return_value), rot13_from, rot13_to, 52);
}
/* }}} */

static void himote_string_shuffle(char *str, long len TSRMLS_DC) /* {{{ */
{
...
HIMOTE_FUNCTION(str_shuffle)
{
	...
	if (Z_STRLEN_P(return_value) > 1) { 
		himote_string_shuffle(Z_STRVAL_P(return_value), (long) Z_STRLEN_P(return_value) TSRMLS_CC);
	}
}
/* }}} */
...
HIMOTE_FUNCTION(str_word_count)
{
	...
	switch(type) {
		...
		case 0:
			...
			break;
		default:
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid format value %ld", type);
			RETURN_FALSE;
	}

	if (char_list) {
		himote_charmask((unsigned char *)char_list, char_list_len, ch TSRMLS_CC);
	}
...
HIMOTE_FUNCTION(money_format)
{
	...
	while ((p = memchr(p, '%', (e - p)))) {
		if (*(p + 1) == '%') {
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Only a single %%i or %%n token can be used");
			RETURN_FALSE;
		}
	}
...
HIMOTE_FUNCTION(str_split)
{
	...
	if (split_length <= 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "The length of each segment must be greater than zero");
		RETURN_FALSE;
	}
...
HIMOTE_FUNCTION(strpbrk)
{
	...
	if (!char_list_len) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "The character list cannot be empty");
		RETURN_FALSE;	
	}
...
HIMOTE_FUNCTION(substr_compare)
{
	...
	if (ZEND_NUM_ARGS() >= 4 && len <= 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "The length must be greater than zero");
		RETURN_FALSE;
	}
	...
	if (offset >= s1_len) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "The start position cannot exceed initial string length");
		RETURN_FALSE;
	}
