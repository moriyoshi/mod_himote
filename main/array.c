...
/* {{{ himote_array_init_globals
*/
static void himote_array_init_globals(zend_array_globals *array_globals)
{
...
{
	ZEND_INIT_MODULE_GLOBALS(array, himote_array_init_globals, NULL);
	...
	REGISTER_LONG_CONSTANT("SORT_ASC", HIMOTE_SORT_ASC, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SORT_DESC", HIMOTE_SORT_DESC, CONST_CS | CONST_PERSISTENT);

	REGISTER_LONG_CONSTANT("SORT_REGULAR", HIMOTE_SORT_REGULAR, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SORT_NUMERIC", HIMOTE_SORT_NUMERIC, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SORT_STRING", HIMOTE_SORT_STRING, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("SORT_LOCALE_STRING", HIMOTE_SORT_LOCALE_STRING, CONST_CS | CONST_PERSISTENT);
...
static void himote_set_compare_func(int sort_type TSRMLS_DC) /* {{{ */
{
	switch (sort_type) {
		case HIMOTE_SORT_NUMERIC:
			ARRAYG(compare_func) = numeric_compare_function;
			break;

		case HIMOTE_SORT_STRING:
			ARRAYG(compare_func) = string_compare_function;
			break;

#if HAVE_STRCOLL
		case HIMOTE_SORT_LOCALE_STRING:
			ARRAYG(compare_func) = string_locale_compare_function;
			break;
#endif

		case HIMOTE_SORT_REGULAR:
		default:
...
static int himote_array_key_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	Bucket *f;
	Bucket *s;
	zval result;
...
static int himote_array_reverse_key_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	return himote_array_key_compare(a, b TSRMLS_CC) * -1;
}
/* }}} */
...
{
	zval *array;
	long sort_type = HIMOTE_SORT_REGULAR;

	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "a|l", &array, &sort_type) == FAILURE) {
		RETURN_FALSE;
	}

	himote_set_compare_func(sort_type TSRMLS_CC);

	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_reverse_key_compare, 0 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
HIMOTE_FUNCTION(ksort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_key_compare, 0 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
static int himote_count_recursive(zval *array, long mode TSRMLS_DC) /* {{{ */
{
	long cnt = 0;
	zval **element;
	...
	if (Z_TYPE_P(array) == IS_ARRAY) {
		if (Z_ARRVAL_P(array)->nApplyCount > 1) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "recursion detected");
			return 0;
		}
		...
		if (mode == COUNT_RECURSIVE) {
			...
			for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_P(array), &pos);
				...
				cnt += himote_count_recursive(*element, COUNT_RECURSIVE TSRMLS_CC);
				Z_ARRVAL_P(array)->nApplyCount--;
			}
		}
	}
...
HIMOTE_FUNCTION(count)
{
	...
	switch (Z_TYPE_P(array)) {
		case IS_NULL:
			...
			break;
		case IS_ARRAY:
			RETURN_LONG (himote_count_recursive (array, mode TSRMLS_CC));
			break;
		case IS_OBJECT: {
 ...
 */
static int himote_array_data_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	Bucket *f;
	Bucket *s;
	zval result;
...
static int himote_array_reverse_data_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	return himote_array_data_compare(a, b TSRMLS_CC) * -1;
}
/* }}} */

static int himote_array_natural_general_compare(const void *a, const void *b, int fold_case) /* {{{ */
{
	Bucket *f, *s;
	zval *fval, *sval;
	zval first, second;
...
static int himote_array_natural_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	return himote_array_natural_general_compare(a, b, 0);
}
/* }}} */

static int himote_array_natural_case_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	return himote_array_natural_general_compare(a, b, 1);
}
/* }}} */

static void himote_natsort(INTERNAL_FUNCTION_PARAMETERS, int fold_case) /* {{{ */
{
	zval *array;
	...
	if (fold_case) {
		if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_natural_case_compare, 0 TSRMLS_CC) == FAILURE) {
			return;
		}
	} else {
		if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_natural_compare, 0 TSRMLS_CC) == FAILURE) {
			return;
		}
	}
...
{
	himote_natsort(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	himote_natsort(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
HIMOTE_FUNCTION(asort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_data_compare, 0 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
HIMOTE_FUNCTION(arsort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_reverse_data_compare, 0 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
HIMOTE_FUNCTION(sort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_data_compare, 1 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
HIMOTE_FUNCTION(rsort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_reverse_data_compare, 1 TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
static int himote_array_user_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	Bucket *f;
	Bucket *s;
	zval **args[2];
...
#define HIMOTE_ARRAY_CMP_FUNC_CHECK(func_name)	\
	if (!zend_is_callable(*func_name, 0, NULL TSRMLS_CC)) {	\
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid comparison function");	\
...
#define HIMOTE_ARRAY_CMP_FUNC_VARS \
	zend_fcall_info old_user_compare_fci; \
	zend_fcall_info_cache old_user_compare_fci_cache \

#define HIMOTE_ARRAY_CMP_FUNC_BACKUP() \
	old_user_compare_fci = BG(user_compare_fci); \
	old_user_compare_fci_cache = BG(user_compare_fci_cache); \
	BG(user_compare_fci_cache) = empty_fcall_info_cache; \

#define HIMOTE_ARRAY_CMP_FUNC_RESTORE() \
	BG(user_compare_fci) = old_user_compare_fci; \
	BG(user_compare_fci_cache) = old_user_compare_fci_cache; \
...
HIMOTE_FUNCTION(usort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_user_compare, 1 TSRMLS_CC) == FAILURE) {
		RETVAL_FALSE;
	} else {
		if (refcount > Z_REFCOUNT_P(array)) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Array was modified by the user comparison function");
			RETVAL_FALSE;
		} else {
			RETVAL_TRUE;
		}
...
HIMOTE_FUNCTION(uasort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_user_compare, 0 TSRMLS_CC) == FAILURE) {
		RETVAL_FALSE;
	} else {
		if (refcount > Z_REFCOUNT_P(array)) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Array was modified by the user comparison function");
...
static int himote_array_user_key_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	Bucket *f;
	Bucket *s;
	zval *key1, *key2;
...
HIMOTE_FUNCTION(uksort)
{
	...
	if (zend_hash_sort(Z_ARRVAL_P(array), zend_qsort, himote_array_user_key_compare, 0 TSRMLS_CC) == FAILURE) {
		RETVAL_FALSE;
	} else {
		if (refcount > Z_REFCOUNT_P(array)) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Array was modified by the user comparison function");
...
HIMOTE_FUNCTION(min)
{
	...
	himote_set_compare_func(HIMOTE_SORT_REGULAR TSRMLS_CC);
	...
	if (argc == 1) {
		...
		if (Z_TYPE_PP(args[0]) != IS_ARRAY) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "When only one parameter is given, it must be an array");
			RETVAL_NULL();
		} else {
			if (zend_hash_minmax(Z_ARRVAL_PP(args[0]), himote_array_data_compare, 0, (void **) &result TSRMLS_CC) == SUCCESS) {
				RETVAL_ZVAL(*result, 1, 0);
			} else {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Array must contain at least one element");
				RETVAL_FALSE;
			}
		}
	} else {
...
HIMOTE_FUNCTION(max)
{
	...
	if (argc == 1) {
		...
		if (Z_TYPE_PP(args[0]) != IS_ARRAY) {
			...
			if (zend_hash_minmax(Z_ARRVAL_PP(args[0]), himote_array_data_compare, 1, (void **) &result TSRMLS_CC) == SUCCESS) {
				RETVAL_ZVAL(*result, 1, 0);
			} else {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Array must contain at least one element");
				RETVAL_FALSE;
...
static int himote_array_walk(HashTable *target_hash, zval *userdata, int recursive TSRMLS_DC) /* {{{ */
{
	...
	while (!EG(exception) && zend_hash_get_current_data(target_hash, (void **)&args[0]) == SUCCESS) {
		if (recursive && Z_TYPE_PP(args[0]) == IS_ARRAY) {
			...
			thash->nApplyCount++;
			himote_array_walk(thash, userdata, recursive TSRMLS_CC);
			thash->nApplyCount--;
...
HIMOTE_FUNCTION(array_walk)
{
	...
	himote_array_walk(array, userdata, 0 TSRMLS_CC);
	BG(array_walk_fci) = orig_array_walk_fci;
	BG(array_walk_fci_cache) = orig_array_walk_fci_cache;
	RETURN_TRUE;
}
...
HIMOTE_FUNCTION(array_walk_recursive)
{
	...
	himote_array_walk(array, userdata, 1 TSRMLS_CC);
	BG(array_walk_fci) = orig_array_walk_fci;
	BG(array_walk_fci_cache) = orig_array_walk_fci_cache;
	RETURN_TRUE;
}
...
/* void himote_search_array(INTERNAL_FUNCTION_PARAMETERS, int behavior)
 * 0 = return boolean
 * 1 = return key
 */
static void himote_search_array(INTERNAL_FUNCTION_PARAMETERS, int behavior) /* {{{ */
{
...
{
	himote_search_array(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
{
	himote_search_array(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */

static int himote_valid_var_name(char *var_name, int var_name_len) /* {{{ */
{
	int i, ch;
...
HIMOTE_FUNCTION(extract)
{
	...
	if (extract_type < EXTR_OVERWRITE || extract_type > EXTR_IF_EXISTS) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid extract type");
		return;
	}

	if (extract_type > EXTR_SKIP && extract_type <= EXTR_PREFIX_IF_EXISTS && ZEND_NUM_ARGS() < 3) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "specified extract type requires the prefix parameter");
		return;
	}

	if (prefix) {
		convert_to_string(prefix);
		if (Z_STRLEN_P(prefix) && !himote_valid_var_name(Z_STRVAL_P(prefix), Z_STRLEN_P(prefix))) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "prefix is not a valid identifier");
			return;
		}
	}
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(var_array), (void **)&entry, &pos) == SUCCESS) {
		...
		if (key_type == HASH_KEY_IS_STRING) {
			...
			himote_prefix_varname(&final_name, prefix, Z_STRVAL(num), Z_STRLEN(num), 1 TSRMLS_CC);
			zval_dtor(&num);
		} else {
		...
		switch (extract_type) {
			...
			case EXTR_PREFIX_IF_EXISTS:
				if (var_exists) {
					himote_prefix_varname(&final_name, prefix, var_name, var_name_len, 1 TSRMLS_CC);
				}
				break;
			...
			case EXTR_PREFIX_INVALID:
				if (Z_TYPE(final_name) == IS_NULL) {
					if (!himote_valid_var_name(var_name, var_name_len)) {
						himote_prefix_varname(&final_name, prefix, var_name, var_name_len, 1 TSRMLS_CC);
					} else {
						ZVAL_STRINGL(&final_name, var_name, var_name_len, 1);
					}
		...
		if (Z_TYPE(final_name) != IS_NULL && himote_valid_var_name(Z_STRVAL(final_name), Z_STRLEN(final_name))) {
			if (extract_refs) {
				zval **orig_var;
...
static void himote_compact_var(HashTable *eg_active_symbol_table, zval *return_value, zval *entry TSRMLS_DC) /* {{{ */
{
	...
	if (Z_TYPE_P(entry) == IS_STRING) {
		...
		while (zend_hash_get_current_data_ex(Z_ARRVAL_P(entry), (void**)&value_ptr, &pos) == SUCCESS) {
			...
			himote_compact_var(eg_active_symbol_table, return_value, value TSRMLS_CC);
...
HIMOTE_FUNCTION(compact)
{
	...
	for (i=0; i<ZEND_NUM_ARGS(); i++) {
		himote_compact_var(EG(active_symbol_table), return_value, *args[i] TSRMLS_CC);
	}
...
HIMOTE_FUNCTION(array_fill)
{
	...
	if (num < 1) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Number of elements must be positive");
		RETURN_FALSE;
	}
	...
	while (num--) {
		if (zend_hash_next_index_insert(Z_ARRVAL_P(return_value), &val, sizeof(zval *), NULL) == SUCCESS) {
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Cannot add element to the array as the next element is already occupied");
			RETURN_FALSE;
		}
	}
}
...
err:
	if (err) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "step exceeds the specified range");
		zval_dtor(return_value);
		RETURN_FALSE;
	}
}
...
static void himote_array_data_shuffle(zval *array TSRMLS_DC) /* {{{ */
{
	Bucket **elems, *temp;
	HashTable *hash;
	...
	for (j = 0, temp = hash->pListHead; temp; temp = temp->pListNext)
		elems[j++] = temp;
	while (--n_left) {
		rnd_idx = himote_rand(TSRMLS_C);
		RAND_RANGE(rnd_idx, 0, n_left, HIMOTE_RAND_MAX);
		if (rnd_idx != n_left) {
			temp = elems[n_left];
...
HIMOTE_FUNCTION(shuffle)
{
	...
	himote_array_data_shuffle(array TSRMLS_CC);

	RETURN_TRUE;
}
/* }}} */

HIMOTEAPI HashTable* himote_splice(HashTable *in_hash, int offset, int length, zval ***list, int list_count, HashTable **removed) /* {{{ */
{
...
/* {{{ void _himotei_pop(INTERNAL_FUNCTION_PARAMETERS, int off_the_end) */
static void _himotei_pop(INTERNAL_FUNCTION_PARAMETERS, int off_the_end)
{
...
{
	_himotei_pop(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
{
	_himotei_pop(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */
...
HIMOTE_FUNCTION(array_unshift)
{
	...
	new_hash = himote_splice(Z_ARRVAL_P(stack), 0, 0, &args[0], argc, NULL);
...
HIMOTE_FUNCTION(array_splice)
{
	...
	/* Perform splice */
	new_hash = himote_splice(Z_ARRVAL_P(array), offset, length, repl, repl_num, rem_hash);
...
HIMOTEAPI int himote_array_merge(HashTable *dest, HashTable *src, int recursive TSRMLS_DC) /* {{{ */
{
		...
		switch (zend_hash_get_current_key_ex(src, &string_key, &string_key_len, &num_key, 0, &pos)) {
			case HASH_KEY_IS_STRING:
				if (recursive && zend_hash_find(dest, string_key, string_key_len, (void **)&dest_entry) == SUCCESS) {
					...
					if (!himote_array_merge(Z_ARRVAL_PP(dest_entry), Z_ARRVAL_PP(src_entry), recursive TSRMLS_CC)) {
						if (thash) {
							thash->nApplyCount--;
						}
						return 0;
...
HIMOTEAPI int himote_array_replace_recursive(HashTable *dest, HashTable *src TSRMLS_DC) /* {{{ */
{
	zval **src_entry, **dest_entry;
	char *string_key;
	uint string_key_len;
	...
	for (zend_hash_internal_pointer_reset_ex(src, &pos);
		...
		if (!himote_array_replace_recursive(Z_ARRVAL_PP(dest_entry), Z_ARRVAL_PP(src_entry) TSRMLS_CC)) {
			Z_ARRVAL_PP(dest_entry)->nApplyCount--;
			Z_ARRVAL_PP(src_entry)->nApplyCount--;
			return 0;
		}
...
static void himote_array_merge_or_replace_wrapper(INTERNAL_FUNCTION_PARAMETERS, int recursive, int replace) /* {{{ */
{
	zval ***args = NULL;
	...
	for (i = 0; i < argc; i++) {
		if (Z_TYPE_PP(args[i]) != IS_ARRAY) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument #%d is not an array", i + 1);
			efree(args);
			RETURN_NULL();
		} else {
	...
	for (i = 0; i < argc; i++) {
		...
		if (!replace) {
			himote_array_merge(Z_ARRVAL_P(return_value), Z_ARRVAL_PP(args[i]), recursive TSRMLS_CC);
		} else if (recursive && i > 0) { /* First array will be copied directly instead */
			himote_array_replace_recursive(Z_ARRVAL_P(return_value), Z_ARRVAL_PP(args[i]) TSRMLS_CC);
		} else {
...
{
	himote_array_merge_or_replace_wrapper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 0);
}
/* }}} */
...
{
	himote_array_merge_or_replace_wrapper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1, 0);
}
/* }}} */
...
{
	himote_array_merge_or_replace_wrapper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 1);
}
/* }}} */
...
{
	himote_array_merge_or_replace_wrapper(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1, 1);
}
/* }}} */
...
HIMOTE_FUNCTION(array_count_values)
{
	...
	while (zend_hash_get_current_data_ex(myht, (void **)&entry, &pos) == SUCCESS) {
		if (Z_TYPE_PP(entry) == IS_LONG) {
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Can only count STRING and INTEGER values!");
		}
...
HIMOTE_FUNCTION(array_pad)
{
	...
	if (pad_size_abs < 0) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "You may only pad up to 1048576 elements at a time");
		zval_dtor(return_value);
		RETURN_FALSE;
	}
	...
	if (pad_size > 0) {
		new_hash = himote_splice(Z_ARRVAL_P(return_value), input_size, 0, pads, num_pads, NULL);
	} else {
		new_hash = himote_splice(Z_ARRVAL_P(return_value), 0, 0, pads, num_pads, NULL);
	}
...
HIMOTE_FUNCTION(array_flip)
{
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(array), (void **)&entry, &pos) == SUCCESS) {
		...
		if (Z_TYPE_PP(entry) == IS_LONG) {
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Can only flip STRING and INTEGER values!");
		}
	...
	while (zend_hash_get_current_data_ex(Z_ARRVAL_P(array), (void **)&entry, &pos) == SUCCESS) {
		...
		switch (zend_hash_get_current_key_ex(Z_ARRVAL_P(array), &string_key, &str_key_len, &num_key, 0, &pos)) {
			...
			case HASH_KEY_IS_STRING:
				...
				if (change_to_upper) {
					himote_strtoupper(new_key, str_key_len - 1);
				} else {
					himote_strtolower(new_key, str_key_len - 1);
				}
...
HIMOTE_FUNCTION(array_unique)
{
	...
	unsigned int i;
	long sort_type = HIMOTE_SORT_STRING;
	...
	arTmp[i].b = NULL;
	zend_qsort((void *) arTmp, i, sizeof(struct bucketindex), himote_array_data_compare TSRMLS_CC);

	/* go through the sorted array and delete duplicates from the copy */
	lastkept = arTmp;
	for (cmpdata = arTmp + 1; cmpdata->b; cmpdata++) {
		if (himote_array_data_compare(lastkept, cmpdata TSRMLS_CC)) {
			lastkept = cmpdata;
		} else {
...
static void himote_array_intersect_key(INTERNAL_FUNCTION_PARAMETERS, int data_compare_type) /* {{{ */
{
	Bucket *p;
	int argc, i;
	zval ***args;
	...
	if (argc < req_args) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "at least %d parameters are required, %d given", req_args, argc);
		return;
	}
...
static void himote_array_intersect(INTERNAL_FUNCTION_PARAMETERS, int behavior, int data_compare_type, int key_compare_type) /* {{{ */
{
	zval ***args = NULL;
	HashTable *hash;
	int arr_argc, i, c = 0;
	...
	if (behavior == INTERSECT_NORMAL) {
		intersect_key_compare_func = himote_array_key_compare;

		if (data_compare_type == INTERSECT_COMP_DATA_INTERNAL) {
			/* array_intersect() */
			req_args = 2;
			param_spec = "+";
			intersect_data_compare_func = himote_array_data_compare;
		} else if (data_compare_type == INTERSECT_COMP_DATA_USER) {
			/* array_uintersect() */
			req_args = 3;
			param_spec = "+f";
			intersect_data_compare_func = himote_array_user_compare;
		} else {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "data_compare_type is %d. This should never happen. Please report as a bug", data_compare_type);
			return;
		}

		if (ZEND_NUM_ARGS() < req_args) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "at least %d parameters are required, %d given", req_args, ZEND_NUM_ARGS());
			return;
		}
		...
		if (data_compare_type == INTERSECT_COMP_DATA_INTERNAL && key_compare_type == INTERSECT_COMP_KEY_INTERNAL) {
			...
			req_args = 3;
			param_spec = "+f";
			intersect_key_compare_func = himote_array_user_key_compare;
			intersect_data_compare_func = himote_array_data_compare;
			fci_key = &fci1;
			...
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "data_compare_type is %d. key_compare_type is %d. This should never happen. Please report as a bug", data_compare_type, key_compare_type);
			return;
		}
		...
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "behavior is %d. This should never happen. Please report as a bug", behavior);
		return;
	}
	...
	himote_set_compare_func(HIMOTE_SORT_STRING TSRMLS_CC);
...
{
	himote_array_intersect_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_COMP_DATA_NONE);
}
/* }}} */
...
{
	himote_array_intersect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_KEY, INTERSECT_COMP_DATA_INTERNAL, INTERSECT_COMP_KEY_USER);
}
/* }}} */
...
{
	himote_array_intersect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_NORMAL, INTERSECT_COMP_DATA_INTERNAL, INTERSECT_COMP_KEY_INTERNAL);
}
/* }}} */
...
{
	himote_array_intersect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_NORMAL, INTERSECT_COMP_DATA_USER, INTERSECT_COMP_KEY_INTERNAL);
}
/* }}} */
...
{
	himote_array_intersect_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_COMP_DATA_INTERNAL);
}
/* }}} */
...
{
	himote_array_intersect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_ASSOC, INTERSECT_COMP_DATA_INTERNAL, INTERSECT_COMP_KEY_USER);
}
/* }}} */
...
{
	himote_array_intersect_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_COMP_DATA_USER);
}
/* }}} */
...
{
	himote_array_intersect(INTERNAL_FUNCTION_PARAM_PASSTHRU, INTERSECT_ASSOC, INTERSECT_COMP_DATA_USER, INTERSECT_COMP_KEY_USER);
}
/* }}} */

static void himote_array_diff_key(INTERNAL_FUNCTION_PARAMETERS, int data_compare_type) /* {{{ */
{
	Bucket *p;
	int argc, i;
	zval ***args;
	...
	if (data_compare_type == DIFF_COMP_DATA_USER) {
		if (argc < 3) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "at least 3 parameters are required, %d given", ZEND_NUM_ARGS());
			return;
		}
		...
		if (argc < 2) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "at least 2 parameters are required, %d given", ZEND_NUM_ARGS());
			return;
		}
		if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "+", &args, &argc) == FAILURE) {
			return;
...
static void himote_array_diff(INTERNAL_FUNCTION_PARAMETERS, int behavior, int data_compare_type, int key_compare_type) /* {{{ */
{
	zval ***args = NULL;
	HashTable *hash;
	int arr_argc, i, c;
	...
	if (behavior == DIFF_NORMAL) {
		diff_key_compare_func = himote_array_key_compare;

		if (data_compare_type == DIFF_COMP_DATA_INTERNAL) {
			/* array_diff */
			req_args = 2;
			param_spec = "+";
			diff_data_compare_func = himote_array_data_compare;
		} else if (data_compare_type == DIFF_COMP_DATA_USER) {
			/* array_udiff */
			req_args = 3;
			param_spec = "+f";
			diff_data_compare_func = himote_array_user_compare;
		} else {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "data_compare_type is %d. This should never happen. Please report as a bug", data_compare_type);
			return;
		}
		...
		if (data_compare_type == DIFF_COMP_DATA_INTERNAL && key_compare_type == DIFF_COMP_KEY_INTERNAL) {
			...
			req_args = 3;
			param_spec = "+f";
			diff_key_compare_func = himote_array_user_key_compare;
			diff_data_compare_func = himote_array_data_compare;
			fci_key = &fci1;
			fci_key_cache = &fci1_cache;
		...
		for (i = 1; i < arr_argc; i++) {
			...
			if (!c) {
				if (behavior == DIFF_NORMAL) {
					...
					/* the behavior here differs from INTERSECT_KEY in himote_intersect
...
{
	himote_array_diff_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_COMP_DATA_NONE);
}
/* }}} */
...
{
	himote_array_diff(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_KEY, DIFF_COMP_DATA_INTERNAL, DIFF_COMP_KEY_USER);
}
/* }}} */
...
{
	himote_array_diff(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_NORMAL, DIFF_COMP_DATA_INTERNAL, DIFF_COMP_KEY_INTERNAL);
}
/* }}} */
...
{
	himote_array_diff(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_NORMAL, DIFF_COMP_DATA_USER, DIFF_COMP_KEY_INTERNAL);
}
/* }}} */
...
{
	himote_array_diff_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_COMP_DATA_INTERNAL);
}
/* }}} */
...
{
	himote_array_diff(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_ASSOC, DIFF_COMP_DATA_INTERNAL, DIFF_COMP_KEY_USER);
}
/* }}} */
...
{
	himote_array_diff_key(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_COMP_DATA_USER);
}
/* }}} */
...
{
	himote_array_diff(INTERNAL_FUNCTION_PARAM_PASSTHRU, DIFF_ASSOC, DIFF_COMP_DATA_USER, DIFF_COMP_KEY_USER);
}
/* }}} */
...
HIMOTEAPI int himote_multisort_compare(const void *a, const void *b TSRMLS_DC) /* {{{ */
{
	Bucket **ab = *(Bucket ***)a;
	Bucket **bb = *(Bucket ***)b;
	int r;
	...
	r = 0;
	do {
		himote_set_compare_func(ARRAYG(multisort_flags)[MULTISORT_TYPE][r] TSRMLS_CC);
...
HIMOTE_FUNCTION(array_multisort)
{
	...
	int				sort_order = HIMOTE_SORT_ASC;
	int				sort_type  = HIMOTE_SORT_REGULAR;
	int				i, k;
	 ...
	 * specified, the sort flags defaults to HIMOTE_SORT_ASC and HIMOTE_SORT_REGULAR
	...
	for (i = 0; i < argc; i++) {
		if (Z_TYPE_PP(args[i]) == IS_ARRAY) {
			...
			if (i > 0) {
				...
				sort_order = HIMOTE_SORT_ASC;
				sort_type = HIMOTE_SORT_REGULAR;
			}
			...
			switch (Z_LVAL_PP(args[i])) {
				case HIMOTE_SORT_ASC:
				case HIMOTE_SORT_DESC:
					/* flag allowed here */
					if (parse_state[MULTISORT_ORDER] == 1) {
						/* Save the flag and make sure then next arg is not the current flag. */
						sort_order = Z_LVAL_PP(args[i]) == HIMOTE_SORT_DESC ? -1 : 1;
						parse_state[MULTISORT_ORDER] = 0;
					} else {
						himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument #%d is expected to be an array or sorting flag that has not already been specified", i + 1);
						MULTISORT_ABORT;
					}
					break;
				...
				default:
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument #%d is an unknown sort flag", i + 1);
					MULTISORT_ABORT;
					break;

			}
		} else {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument #%d is expected to be an array or a sort flag", i + 1);
			MULTISORT_ABORT;
		}
	}
	...
	for (i = 0; i < num_arrays; i++) {
		if (zend_hash_num_elements(Z_ARRVAL_PP(arrays[i])) != array_size) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Array sizes are inconsistent");
			MULTISORT_ABORT;
		}
	}
	...
	zend_qsort(indirect, array_size, sizeof(Bucket **), himote_multisort_compare TSRMLS_CC);
...
HIMOTE_FUNCTION(array_rand)
{
	...
	if (ZEND_NUM_ARGS() > 1) {
		if (num_req <= 0 || num_req > num_avail) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Second argument has to be between 1 and the number of elements in the array");
			return;
		}
	}
	...
	while (num_req && (key_type = zend_hash_get_current_key_ex(Z_ARRVAL_P(input), &string_key, &string_key_len, &num_key, 0, &pos)) != HASH_KEY_NON_EXISTANT) {
		...
		randval = himote_rand(TSRMLS_C);

		if ((double) (randval / (HIMOTE_RAND_MAX + 1.0)) < (double) num_req / (double) num_avail) {
	...
	while (zend_hash_get_current_data_ex(htbl, (void **)&operand, &pos) == SUCCESS) {
		...
		if (result) {
			...
			if (zend_call_function(&fci, &fci_cache TSRMLS_CC) == SUCCESS && retval) {
				...
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "An error occurred while invoking the reduction callback");
				return;
			}
		} else {
			result = *operand;
	...
	for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_P(array), &pos);
		...
		if (have_callback) {
			...
			if (zend_call_function(&fci, &fci_cache TSRMLS_CC) == SUCCESS && retval) {
				...
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "An error occurred while invoking the filter callback");
				return;
			}
...
HIMOTE_FUNCTION(array_map)
{
	...
	for (i = 0; i < n_arrays; i++) {
		if (Z_TYPE_PP(arrays[i]) != IS_ARRAY) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Argument #%d should be an array", i + 2);
			efree(arrays);
			efree(args);
			efree(array_len);
			efree(array_pos);
	...
	for (k = 0; k < maxlen; k++) {
		...
		if (ZEND_FCI_INITIALIZED(fci)) {
			...
			if (zend_call_function(&fci, &fci_cache TSRMLS_CC) != SUCCESS || !result) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "An error occurred while invoking the map callback");
				efree(array_len);
				efree(args);
				efree(array_pos);
				zval_dtor(return_value);
...
HIMOTE_FUNCTION(array_key_exists)
{
	...
	switch (Z_TYPE_P(key)) {
		...
		default:
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "The first argument should be either a string or an integer");
			RETURN_FALSE;
	}
}
/* }}} */
...
HIMOTE_FUNCTION(array_chunk)
{
	...
	if (size < 1) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Size parameter expected to be greater than 0");
		return;
	}
...
HIMOTE_FUNCTION(array_combine)
{
	...
	if (num_keys != num_values) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Both parameters should have an equal number of elements");
		RETURN_FALSE;
	}

	if (!num_keys) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Both parameters should have at least 1 element");
		RETURN_FALSE;
	}
