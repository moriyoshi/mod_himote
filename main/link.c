...
HIMOTE_FUNCTION(readlink)
{
	...
	if (himote_check_open_basedir(link TSRMLS_CC)) {
		RETURN_FALSE;
	}

	ret = himote_sys_readlink(link, buff, MAXPATHLEN-1);
...
HIMOTE_FUNCTION(linkinfo)
{
	...
	dir_len = himote_dirname(dirname, link_len);

	if (himote_check_open_basedir(dirname TSRMLS_CC)) {
		efree(dirname);
		RETURN_FALSE;
	}
...
HIMOTE_FUNCTION(symlink)
{
	...
	if (himote_check_open_basedir(dest_p TSRMLS_CC)) {
		RETURN_FALSE;
	}

	if (himote_check_open_basedir(source_p TSRMLS_CC)) {
		RETURN_FALSE;
	}
