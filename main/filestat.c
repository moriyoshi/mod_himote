...
static int himote_disk_total_space(char *path, double *space TSRMLS_DC) /* {{{ */
#if defined(WINDOWS) /* {{{ */
{
	double bytestotal = 0;
	HINSTANCE kernel32;
	...
	if (kernel32) {
		...
		if (gdfse) {
			...
			if (func(path,
				...
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%s", himote_win_err());
				return FAILURE;
			}
		...
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to load kernel32.dll");
		return FAILURE;
	}
...
HIMOTE_FUNCTION(disk_total_space)
{
	...
	if (himote_check_open_basedir(path TSRMLS_CC)) {
		RETURN_FALSE;
	}

	if (himote_disk_total_space(path, &bytestotal TSRMLS_CC) == SUCCESS) {
...
static int himote_disk_free_space(char *path, double *space TSRMLS_DC) /* {{{ */
#if defined(WINDOWS) /* {{{ */
{
	double bytesfree = 0;
...
HIMOTE_FUNCTION(disk_free_space)
{
	...
	if (himote_disk_free_space(path, &bytesfree TSRMLS_CC) == SUCCESS) {
...
static void himote_do_chgrp(INTERNAL_FUNCTION_PARAMETERS, int do_lchgrp) /* {{{ */
{
	char *filename;
	int filename_len;
	zval *group;
	...
	if (Z_TYPE_P(group) == IS_LONG) {
		...
		if (getgrnam_r(Z_STRVAL_P(group), &gr, grbuf, grbuflen, &retgrptr) != 0 || retgrptr == NULL) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find gid for %s", Z_STRVAL_P(group));
			efree(grbuf);
			RETURN_FALSE;
		}
		efree(grbuf);
		...
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "parameter 2 should be string or integer, %s given", zend_zval_type_name(group));
		RETURN_FALSE;
	}
...
{
#if !defined(WINDOWS)
	himote_do_chgrp(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
#else
	RETURN_FALSE;
#endif
}
...
{
# if !defined(WINDOWS)
	himote_do_chgrp(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
# else
	RETURN_FALSE;
# endif
}
...
#if !defined(WINDOWS) && !defined(NETWARE)
static void himote_do_chown(INTERNAL_FUNCTION_PARAMETERS, int do_lchown) /* {{{ */
{
	char *filename;
	int filename_len;
	zval *user;
	...
	if (Z_TYPE_P(user) == IS_LONG) {
		...
		if (getpwnam_r(Z_STRVAL_P(user), &pw, pwbuf, pwbuflen, &retpwptr) != 0 || retpwptr == NULL) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to find uid for %s", Z_STRVAL_P(user));
			efree(pwbuf);
			RETURN_FALSE;
		}
		efree(pwbuf);
		...
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "parameter 2 should be string or integer, %s given", zend_zval_type_name(user));
		RETURN_FALSE;
	}
...
#if !defined(WINDOWS)
	RETVAL_TRUE;
	himote_do_chown(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
#else
	RETURN_FALSE;
#endif
}
...
# if !defined(WINDOWS)
	RETVAL_TRUE;
	himote_do_chown(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
# else
	RETURN_FALSE;
# endif
}
...
HIMOTE_FUNCTION(touch)
{
	...
	if (VCWD_ACCESS(filename, F_OK) != 0) {
		...
		if (file == NULL) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Unable to create file %s because %s", filename, strerror(errno));
			RETURN_FALSE;
		}
		fclose(file);
	}
	...
	if (ret == -1) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Utime failed: %s", strerror(errno));
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
HIMOTEAPI void himote_clear_stat_cache(zend_bool clear_realpath_cache, const char *filename, int filename_len TSRMLS_DC)
{
	 ...
	 * in this directory, as shown by lstat_stat_variation9.himotet) */
...
HIMOTE_FUNCTION(clearstatcache)
{
	...
	himote_clear_stat_cache(clear_realpath_cache, filename, filename_len TSRMLS_CC);
}
/* }}} */
...
HIMOTEAPI void himote_stat(const char *filename, himote_stat_len filename_length, int type, zval *return_value TSRMLS_DC)
{
	...
	if ((wrapper = himote_stream_locate_url_wrapper(filename, &local, 0 TSRMLS_CC)) == &himote_plain_files_wrapper) {
		if (himote_check_open_basedir(local TSRMLS_CC)) {
			RETURN_FALSE;
		}
	}
	...
	if (IS_LINK_OPERATION(type)) {
		flags |= HIMOTE_STREAM_URL_STAT_LINK;
	}
	if (IS_EXISTS_CHECK(type)) {
		flags |= HIMOTE_STREAM_URL_STAT_QUIET;
	}

	if (himote_stream_stat_path_ex((char *)filename, flags, &ssb, NULL)) {
		/* Error Occured */
		if (!IS_EXISTS_CHECK(type)) {
			himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%sstat failed for %s", IS_LINK_OPERATION(type) ? "L" : "", filename);
		}
		RETURN_FALSE;
	}
	...
	case FS_TYPE:
		...
		himote_error_docref(NULL TSRMLS_CC, E_NOTICE, "Unknown file type (%d)", ssb.sb.st_mode&S_IFMT);
	...
	himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Didn't understand stat call");
	RETURN_FALSE;
}
/* }}} */
...
void name(INTERNAL_FUNCTION_PARAMETERS) { \
	...
	\
	himote_stat(filename, (himote_stat_len) filename_len, funcnum, return_value TSRMLS_CC); \
}
/* }}} */

/* {{{ proto int fileperms(string filename)
   Get file permissions */
FileFunction(HIMOTE_FN(fileperms), FS_PERMS)
/* }}} */

/* {{{ proto int fileinode(string filename)
   Get file inode */
FileFunction(HIMOTE_FN(fileinode), FS_INODE)
/* }}} */

/* {{{ proto int filesize(string filename)
   Get file size */
FileFunction(HIMOTE_FN(filesize), FS_SIZE)
/* }}} */

/* {{{ proto int fileowner(string filename)
   Get file owner */
FileFunction(HIMOTE_FN(fileowner), FS_OWNER)
/* }}} */

/* {{{ proto int filegroup(string filename)
   Get file group */
FileFunction(HIMOTE_FN(filegroup), FS_GROUP)
/* }}} */

/* {{{ proto int fileatime(string filename)
   Get last access time of file */
FileFunction(HIMOTE_FN(fileatime), FS_ATIME)
/* }}} */

/* {{{ proto int filemtime(string filename)
   Get last modification time of file */
FileFunction(HIMOTE_FN(filemtime), FS_MTIME)
/* }}} */

/* {{{ proto int filectime(string filename)
   Get inode modification time of file */
FileFunction(HIMOTE_FN(filectime), FS_CTIME)
/* }}} */

/* {{{ proto string filetype(string filename)
   Get file type */
FileFunction(HIMOTE_FN(filetype), FS_TYPE)
/* }}} */

/* {{{ proto bool is_writable(string filename)
   Returns true if file can be written */
FileFunction(HIMOTE_FN(is_writable), FS_IS_W)
/* }}} */

/* {{{ proto bool is_readable(string filename)
   Returns true if file can be read */
FileFunction(HIMOTE_FN(is_readable), FS_IS_R)
/* }}} */

/* {{{ proto bool is_executable(string filename)
   Returns true if file is executable */
FileFunction(HIMOTE_FN(is_executable), FS_IS_X)
/* }}} */

/* {{{ proto bool is_file(string filename)
   Returns true if file is a regular file */
FileFunction(HIMOTE_FN(is_file), FS_IS_FILE)
/* }}} */

/* {{{ proto bool is_dir(string filename)
   Returns true if file is directory */
FileFunction(HIMOTE_FN(is_dir), FS_IS_DIR)
/* }}} */

/* {{{ proto bool is_link(string filename)
   Returns true if file is symbolic link */
FileFunction(HIMOTE_FN(is_link), FS_IS_LINK)
/* }}} */

/* {{{ proto bool file_exists(string filename)
   Returns true if filename exists */
FileFunction(HIMOTE_FN(file_exists), FS_EXISTS)
/* }}} */

/* {{{ proto array lstat(string filename)
   Give information about a file or symbolic link */
FileFunction(himote_if_lstat, FS_LSTAT)
/* }}} */

/* {{{ proto array stat(string filename)
   Give information about a file */
FileFunction(himote_if_stat, FS_STAT)
/* }}} */
