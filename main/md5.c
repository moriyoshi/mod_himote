...
HIMOTE_NAMED_FUNCTION(himote_if_md5)
{
	...
	md5str[0] = '\0';
	HIMOTE_MD5Init(&context);
	HIMOTE_MD5Update(&context, arg, arg_len);
	HIMOTE_MD5Final(digest, &context);
	if (raw_output) {
...
HIMOTE_NAMED_FUNCTION(himote_if_md5_file)
{
	...
	while ((n = himote_stream_read(stream, buf, sizeof(buf))) > 0) {
		HIMOTE_MD5Update(&context, buf, n);
	}
...
# define SET(n) \
	(ctx->block[(n)] = \
	(himote_uint32)ptr[(n) * 4] | \
	((himote_uint32)ptr[(n) * 4 + 1] << 8) | \
	((himote_uint32)ptr[(n) * 4 + 2] << 16) | \
	((himote_uint32)ptr[(n) * 4 + 3] << 24))
# define GET(n) \
	(ctx->block[(n)])
#endif
 ...
 */
static const void *body(HIMOTE_MD5_CTX *ctx, const void *data, size_t size)
{
	const unsigned char *ptr;
	himote_uint32 a, b, c, d;
	himote_uint32 saved_a, saved_b, saved_c, saved_d;
...
{
	himote_uint32 saved_lo;
	himote_uint32 used, free;
