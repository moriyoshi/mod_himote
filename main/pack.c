...
#define INC_OUTPUTPOS(a,b) \
	if ((a) < 0 || ((INT_MAX - outputpos)/((int)b)) < (a)) { \
		...
		efree(formatcodes);	\
		efree(formatargs);	\
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: integer overflow in format string", code); \
		RETURN_FALSE; \
	} \
	outputpos += (a)*(b);
...
/* {{{ himote_pack
 */
static void himote_pack(zval **val, int size, int *map, char *output)
{
	int i;
	char *v;
...
HIMOTE_FUNCTION(pack)
{
	...
	for (i = 0; i < formatlen; formatcount++) {
		...
		switch ((int) code) {
			...
			case '@':
				if (arg < 0) {
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: '*' ignored", code);
					arg = 1;
				}
				break;
			...
			case 'H':
				if (currentarg >= num_args) {
					...
					efree(formatcodes);
					efree(formatargs);
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: not enough arguments", code);
					RETURN_FALSE;
				}
			...
			case 'd': 
				...
				if (currentarg > num_args) {
					...
					efree(formatcodes);
					efree(formatargs);
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: too few arguments", code);
					RETURN_FALSE;
				}
				break;
			...
			default:
				...
				efree(formatcodes);
				efree(formatargs);
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: unknown format code", code);
				RETURN_FALSE;
		}
	...
	if (currentarg < num_args) {
		himote_error_docref(NULL TSRMLS_CC, E_WARNING, "%d arguments unused", (num_args - currentarg));
	}
	...
	for (i = 0; i < formatcount; i++) {
		...
		switch ((int) code) {
			...
			case 'X':
				...
				if (outputpos < 0) {
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: outside of string", code);
					outputpos = 0;
				}
				break;
		...
		switch ((int) code) {
			...
			case 'H': {
				...
				if(arg > Z_STRLEN_PP(val)) {
					himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: not enough characters in string", code);
					arg = Z_STRLEN_PP(val);
				}
				...
				while (arg-- > 0) {
					...
					if (n >= '0' && n <= '9') {
						...
						himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: illegal hex digit %c", code, n);
						n = 0;
					}
			...
			case 'C':
				while (arg-- > 0) {
					himote_pack(argv[currentarg++], 1, byte_map, &output[outputpos]);
					outputpos++;
				}
				break;
			...
			case 'v': {
				...
				while (arg-- > 0) {
					himote_pack(argv[currentarg++], 2, map, &output[outputpos]);
					outputpos += 2;
				}
				break;
			}
			...
			case 'I': 
				while (arg-- > 0) {
					himote_pack(argv[currentarg++], sizeof(int), int_map, &output[outputpos]);
			...
			case 'V': {
				...
				while (arg-- > 0) {
					himote_pack(argv[currentarg++], 4, map, &output[outputpos]);
					outputpos += 4;
				}
				break;
			}
...
/* {{{ himote_unpack
 */
static long himote_unpack(char *data, int size, int issigned, int *map)
{
	long result;
...
HIMOTE_FUNCTION(unpack)
{
	...
	while (formatlen-- > 0) {
		...
		switch ((int) type) {
			...
			default:
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Invalid format type %c", type);
		...
		for (i = 0; i != arg; i++ ) {
			...
			if (size != 0 && size != -1 && INT_MAX - size + 1 < inputpos) {
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: integer overflow", type);
				inputpos = 0;
			}
			...
			if ((inputpos + size) <= inputlen) {
				switch ((int) type) {
					...
					case 'C': {
						...
						long v = himote_unpack(&input[inputpos], 1, issigned, byte_map);
					...
					case 'v': {
						...
						v = himote_unpack(&input[inputpos], 2, issigned, map);
						add_assoc_long(return_value, n, v);
						break;
					}
					...
					case 'I': {
						...
						v = himote_unpack(&input[inputpos], sizeof(int), issigned, int_map);
						add_assoc_long(return_value, n, v);
						break;
					}
					...
					case 'V': {
						...
						v |= himote_unpack(&input[inputpos], 4, issigned, map);
						if (sizeof(long) > 4) {
 							if (type == 'l') {
								v = (signed int) v; 
							} else {
					...
					case 'X':
						if (inputpos < size) {
							...
							if (arg >= 0) {
								himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: outside of string", type);
							}
						}
						break;
				...
				himote_error_docref(NULL TSRMLS_CC, E_WARNING, "Type %c: not enough input, need %d, have %d", type, size, inputlen - inputpos);
				zval_dtor(return_value);
				RETURN_FALSE;
			}
		}
