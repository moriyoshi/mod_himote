...
#ifdef ZTS
MUTEX_T himote_crypt_extended_init_lock;
#endif
...
{
#ifdef ZTS
	himote_crypt_extended_init_lock = tsrm_mutex_alloc();
#endif
}
...
{
#ifdef ZTS
	tsrm_mutex_free(himote_crypt_extended_init_lock);
#endif
}
...
#ifdef ZTS
	tsrm_mutex_lock(himote_crypt_extended_init_lock);
#endif
...
#ifdef ZTS
	tsrm_mutex_unlock(himote_crypt_extended_init_lock);
#endif
}
...
#if HIMOTE_WIN32
char * himote_md5_crypt_r(const char *pw, const char *salt, char *out) {
...
char * himote_md5_crypt_r(const char *pw, const char *salt, char *out)
{
	...
	/* The password first, since that is what is most unknown */
	HIMOTE_MD5Update(&ctx, (const unsigned char *)pw, pwl);

	/* Then our magic string */
	HIMOTE_MD5Update(&ctx, (const unsigned char *)MD5_MAGIC, MD5_MAGIC_LEN);

	/* Then the raw salt */
	HIMOTE_MD5Update(&ctx, (const unsigned char *)sp, sl);

	/* Then just as many characters of the MD5(pw,salt,pw) */
	HIMOTE_MD5Init(&ctx1);
	HIMOTE_MD5Update(&ctx1, (const unsigned char *)pw, pwl);
	HIMOTE_MD5Update(&ctx1, (const unsigned char *)sp, sl);
	HIMOTE_MD5Update(&ctx1, (const unsigned char *)pw, pwl);
	HIMOTE_MD5Final(final, &ctx1);

	for (pl = pwl; pl > 0; pl -= 16)
		HIMOTE_MD5Update(&ctx, final, (unsigned int)(pl > 16 ? 16 : pl));
	...
	for (i = pwl; i != 0; i >>= 1)
		if ((i & 1) != 0)
		    HIMOTE_MD5Update(&ctx, final, 1);
		else
		    HIMOTE_MD5Update(&ctx, (const unsigned char *)pw, 1);
	...
	for (i = 0; i < 1000; i++) {
		...
		if ((i & 1) != 0)
			...
			HIMOTE_MD5Update(&ctx1, final, 16);
