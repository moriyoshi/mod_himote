...
HIMOTEAPI int (*himote_rfc1867_callback)(unsigned int event, void *event_data, void **extra TSRMLS_DC) = NULL;
...
void himote_rfc1867_register_constants(TSRMLS_D) /* {{{ */
{
...
static void safe_himote_register_variable(char *var, char *strval, int val_len, zval *track_vars_array, zend_bool override_protection TSRMLS_DC) /* {{{ */
{
	if (override_protection || !is_protected_variable(var TSRMLS_CC)) {
		himote_register_variable_safe(var, strval, val_len, track_vars_array TSRMLS_CC);
	}
}
/* }}} */

static void safe_himote_register_variable_ex(char *var, zval *val, zval *track_vars_array, zend_bool override_protection TSRMLS_DC) /* {{{ */
{
	if (override_protection || !is_protected_variable(var TSRMLS_CC)) {
		himote_register_variable_ex(var, val, track_vars_array TSRMLS_CC);
	}
}
/* }}} */
...
static void register_http_post_files_variable(char *strvar, char *val, zval *http_post_files, zend_bool override_protection TSRMLS_DC) /* {{{ */
{
	...
	safe_himote_register_variable(strvar, val, strlen(val), http_post_files, override_protection TSRMLS_CC);
...
static void register_http_post_files_variable_ex(char *var, zval *val, zval *http_post_files, zend_bool override_protection TSRMLS_DC) /* {{{ */
{
	...
	PG(register_globals) = 0;
	safe_himote_register_variable_ex(var, val, http_post_files, override_protection TSRMLS_CC);
	PG(register_globals) = register_globals;
}
/* }}} */
...
/* Free header entry */
static void himote_free_hdr_entry(mime_header_entry *h)
{
	if (h->key) {
		efree(h->key);
	}
...
static char *himote_mime_get_hdr_value(zend_llist header, char *key)
{
...
static char *himote_ap_getword(char **line, char stop)
{
	char *pos = *line, quote;
	char *res;
...
static char *himote_ap_getword_conf(char **line TSRMLS_DC)
{
 ...
 */
static void *himote_ap_memstr(char *haystack, int haystacklen, char *needle, int needlen, int partial)
{
	int len = haystacklen;
	char *ptr = haystack;
...
static int multipart_buffer_read(multipart_buffer *self, char *buf, int bytes, int *end TSRMLS_DC)
{
	...
	if ((bound = himote_ap_memstr(self->buf_begin, self->bytes_in_buffer, self->boundary_next, self->boundary_next_len, 1))) {
		max = bound - self->buf_begin;
		if (end && himote_ap_memstr(self->buf_begin, self->bytes_in_buffer, self->boundary_next, self->boundary_next_len, 0)) {
			*end = 1;
		}
	} else {
...
SAPI_API SAPI_POST_HANDLER_FUNC(rfc1867_post_handler) /* {{{ */
{
	...
	if (!boundary) {
		...
		himote_strtolower(content_type_lcase, content_type_len);
	...
	zend_llist_init(&header, sizeof(mime_header_entry), (llist_dtor_func_t) himote_free_hdr_entry, 0);

	if (himote_rfc1867_callback != NULL) {
		multipart_event_start event_start;

		event_start.content_length = SG(request_info).content_length;
		if (himote_rfc1867_callback(MULTIPART_EVENT_START, &event_start, &event_extra_data TSRMLS_CC) == FAILURE) {
	...
	while (!multipart_buffer_eof(mbuff TSRMLS_CC))
	{
		...
		if ((cd = himote_mime_get_hdr_value(header, "Content-Disposition"))) {
			char *pair = NULL;
			int end = 0;
			...
			while (*cd && (pair = himote_ap_getword(&cd, ';')))
			{
				...
				if (strchr(pair, '=')) {
					key = himote_ap_getword(&pair, '=');

					if (!strcasecmp(key, "name")) {
						if (param) {
							efree(param);
						}
						param = himote_ap_getword_conf(&pair TSRMLS_CC);
					} else if (!strcasecmp(key, "filename")) {
						if (filename) {
							efree(filename);
						}
						filename = himote_ap_getword_conf(&pair TSRMLS_CC);
					}
				}
				if (key) {
					efree(key);
			...
			if (!filename && param) {
				...
				if (++count <= PG(max_input_vars) && sapi_module.input_filter(PARSE_POST, param, &value, value_len, &new_val_len TSRMLS_CC)) {
					if (himote_rfc1867_callback != NULL) {
						...
						if (himote_rfc1867_callback(MULTIPART_EVENT_FORMDATA, &event_formdata, &event_extra_data TSRMLS_CC) == FAILURE) {
							efree(param);
							efree(value);
							continue;
						}
					...
					safe_himote_register_variable(param, value, new_val_len, array_ptr, 0 TSRMLS_CC);
				} else {
					...
					if (himote_rfc1867_callback != NULL) {
						...
						himote_rfc1867_callback(MULTIPART_EVENT_FORMDATA, &event_formdata, &event_extra_data TSRMLS_CC);
					}
				}
			...
			if (!skip_upload && himote_rfc1867_callback != NULL) {
				multipart_event_file_start event_file_start;

				event_file_start.post_bytes_processed = SG(read_post_bytes);
				event_file_start.name = param;
				event_file_start.filename = &filename;
				if (himote_rfc1867_callback(MULTIPART_EVENT_FILE_START, &event_file_start, &event_extra_data TSRMLS_CC) == FAILURE) {
					temp_filename = "";
					efree(param);
					efree(filename);
					continue;
			...
			if (!cancel_upload) {
				...
				{
#endif
					fd = himote_open_temporary_fd_ex(PG(upload_tmp_dir), "himote", &temp_filename, 1 TSRMLS_CC);
					upload_cnt--;
					if (fd == -1) {
			...
			while (!cancel_upload && (blen > 0))
			{
				if (himote_rfc1867_callback != NULL) {
					...
					if (himote_rfc1867_callback(MULTIPART_EVENT_FILE_DATA, &event_file_data, &event_extra_data TSRMLS_CC) == FAILURE) {
			...
			if (himote_rfc1867_callback != NULL) {
				...
				if (himote_rfc1867_callback(MULTIPART_EVENT_FILE_END, &event_file_end, &event_extra_data TSRMLS_CC) == FAILURE) {
					cancel_upload = UPLOAD_ERROR_X;
				}
			}
			...
			if (!is_anonymous) {
				if (s && s >= filename) {
					safe_himote_register_variable(lbuf, s+1, strlen(s+1), NULL, 0 TSRMLS_CC);
				} else {
					safe_himote_register_variable(lbuf, filename, strlen(filename), NULL, 0 TSRMLS_CC);
				}
			}
			...
			if (cancel_upload || !(cd = himote_mime_get_hdr_value(header, "Content-Type"))) {
				cd = "";
			} else {
				/* fix for Opera 6.01 */
				s = strchr(cd, ';');
			...
			if (!is_anonymous) {
				safe_himote_register_variable(lbuf, cd, strlen(cd), NULL, 0 TSRMLS_CC);
			}
...

			{
				...
				if (!is_anonymous) {
					...
					safe_himote_register_variable_ex(param, &zfilename, NULL, 1 TSRMLS_CC);
				}
...

			{
				...
				if (!is_anonymous) {
					safe_himote_register_variable_ex(lbuf, &file_size, NULL, 0 TSRMLS_CC);
				}
...
fileupload_done:
	if (himote_rfc1867_callback != NULL) {
		...
		himote_rfc1867_callback(MULTIPART_EVENT_END, &event_end, &event_extra_data TSRMLS_CC);
	}
