...
/* output functions */
static int himote_b_body_write(const char *str, uint str_length TSRMLS_DC);

static int himote_ob_init(uint initial_size, uint block_size, zval *output_handler, uint chunk_size, zend_bool erase TSRMLS_DC);
static void himote_ob_append(const char *text, uint text_length TSRMLS_DC);
#if 0
static void himote_ob_prepend(const char *text, uint text_length);
#endif

#ifdef ZTS
int output_globals_id;
#else
himote_output_globals output_globals;
#endif

/* {{{ himote_default_output_func */
...
/* {{{ himote_output_init_globals */
static void himote_output_init_globals(himote_output_globals *output_globals_p TSRMLS_DC)
{
	OG(himote_body_write) = himote_default_output_func;
	OG(himote_header_write) = himote_default_output_func;
	OG(implicit_flush) = 0;
...
{
#ifdef ZTS
	ts_allocate_id(&output_globals_id, sizeof(himote_output_globals), (ts_allocate_ctor) himote_output_init_globals, NULL);
#else
	himote_output_init_globals(&output_globals TSRMLS_CC);
#endif
}
/* }}} */

/* {{{ himote_output_activate
 * Initilize output global for activation */
HIMOTEAPI void himote_output_activate(TSRMLS_D)
{
	OG(himote_body_write) = himote_ub_body_write;
	OG(himote_header_write) = sapi_module.ub_write;
...
/* {{{ himote_output_register_constants */
void himote_output_register_constants(TSRMLS_D)
{
	REGISTER_MAIN_LONG_CONSTANT("HIMOTE_OUTPUT_HANDLER_START", HIMOTE_OUTPUT_HANDLER_START, CONST_CS | CONST_PERSISTENT);
	REGISTER_MAIN_LONG_CONSTANT("HIMOTE_OUTPUT_HANDLER_CONT", HIMOTE_OUTPUT_HANDLER_CONT, CONST_CS | CONST_PERSISTENT);
	REGISTER_MAIN_LONG_CONSTANT("HIMOTE_OUTPUT_HANDLER_END", HIMOTE_OUTPUT_HANDLER_END, CONST_CS | CONST_PERSISTENT);
}
/* }}} */

/* {{{ himote_output_set_status
...
{
	return OG(himote_body_write)(str, str_length TSRMLS_CC);
}
/* }}} */
...
HIMOTEAPI int himote_header_write(const char *str, uint str_length TSRMLS_DC)
{
	if (OG(disable_output)) {
		...
		return OG(himote_header_write)(str, str_length TSRMLS_CC);
	}
}
/* }}} */

/* {{{ himote_start_ob_buffer
...
HIMOTEAPI int himote_start_ob_buffer(zval *output_handler, uint chunk_size, zend_bool erase TSRMLS_DC)
{
	...
	if (OG(ob_lock)) {
		if (SG(headers_sent) && !SG(request_info).headers_only) {
			OG(himote_body_write) = himote_ub_body_write_no_header;
		} else {
			OG(himote_body_write) = himote_ub_body_write;
		}
		OG(ob_nesting_level) = 0;
		himote_error_docref("ref.outcontrol" TSRMLS_CC, E_ERROR, "Cannot use output buffering in output buffering display handlers");
		return FAILURE;
	}
	if (chunk_size > 0) {
		if (chunk_size==1) {
	...
	return himote_ob_init(initial_size, block_size, output_handler, chunk_size, erase TSRMLS_CC);
}
/* }}} */

/* {{{ himote_start_ob_buffer_named
 * Start output buffering */
HIMOTEAPI int himote_start_ob_buffer_named(const char *output_handler_name, uint chunk_size, zend_bool erase TSRMLS_DC)
{
	...
	result = himote_start_ob_buffer(output_handler, chunk_size, erase TSRMLS_CC);
...
HIMOTEAPI void himote_end_ob_buffer(zend_bool send_buffer, zend_bool just_flush TSRMLS_DC)
{
	...
	int status;
	himote_ob_buffer *prev_ob_buffer_p=NULL;
	himote_ob_buffer orig_ob_buffer;

	if (OG(ob_nesting_level)==0) {
		return;
	}
	status = 0;
	if (!OG(active_ob_buffer).status & HIMOTE_OUTPUT_HANDLER_START) {
		/* our first call */
		status |= HIMOTE_OUTPUT_HANDLER_START;
	}
	if (just_flush) {
		status |= HIMOTE_OUTPUT_HANDLER_CONT;
	} else {
		status |= HIMOTE_OUTPUT_HANDLER_END;
	}
...
#if 0
 {
	...
	if (send_buffer) {
		...
		OG(himote_body_write)(final_buffer, final_buffer_length TSRMLS_CC);
	}

	if (just_flush) { /* we restored the previous ob, return to the current */
		if (prev_ob_buffer_p) {
			zend_stack_push(&OG(ob_buffers), &OG(active_ob_buffer), sizeof(himote_ob_buffer));
	...
	if (status & HIMOTE_OUTPUT_HANDLER_END) {
	...
	if (!just_flush) {
		...
		OG(active_ob_buffer).status |= HIMOTE_OUTPUT_HANDLER_START;
		OG(himote_body_write) = himote_b_body_write;
	}
...
HIMOTEAPI void himote_end_ob_buffers(zend_bool send_buffer TSRMLS_DC)
{
	while (OG(ob_nesting_level)!=0) {
		himote_end_ob_buffer(send_buffer, 0 TSRMLS_CC);
	}
}
/* }}} */

/* {{{ himote_start_implicit_flush
 */
...
/* {{{ char *himote_get_output_start_filename(TSRMLS_D)
...
/* {{{ char *himote_get_output_start_lineno(TSRMLS_D)
...
/* {{{ himote_ob_set_internal_handler
 */
HIMOTEAPI void himote_ob_set_internal_handler(himote_output_handler_func_t internal_output_handler, uint buffer_size, char *handler_name, zend_bool erase TSRMLS_DC)
{
	if (OG(ob_nesting_level) == 0 || OG(active_ob_buffer).internal_output_handler || strcmp(OG(active_ob_buffer).handler_name, OB_DEFAULT_HANDLER_NAME)) {
		himote_start_ob_buffer(NULL, buffer_size, erase TSRMLS_CC);
	}
...
/* {{{ himote_ob_allocate
 */
static inline void himote_ob_allocate(uint text_length TSRMLS_DC)
{
...
{
	if (himote_ob_handler_used(handler_set TSRMLS_CC)) {
		himote_error_docref("ref.outcontrol" TSRMLS_CC, E_WARNING, "output handler '%s' conflicts with '%s'", handler_new, handler_set);
		return 1;
	}
	return 0;
}
...
/* {{{ himote_ob_init_named
 */
static int himote_ob_init_named(uint initial_size, uint block_size, char *handler_name, zval *output_handler, uint chunk_size, zend_bool erase TSRMLS_DC)
{
	himote_ob_buffer tmp_buf;
...
/* {{{ himote_ob_handler_from_string
 * Create zval output handler from string */
static zval* himote_ob_handler_from_string(const char *handler_name, int len TSRMLS_DC)
{
	zval *output_handler;
	...
	if (output_handler && output_handler->type == IS_STRING) {
		...
		if (handler_len && handler_name[0] != '\0') {
			while ((next_handler_name=strchr(handler_name, ',')) != NULL) {
				...
				handler_zval = himote_ob_handler_from_string(next_handler_name, len TSRMLS_CC);
				result = himote_ob_init_named(initial_size, block_size, next_handler_name, handler_zval, chunk_size, erase TSRMLS_CC);
				if (result != SUCCESS) {
		...
		if (result == SUCCESS) {
			handler_zval = himote_ob_handler_from_string(handler_name, handler_len TSRMLS_CC);
			result = himote_ob_init_named(initial_size, block_size, handler_name, handler_zval, chunk_size, erase TSRMLS_CC);
			if (result != SUCCESS) {
				zval_dtor(handler_zval);
				FREE_ZVAL(handler_zval);
			}
		...
		if (zend_is_callable(output_handler, 0, &handler_name TSRMLS_CC)) {
			...
			result = himote_ob_init_named(initial_size, block_size, handler_name, output_handler, chunk_size, erase TSRMLS_CC);
			efree(handler_name);
		} else {
			efree(handler_name);
			...
			while (zend_hash_get_current_data_ex(Z_ARRVAL_P(output_handler), (void **)&tmp, &pos) == SUCCESS) {
				result = himote_ob_init(initial_size, block_size, *tmp, chunk_size, erase TSRMLS_CC);
				if (result == FAILURE) {
					break;
				}
		...
		if (zend_is_callable(output_handler, 0, &handler_name TSRMLS_CC)) {
			...
			efree(handler_name);
			himote_error_docref(NULL TSRMLS_CC, E_ERROR, "No method name given: use ob_start(array($object,'method')) to specify instance $object and the name of a method of class %s to use as output handler", Z_OBJCE_P(output_handler)->name);
			result = FAILURE;
		}
	} else {
		result = himote_ob_init_named(initial_size, block_size, OB_DEFAULT_HANDLER_NAME, NULL, chunk_size, erase TSRMLS_CC);
	}
	return result;
}
/* }}} */
...
/* {{{ himote_ob_list_each
 */
static int himote_ob_list_each(himote_ob_buffer *ob_buffer, zval *ob_handler_array)
{
...
static int himote_ob_handler_used_each(himote_ob_buffer *ob_buffer, char **handler_name)
{
...
HIMOTEAPI int himote_ob_handler_used(char *handler_name TSRMLS_DC)
{
	...
	if (OG(ob_nesting_level)) {
		...
		if (OG(ob_nesting_level)>1) {
			zend_stack_apply_with_argument(&OG(ob_buffers), ZEND_STACK_APPLY_BOTTOMUP, (int (*)(void *element, void *)) himote_ob_handler_used_each, &tmp);
		}
	}
	return tmp ? 0 : 1;
}
...
/* {{{ himote_ob_append
 */
static inline void himote_ob_append(const char *text, uint text_length TSRMLS_DC)
{
	char *target;
	...
	himote_ob_allocate(text_length TSRMLS_CC);
	...
	if (OG(active_ob_buffer).chunk_size
		...
		himote_end_ob_buffer(1, 1 TSRMLS_CC);
		return;
	}
}
/* }}} */
...
#if 0
static inline void himote_ob_prepend(const char *text, uint text_length)
{
	char *p, *start;
	TSRMLS_FETCH();
	...
	/* himote_ob_allocate() may change OG(ob_buffer), so we can't initialize p&start earlier */
...
static int himote_b_body_write(const char *str, uint str_length TSRMLS_DC)
{
	himote_ob_append(str, str_length TSRMLS_CC);
	return str_length;
}

/* {{{ himote_ub_body_write_no_header
 */
HIMOTEAPI int himote_ub_body_write_no_header(const char *str, uint str_length TSRMLS_DC)
{
	...
	result = OG(himote_header_write)(str, str_length TSRMLS_CC);
...
HIMOTEAPI int himote_ub_body_write(const char *str, uint str_length TSRMLS_DC)
{
	...
	if (SG(request_info).headers_only) {
		...
		himote_header(TSRMLS_C);
		zend_bailout();
	}
	if (himote_header(TSRMLS_C)) {
		...
		OG(himote_body_write) = himote_ub_body_write_no_header;
		result = himote_ub_body_write_no_header(str, str_length TSRMLS_CC);
	}
...
/* {{{ int himote_ob_buffer_status(himote_ob_buffer *ob_buffer, zval *result)
 */
static int himote_ob_buffer_status(himote_ob_buffer *ob_buffer, zval *result)
{
	zval *elem;
	...
	if (ob_buffer->internal_output_handler) {
		add_assoc_long(elem, "type", HIMOTE_OUTPUT_HANDLER_INTERNAL);
		add_assoc_long(elem, "buffer_size", ob_buffer->internal_output_handler_buffer_size);
	} else {
		add_assoc_long(elem, "type", HIMOTE_OUTPUT_HANDLER_USER);
	}
...
HIMOTE_FUNCTION(ob_start)
{
	...
	if (himote_start_ob_buffer(output_handler, chunk_size, erase TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
HIMOTE_FUNCTION(ob_flush)
{
	...
	if (!OG(ob_nesting_level)) {
		himote_error_docref("ref.outcontrol" TSRMLS_CC, E_NOTICE, "failed to flush buffer. No buffer to flush");
		RETURN_FALSE;
	}

	if (!OG(active_ob_buffer).status && !OG(active_ob_buffer).erase) {
		himote_error_docref("ref.outcontrol" TSRMLS_CC, E_NOTICE, "failed to flush buffer %s", OG(active_ob_buffer).handler_name);
		RETURN_FALSE;
	}
...
HIMOTE_FUNCTION(ob_clean)
{
	...
	if (!OG(ob_nesting_level)) {
		himote_error_docref("ref.outcontrol" TSRMLS_CC, E_NOTICE, "failed to delete buffer. No buffer to delete");
		RETURN_FALSE;
	}

	if (!OG(active_ob_buffer).status && !OG(active_ob_buffer).erase) {
		himote_error_docref("ref.outcontrol" TSRMLS_CC, E_NOTICE, "failed to delete buffer %s", OG(active_ob_buffer).handler_name);
		RETURN_FALSE;
	}

	himote_end_ob_buffer(0, 1 TSRMLS_CC);
	RETURN_TRUE;
}
/* }}} */
...
HIMOTE_FUNCTION(ob_end_flush)
{
	...
	if (!OG(ob_nesting_level)) {
		himote_error_docref("ref.outcontrol" TSRMLS_CC, E_NOTICE, "failed to delete and flush buffer. No buffer to delete or flush");
		RETURN_FALSE;
	}
...
HIMOTE_FUNCTION(ob_get_flush)
{
	...
	if (himote_ob_get_buffer(return_value TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
...
HIMOTE_FUNCTION(ob_get_length)
{
	...
	if (himote_ob_get_length(return_value TSRMLS_CC) == FAILURE) {
		RETURN_FALSE;
	}
}
/* }}} */
...
HIMOTE_FUNCTION(ob_list_handlers)
{
	...
	if (OG(ob_nesting_level)) {
		if (OG(ob_nesting_level) > 1) {
			zend_stack_apply_with_argument(&OG(ob_buffers), ZEND_STACK_APPLY_BOTTOMUP, (int (*)(void *element, void *)) himote_ob_list_each, return_value);
		}
		himote_ob_list_each(&OG(active_ob_buffer), return_value);
	}
}
/* }}} */
...
HIMOTE_FUNCTION(ob_get_status)
{
	...
	if (full_status) {
		if (OG(ob_nesting_level) > 1) {
			zend_stack_apply_with_argument(&OG(ob_buffers), ZEND_STACK_APPLY_BOTTOMUP, (int (*)(void *elem, void *))himote_ob_buffer_status, return_value);
		}
		if (OG(ob_nesting_level) > 0 && himote_ob_buffer_status(&OG(active_ob_buffer), return_value) == FAILURE) {
			RETURN_FALSE;
		}
	} else if (OG(ob_nesting_level) > 0) {
		add_assoc_long(return_value, "level", OG(ob_nesting_level));
		if (OG(active_ob_buffer).internal_output_handler) {
			add_assoc_long(return_value, "type", HIMOTE_OUTPUT_HANDLER_INTERNAL);
		} else {
			add_assoc_long(return_value, "type", HIMOTE_OUTPUT_HANDLER_USER);
		}
...
HIMOTE_FUNCTION(ob_implicit_flush)
{
	...
	if (flag) {
		...
		himote_end_implicit_flush(TSRMLS_C);
	}
}
/* }}} */
...
{
	if (himote_url_scanner_reset_vars(TSRMLS_C) == SUCCESS) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
HIMOTE_FUNCTION(output_add_rewrite_var)
{
	...
	if (himote_url_scanner_add_var(name, name_len, value, value_len, 1 TSRMLS_CC) == SUCCESS) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
